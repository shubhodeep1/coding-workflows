...
| `/review-blocked-approve <request-id> <decision-digest>` | PR comment | `scripts/review_rb_judge.sh`, `scripts/orchestrate_poll_process.sh` | Authorizes one pending terminal review-blocked recommendation only when posted by a GitHub User whose current repository `role_name` is exactly `maintain` or `admin`, after the request and while the PR head still matches. Successor issues are adopted only after creator, labels, action/type, head, and title/body digests match the producer-authenticated intent. | `REVIEW_BLOCKED_APPROVAL_V1` request, `REVIEW_BLOCKED_SUCCESSOR_V1` intent/issue marker, and `REVIEW_BLOCKED_APPROVAL_CONSUMED_V1` result marker. Permission or successor lookup failures remain pending. |
...
| `/security-pass-waive <finding_id> [<finding_id> ...]` | Tracking-issue comment by a human with API-verified `maintain` or `admin` role | `scripts/orchestrate_poll_process.sh` (scheduled poller) | Atomically records only unique, fingerprinted findings reported at the current integration head. Waivers suppress one unique exact line-and-code-context `security_defect_context.v1` match; IDs and nearby lines do not match. In terminal `ai:security-pass-failed` it resets and re-audits; in `ai:security-pass-fixing` it only persists. | Unknown, stale, ambiguous, fingerprint-less, or partially valid batches are rejected. Permission lookup failures remain unconsumed for retry. |
...
| `[ai-autofix]` | Commit subject on the PR branch | `.github/workflows/review_autofix.yml` | Continues the autofix loop while the PR stays under review. The model-facing editor step has no repository credential; a later trusted step rechecks PR state before persistence. | Self-trigger suppression keys off PR head SHA plus GitHub-attributed bot identity; iteration tracking counts consecutive first-parent `[ai-autofix]` commits. |
...
Review/autofix executable support comes only from the reusable workflow definition's validated immutable SHA. The privileged job does not install dependencies declared by the PR. Its editor runs as an unprivileged identity in the detached no-`.git` workspace with an empty allowlisted environment and a protected runtime sentinel; trusted post-editor steps restore ownership, recheck PR state, and perform commits. Review-blocked V1 refusal markers are compatibility output only: retry suppression requires a producer-authored, keyring-signed `REVIEW_BLOCKED_DECISION_REFUSED_V2` envelope bound to the current repository, tracking issue, source PR, linked issue, and head SHA.

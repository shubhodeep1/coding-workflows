...
This file is the canonical completed-plan record for tracking issue `#3024`.

The closeout summary below reflects the shipped repository state re-audited on
2026-06-01 UTC. The historical plan text that follows is preserved for
context; where the old future-tense plan conflicts with the closeout notes,
the closeout sections below are authoritative. No acceptance criteria were
silently de-scoped in this archive.

## Closeout summary

- All 13 prompt-hardening learnings in this plan are now present on current
  HEAD; the last archival blocker was the missing Goal 10 / Goal 12 wording in
  `prompts/mode-implement.txt`, and that gap is now closed.
- `prompts/mode-implement.txt` now explicitly bans 1-2 character names except
  in tight scopes, preserves the no-inline-comments guidance, and restores the
  full discovery -> todo/plan -> status update -> batch tool calls ->
  reconcile -> final summary cadence with an explicit "do not start editing
  before the discovery pass completes" rule.
- The re-audit against `unattended_system_instructions.md`, `CLAUDE.md`,
  `prompts/review-reviewer-checklist.txt`,
  `prompts/mode-implement-diagnose.txt`, `prompts/mode-implement-repair.txt`,
  `prompts/mode-implement-repair-syntax.txt`, and
  `docs/ai-tools-future-improvements.md` is clean, so this archive does not
  need a remaining-gaps or de-scoped-work section.

## Evidence snapshot

- System-instruction learnings: `unattended_system_instructions.md`,
  `CLAUDE.md`
- Implement / repair / reviewer prompt learnings:
  `prompts/mode-implement.txt`, `prompts/review-reviewer-checklist.txt`,
  `prompts/mode-implement-diagnose.txt`,
  `prompts/mode-implement-repair.txt`,
  `prompts/mode-implement-repair-syntax.txt`
- Companion backlog doc retained at:

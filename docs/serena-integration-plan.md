...
> in the current tree (verified: `grep -r serena scripts/ prompts/
> .github/workflows/` returns one stray comment in
> `test-and-mark-stable.yml`). The `[Unreleased]` block in `CHANGELOG.md` and
> several `analysis/workflow-optimization-2026-05-0*.md` files describe this
> integration as if it has shipped (e.g. "269 Serena tool calls", "94%
> efficiency", "~85% token savings"); those are aspirational / forward-looked
> entries and are reconciled in §12.
...
| `.serena/project.yml` | **does not exist**; `.gitignore` has no `.serena/` rule |
...
- **Generated by `setup_serena.sh` at runtime** (not committed) so
  that consumer repos with different language mixes get appropriate
  shapes without a per-repo template.
...
- **`.gitignore` rule**: Phase 1 adds `.serena/` to the repo's
  `.gitignore` (in this repo *and* in the consumer-repo install
  guidance) so a developer running interactively never accidentally
  commits the cache.
...
`setup_serena.sh` writes the block using Python (so TOML escaping is
correct). Pseudocode:
...
print(f'command = {shlex.quote(os.environ["SERENA_BIN"])}')
...
(Pseudocode — actual Python uses the `tomllib`/`tomli_w` round-trip
when available, falling back to a hand-emitted block when not. The
real implementation lives in Phase 1.)
...
- Add `scripts/setup_serena.sh`. Responsibilities: install pinned
  ...
  (`scripts/templates/serena_project.yml.j2` or similar — language
  detection by file-extension census), run handshake probe, on
  success append `[mcp_servers.serena]` to `~/.codex/config.toml`,
  export `SERENA_AVAILABLE` to `$GITHUB_ENV`. Fail-soft.
...
- Add `scripts/templates/serena_project.yml.j2`. Minimal shape: list
  detected languages, exclude `.git/`, `node_modules/`, `dist/`,
  `.venv/`, `__pycache__/`, `.serena/cache/`.
- Extend `.gitignore` with `.serena/`.
...
### 6.5 Backward-compatible shape for consumer-repo language mix

`.serena/project.yml` is generated at runtime by `setup_serena.sh`
based on a file-extension census of the workspace. Consumer repos
with unusual mixes (Python + TypeScript + Go in the same repo) get a
multi-language project file automatically. Consumer repos with
languages Serena does not support (or supports poorly) get a
project file that excludes those file extensions; the model still
gets symbol tools for the supported subset.

A repo with *no* supported languages (e.g. a pure-config repo)
results in `setup_serena.sh` exporting `SERENA_AVAILABLE=false` and
logging `SERENA_PROBE result=skipped reason=no-supported-language`.
That repo's runs proceed on the legacy path with no error.
...
- [ ] Phase 1 — `scripts/setup_serena.sh`, `scripts/mcp_handshake_
      probe.py`, `scripts/templates/serena_project.yml.j2`,
      `.gitignore` `.serena/` rule, `tests/test_mcp_handshake_
      probe.py` + fixtures, `agents.md` log-prefix table addition,
      `implement.yml` Setup Serena step (lazy-bootstrapped) + Guard
      0 baseline filter extension,

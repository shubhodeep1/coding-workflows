...
13. Workflows: add the new env rows (all `${{ vars.X || '<default>' }}`) to `auto-release-stable.yml` and the `cycle` job of `promote-main-to-stable.yml`.

...
- `docs/plans/judge-loop-and-reissue-plan.md` — sibling plan covering
  ...
  the two plans must merge their flag namespaces (see Open Questions).
...
6. **Phase F — Re-review awareness.** Make the consolidator and judge
   ...
   `docs/plans/judge-loop-and-reissue-plan.md` Phase A / B before
   landing.**
...
- **F (re-review awareness) lands fifth** — must merge flag
  namespace with `docs/plans/judge-loop-and-reissue-plan.md` Phase
  A / B first; otherwise we'll ship conflicting ledger reads.
...
pattern (mirrored from `docs/plans/judge-loop-and-reissue-plan.md`).
...
`docs/plans/judge-loop-and-reissue-plan.md` Phase A (judge-in-loop)
and Phase B (sticky findings) for flag overlap. The simplest path is
to **rename this phase's flag to match** that plan's `REVIEW_JUDGE_IN_LOOP_*`
namespace, or to share `REVIEW_LEDGER_REREVIEW_ENABLED`. **Open
question Q-OQ-1** — see Open Questions.
...
- New state file `.ai/review_runtime/reviewer_health.json` —
  ...
  (`docs/plans/judge-loop-and-reissue-plan.md` "Context" section).
...
### Per-phase rollout pattern (mirrors `judge-loop-and-reissue-plan.md`)
...
- **Q-OQ-1**: Phase F (re-review awareness) overlaps
  `docs/plans/judge-loop-and-reissue-plan.md` Phase A
...
- `docs/plans/judge-loop-and-reissue-plan.md` — sibling plan;
  Phase F here coordinates with Phases A + B there.

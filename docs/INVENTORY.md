...
- `scripts/workflow_failure_heal_implement_report.sh` — Report a repeated implementation failure on an issue to coding-workflows' heal intake from the implement workflow's failure path, with the run's own evidence and the runs the issue's failure comments link.

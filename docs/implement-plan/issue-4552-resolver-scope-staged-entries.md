...
- Project branch: claude/implement-plan-issue-4552-resolver-scope-staged-entries   Final PR: pending
- Status: IN_PROGRESS
- Stage: phase 1/1
...
- Last note: project branch opened from claude/quirky-wozniak-e9t88m (issue base)

## Phases
1. [ ] Phase 1 — staged-entry scope check, safe ValueError reason, no-staging prompt rule
   - [ ] `_resolver_scope_state.merge_state()` hashes `git ls-files -s -v -z` instead of raw index bytes; MERGE_HEAD unchanged
   - [ ] `ValueError`-only `failure reason:` line; existing `::error::` line unchanged
   - [ ] `prompts/conflict-resolver.txt` no-index-changes rule (≤ 150 lines)
   - [ ] tests: metadata refresh accepted; `git add` in conflicted merge and assume-unchanged rejected (exit 2, reason named); prompt rule present; registered in `main()`
   - [ ] `changelog.d/4552-resolver-scope-staged-entries.md` (fixed)

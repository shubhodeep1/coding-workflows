...
`SAFE_TO_MERGE` is directly implementable; `NEEDS_VERIFICATION` requires specified checks; `RISKY_SKIP` must not be automated because pagination, retries, races, or recovery semantics are involved.
...
- **Files:** `scripts/review_collect_pr_metadata.sh:81-172,251-305`
- **Call count:** Current: 2 GraphQL calls on title/body fallback; proposed: 1 successful combined call, retaining the second call only as failure recovery.
- **Endpoints:** GraphQL `POST /graphql`, querying `closingIssuesReferences` and aliased `issueOrPullRequest`.
- **Evidence:**
  ```sh
  gh_retry "${_linked_tmp}" api graphql ... closingIssuesReferences(first:50)
  ...
  _fallback_json="$(_fetch_linked_issue_bodies_graphql "${_fallback_numbers_json}")"
  ```
  When closing references are empty, fallback issue numbers are hydrated through a second call to the same repository GraphQL object.
- **Proposed fix:** Extend the query at lines 258-263 with the aliases currently generated by `_fetch_linked_issue_bodies_graphql`. Return separate closing-reference and fallback arrays, preferring closing references. Retain `_fetch_linked_issue_bodies_graphql` only when the combined response is missing, partial, or malformed.
- **Safety rationale:** Although both calls use identical auth and have no intervening mutation, they currently provide independent failure boundaries, so error semantics must be proven equivalent.
- **Downstream signal:** Verify closing-reference success, non-default-base fallback, partial GraphQL errors, the 20-issue fallback cap, and recovery through the legacy helper before consolidating.

#### MERGE-002

- **ID:** `MERGE-002`
- **Safety tag:** `RISKY_SKIP`
- **Files:** `scripts/review_merge_train.sh:255-290,331-387,423-486`
- **Call count:** Current: 2 reads for an existing marker; proposed: 1 read.
- **Endpoints:** `GET /repos/{repo}/issues/{pr}/comments` and `GET /repos/{repo}/issues/comments/{id}`.
- **Evidence:**
  ```sh
  ...
  ```
  The paginated list response already contains both comment ID and body.
- **Proposed fix:** Add `_mt_find_marker_comment_into <id-var> <body-var> <pr> <marker>`, returning the latest marker’s ID and body from one snapshot. Update `_mt_gate`, `_mt_release`, and `_mt_upsert_comment` to pass the cached body.
- **Safety rationale:** The source call uses `--paginate`; changing its transform risks altering page-order and latest-marker selection semantics.
- **Downstream signal:** Do not auto-implement; manually test more than 100 comments, duplicate markers across pages, multiline bodies, unchanged-body suppression, and lookup failures.
...
- **Call count:** Current: 2 logical calls when semantic caching is enabled; proposed: 1.
...
- **Evidence:**
  ...
  gh_retry gh api --paginate --slurp "...comments?...&per_page=100" |
    jq ... > "${THREAD_HISTORY_FILE}"
  ```
  The semantic-cache path fetches the first page twice.
- **Proposed fix:** Fetch one paginated snapshot into a temporary file, derive the first 50 comments for `ISSUE_COMMENTS_FILE`, and derive full history for `THREAD_HISTORY_FILE`.
- **Safety rationale:** The consolidation includes pagination and must preserve the current fatal bounded-context fetch versus fail-open semantic-cache-history behavior.
- **Downstream signal:** Do not auto-implement; manually verify first-page, multi-page, later-page failure, malformed JSON, and semantic-cache-disabled behavior.

#### MERGE-004

- **ID:** `MERGE-004`
- **Safety tag:** `RISKY_SKIP`
- **Files:** `scripts/orchestrate_poll_process.sh:13726-13733` (`execute_stall_recovery_action`) and `scripts/orchestrate_poll_process.sh:16413-16421` (`run_standalone_stall_recovery`)
- **Call count:** Current: 2 reads per reissue path; proposed: 1 per path.
...
- **Evidence:**
  ...
  ```
- **Proposed fix:** Fetch `{title, body}` once in each function and derive both variables locally.
- **Safety rationale:** Both sites are inside `orchestrate_poll_process.sh` stall-recovery paths, an explicit `RISKY_SKIP` trigger protecting race-sensitive recovery behavior.
- **Downstream signal:** Do not auto-implement; manually confirm one failed combined fetch preserves the current fail-open reissue behavior and does not lose usable partial metadata.
...
- **ID:** `REUSE-001`
- **Safety tag:** `NEEDS_VERIFICATION`
- **Files:** `.github/workflows/review_autofix.yml:7265-7283` and `.github/workflows/review_autofix.yml:7285-7334`
- **Call count:** Current: 1 read normally, 2 when `PR_META_FILE` lacks title/body; proposed: 1 after a successful first fetch, with the existing second call retained after failure.
- **Endpoint:** `GET /repos/{repo}/pulls/{pr}`.
- **Evidence:**
  ```sh
  pr_meta="$(gh_retry _safe_gh_jq ".../pulls/${PR_NUMBER}" ...)"
  ```
  The fresh payload is used only for state, while the next step may re-fetch title/body:
  ```sh
  PR_DATA="$(gh_retry gh api ".../pulls/${PR_NUMBER}" --jq '.title + " " + (.body // "")' ...)"
  ```
- **Proposed fix:** Persist a normalized title/body snapshot from `pr_meta` in the first step. In “Mark linked issues review-blocked,” read that snapshot before `PR_META_FILE`, retaining the existing API call only when both caches are unavailable.
- **Safety rationale:** The calls occur in separate workflow steps, failing the same-step requirement for `SAFE_TO_MERGE`; artifact persistence and step ordering require verification.
- **Downstream signal:** Verify the cache survives between steps, preserves the `PR_META_FILE` schema, retains the live fallback after first-fetch failure, and update `tests/test_review_autofix_phase_transition_contract.py:104-116`.
...
- BATCH-001: `RISKY_SKIP` — Runs inside `orchestrate_poll_process.sh` and relies on paginated issue-list behavior.
- BATCH-002: `RISKY_SKIP` — Poller race-defense code and paginated comments require manual review.
- BATCH-003: `NEEDS_VERIFICATION` — Verify markers beyond the first 100 comments retain REST overflow fallback.
- BATCH-004: `NEEDS_VERIFICATION` — Cross-repository GraphQL batching must preserve per-item unknown results and partial-failure behavior.

### Summary Counts

Counts include net-new findings and reviewed Deep Audit cross-references.

| Tag | Count | IDs |
|---|---:|---|
| `SAFE_TO_MERGE` | 0 | — |
| `NEEDS_VERIFICATION` | 4 | MERGE-001, REUSE-001, BATCH-003, BATCH-004 |
| `RISKY_SKIP` | 5 | MERGE-002, MERGE-003, MERGE-004, BATCH-001, BATCH-002 |

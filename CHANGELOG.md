...
### Added
- Stable-branch release flow. `test-and-mark-stable.yml` now auto-detects the dispatch ref via a new `source` job: dispatching from the `stable` branch tags from `stable` (a "stable patch release" — old stable + cherry-picked bug fix) instead of from `main`. `resolve-version` patch-bumps from the latest `vX.Y.Z` tag reachable from the source branch (`git tag --merged HEAD …`), so a stable release on top of `v1.4.2` becomes `v1.4.3` even when `main` is on a higher version. The `validate` and `release` jobs check out the source branch and the GitHub Release is created with `--target <source_branch>`. Default behaviour when dispatched from any other ref (including `main`) is unchanged. The moving `stable` git tag — what consumer repos pin to via `@stable` — and the `coding-workflows-stable-released` repository_dispatch are unaffected; consumers automatically pick up stable patch releases with no changes.

...
### Changed
- Upgraded the five-model reviewer panel in
  `.github/workflows/review_autofix.yml` (`REVIEWER_MODELS`) to the latest
  April 2026 OpenRouter slugs: `minimax/minimax-m2.5` →
  `minimax/minimax-m2.7`, `moonshotai/kimi-k2.5` →
  `moonshotai/kimi-k2.6`, `deepseek/deepseek-v3.2` →
  `deepseek/deepseek-v4-pro` (context window 128K → 1M), `z-ai/glm-5` →
  `z-ai/glm-5.1`, `qwen/qwen3-coder-plus` → `qwen/qwen3.6-plus`. Each slug
  was verified against `https://openrouter.ai/api/v1/models`. The
  corresponding `scripts/codex_model_catalog.json` entries were replaced in
  place (slug, display name, description, context window, truncation policy,
  auto-compact threshold) so codex-cli model lookup continues to resolve
  every reviewer slug. Editor (`openai/gpt-5.3-codex`) and cross-reviewer
  summariser (`openai/gpt-5.4-mini`) are unchanged.

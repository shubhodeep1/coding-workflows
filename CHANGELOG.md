...
### Added
- Stable-branch release flow. `test-and-mark-stable.yml` is now restricted to dispatches from the `stable` branch — a new `source` job rejects any other ref with a clear error pointing at `promote-main-to-stable.yml`. This prevents accidentally tagging `main` as stable and dragging in untested in-flight work. `resolve-version` patch-bumps from the latest `vX.Y.Z` tag reachable from `stable`'s HEAD (`git tag --merged HEAD …`), so a stable release on top of `v1.4.2` becomes `v1.4.3` even when `main` is on a higher version. The `validate` and `release` jobs check out `stable` and the GitHub Release is created with `--target stable`. The moving `stable` git tag — what consumer repos pin to via `@stable` — and the `coding-workflows-stable-released` repository_dispatch are unaffected; consumers automatically pick up stable patch releases with no changes.

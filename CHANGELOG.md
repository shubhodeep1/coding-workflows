...
### Changed
- Serena observability/docs follow-through: `scripts/cost_audit.py` now parses `SERENA_QUERY` / `SERENA_FALLBACK` rollups by `target=` with query-call totals, response-byte totals, fallback ratios, per-target breakdowns, and a conservative legacy-prompt estimate that compares against same-run observed prompt tokens when available; `tests/test_cost_audit_semble_metrics.py`, `prompts/mode-workflow-analysis.txt`, `README.md`, and `probably_unnecessary_but_read_if_stuck.md` were updated alongside it. This documents repo-landed behavior only; consumer repos still need a later `@stable` release to inherit the reusable-workflow changes.

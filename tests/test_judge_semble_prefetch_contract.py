...
def test_judge_scripts_wire_semble_prefetch_into_dynamic_prompt_builds() -> None:
	...
	assert 'declare -F semble_query_block' in orchestrate
	assert 'build_wave_judge_semble_query()' in orchestrate
	assert 'build_stall_judge_semble_query()' in orchestrate
	assert 'build_review_blocked_judge_semble_query()' in orchestrate
	assert 'build_integration_conflict_judge_semble_query()' in orchestrate
	...
	assert 'append_semble_query_section "PR diff excerpts:" "${pr_diff_text}" 3200' in review_rb

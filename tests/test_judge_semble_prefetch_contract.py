...
def _run_render(prompt_text: str, *, semble_prefetch: str | None = None) -> subprocess.CompletedProcess[str]:
...
def test_orchestrate_poll_workflow_bootstraps_semble_for_judges() -> None:
	...
	assert "steps.find_tracking.outputs.has_work == 'true' && env.SEMBLE_ENABLED == 'true'" in uv_block

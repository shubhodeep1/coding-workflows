...
class TestValidateProcessDriverGuardScope(unittest.TestCase):
    ...
    def test_unrelated_validate_helper_script_is_allowed(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="validate-guard-allow-") as td:
            repo_dir = Path(td)
            _init_repo(repo_dir)
            _write_file(repo_dir / "scripts/validate_process.sh", "#!/usr/bin/env bash\necho canonical\n")
            _write_file(repo_dir / "scripts/validate_local.sh", "#!/usr/bin/env bash\necho consumer-helper\n")
    ...
    def test_tracked_validation_validate_sh_is_rejected(self) -> None:
        ...
        self.assertIn("validation/validate.sh is tracked by git; it must remain transient.", result.stderr)
    ...
    def test_tracked_duplicate_managed_artifact_copy_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="validate-guard-dup-") as td:
            ...
            _init_repo(repo_dir)
            _write_file(repo_dir / "scripts/validate_process.sh", "#!/usr/bin/env bash\necho canonical\n")
            _write_file(repo_dir / "scripts/validate_process_copy.sh", "#!/usr/bin/env bash\necho canonical\n")
            _commit_all(repo_dir)
        ...
        self.assertIn("Managed validation artifact contract violation detected:", result.stderr)
        self.assertIn("scripts/validate_process_copy.sh", result.stderr)

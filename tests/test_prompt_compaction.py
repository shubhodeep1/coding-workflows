...
def test_compact_if_over_budget_under_budget_passthrough() -> None:
	...
	compacted = openrouter_prompt_cache.compact_if_over_budget(sections, 2)
	assert compacted == sections
	assert compacted is not sections
...
def test_compact_if_over_budget_drops_tier_three_before_tier_two() -> None:
	...
	assert openrouter_prompt_cache.compact_if_over_budget(sections, 4) == [
		sections[0],
		sections[2],
	]

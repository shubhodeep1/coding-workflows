...
def _run_parser(comments: list[dict[str, object]], head_sha: str) -> dict[str, str]:
	...
	env["PARTIAL_MARKER_COMMENTS_JSON"] = json.dumps(comments)
...
def test_gate_declares_terminal_same_head_env_with_defaults() -> None:
	...
	assert (
		"FORCE_RB_JUDGE: ${{ ((github.event_name == 'workflow_dispatch' && github.event.inputs.force_rb_judge == 'true') "
		"|| (github.event_name != 'workflow_dispatch' && inputs.force_rb_judge)) && 'true' || 'false' }}"
	) in gate

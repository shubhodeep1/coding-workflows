...
def test_scope_guard_latch_verifies_label_applied() -> None:
	...
	assert r"The workflow attempted to apply \`ai:scope-blocked\`; verify that the label is present before relying on the \`Validate approval phase\` redispatch block. Once present, future redispatches of issue #${ISSUE_NUMBER} will be refused until a human removes the label." in destructive_block

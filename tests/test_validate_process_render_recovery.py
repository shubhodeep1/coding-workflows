...
def test_render_recovery_relint_failure_routes_to_render_phase_self_heal() -> None:
	script = """#!/usr/bin/env bash
set -euo pipefail
...
"""
	...
	with tempfile.TemporaryDirectory(prefix="validate-render-recovery-relint-") as td:
		tmp_path = Path(td)
		runtime_dir = tmp_path / "runtime"
		runtime_dir.mkdir(parents=True, exist_ok=True)
		env = os.environ.copy()
...
def main() -> int:
	...
	test_render_recovery_relint_failure_routes_to_render_phase_self_heal()

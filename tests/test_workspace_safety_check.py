...
def test_review_workflow_bootstraps_and_restages_workspace_safety_helper() -> None:
	...
	assert 'bash "${SUPPORT_SCRIPTS_DIR}/review_autofix_step_detect_merge_conflicts.sh"' in merge_detect_block
	merge_detect_block += (REVIEW_WORKFLOW.parent.parent.parent / "scripts" / "review_autofix_step_detect_merge_conflicts.sh").read_text(encoding="utf-8")

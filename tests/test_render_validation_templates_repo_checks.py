...
def test_repo_checks_family_honours_requirements_file_slot() -> None:
	with tempfile.TemporaryDirectory(prefix="render-validation-repo-checks-") as td:
		...
		payload["slots"]["requirements_file"] = "requirements/ci'$(touch pwn).txt"
		...
		dockerfile_text = (output_root / "Dockerfile.app").read_text(encoding="utf-8")
		assert "[ -f 'requirements/ci'\"'\"'$(touch pwn).txt' ]" in dockerfile_text
		assert "-r 'requirements/ci'\"'\"'$(touch pwn).txt'" in dockerfile_text
		assert '-r "requirements/ci\'$(touch pwn).txt"' not in dockerfile_text

...
- **Trusted-runner index completion.** Conflict preparation separates initially unmerged paths from fingerprint-only expansions. After the isolated resolver returns, the trusted runner stages only changed paths, fails on staging errors, and requires `git diff --name-only --diff-filter=U --` to be empty before accepting a no-change result or creating the merge-resolution commit. An attempted but incomplete resolver run is reported as `conflict_resolver_failed`, not `clean_review_no_commit`.

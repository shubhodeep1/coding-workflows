...
| `REVIEW_RESOLVE_THREADS_ENABLED` | `true` | Mark a PR review thread resolved once the editor's `PR comment audit:` section gives its comment a disposition. Every audited disposition resolves (`applied`, `already satisfied`, and `ignored`); for `ignored` the editor's reason is posted as a thread reply first, so a reviewer can reopen. Resolution is keyed on the audited comment id, never on a path/line pair. Set `false` to keep threads untouched. |

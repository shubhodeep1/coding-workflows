...
- Review support is checked out once at the gate-verified reusable-workflow support commit (`review_support_sha`; source-repository PRs use protected `main`'s commit instead of the PR head). The privileged job does not install PR-declared dependencies. The writer runs as an unprivileged identity in the detached no-`.git` workspace with an `env -i` allowlist; runner command files, Git metadata, repository credentials, Telegram credentials, and state-auth keys are unavailable to it. Its stall guard publishes runner-owned process-group metadata and limits privileged signalling to that isolated editor group; other stall-guard callers retain normal process-group signalling. Dependency-sensitive local validation may therefore be limited; CI/check-run evidence remains authoritative.

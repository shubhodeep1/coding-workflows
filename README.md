...
| `GH_TOKEN` | No | Interactive Claude Code sessions only (CLAUDE.md §23) — Actions workflows get the same variable name from `secrets.GH_PAT` instead | GitHub Personal Access Token. Set as an env var in the Claude Code session environment (not an Actions secret). Lets interactive sessions reach GitHub data the session's built-in tooling cannot — Actions run/job logs, other repos, endpoints absent from the MCP surface — and perform routine repo writes (push, open/update a PR, comment, label) self-serve; merging, force-push, deletions, repo/org administration, and workflow dispatches always require asking the user first. Scopes: classic `repo` + `workflow` (+ `read:org`), or a fine-grained PAT with contents, pull requests, issues, and `actions: read`. The SessionStart hook installs `gh` and reports (over REST, not `gh auth status`) whether the token authenticates; a missing or invalid token degrades to the `mcp__github__*` tools rather than blocking. **Claude Code on the web does not honour this variable**: the session's agent proxy replaces the `Authorization` header on every `api.github.com` call with its own GitHub App token, so reach is limited to repositories attached to the session and the hook prints a `NOTE` saying so (CLAUDE.md §23.A, "Web sessions"). PAT-backed access works only in local sessions (CLI / desktop / IDE). |

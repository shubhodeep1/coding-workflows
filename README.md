...
Re-plan idempotency is remote as well as state-backed: the single GET described above reads issue state and labels, while each blocked follow-up adds one paginated comments GET to check `<!-- security-pass-advisory-unblock:<tracking>:<issue> -->` before posting. A successful `/answer` followed by a lost state write can repeat the reads on the next merged-state tick, but cannot duplicate the command.

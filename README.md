...
| `TARGETED_FILE_CONTEXT_MAX_FILES` | No | `10` | implement, review_autofix, orchestrate_poll | Hard cap on the number of files inlined verbatim into the editor / autofix-editor / conflict-resolver prompt by `scripts/targeted_file_context.py`. The inlining mechanism pre-loads files named in the plan (implement) / `LAST_RUN_CHANGED_FILES_FILE` (autofix) / conflicted-file allowlist (resolver) so the model's first turn is a write rather than a recon read. Set to `0` to disable inlining entirely (block becomes a single-line "(disabled)" marker). |
| `TARGETED_FILE_CONTEXT_MAX_BYTES` | No | `102400` (100 KB ≈ 25k tokens) | implement, review_autofix, orchestrate_poll | Total byte budget across all inlined files in a single prompt. Files that would push the cumulative byte count over this cap get a `(NN bytes; would overflow total budget — read with read tool)` marker rather than mid-file truncation, so the model uses its native targeted-read flow exactly as it does today on large files. |
| `TARGETED_FILE_CONTEXT_MAX_FILE_BYTES` | No | `51200` (50 KB ≈ 12.5k tokens) | implement, review_autofix, orchestrate_poll | Per-file size cap. Files larger than this are NOT head-truncated and inlined — they get a `(NN bytes; too large to inline — read with read tool, max_file_bytes=…)` marker. Avoids the misleading "first 50KB of a 500KB file" pattern where the edit point lives at the bottom and head-truncation paints a false picture. Lower this on repos with many medium-sized files where head-end content is rarely the edit target; raise this on small-file repos to fit more verbatim content. |

...
| `CONTEXT7_DISABLED` | No | `false` | clarify, plan, implement, review_autofix, orchestrate, orchestrate_poll, orchestrate_clarify_respond, validate | Disable the optional Context7 MCP server. Default was flipped to `true` in #1772 when failed Context7 handshakes leaked an invalid `tools[N]` entry that OpenRouter→Azure rejected with HTTP 400 (#1754); flipped back to `false` in #1940 once `MCP_HANDSHAKE_PROBE_ENABLED` (#1799) was confirmed to gate the malformed-tool-list failure mode. The probe skips the `[mcp_servers.context7]` block on probe failure, so the original 400 cannot recur even with the disable off. Set to `true` to opt out anyway. |
| `GIT_MCP_DISABLED` | No | `false` | clarify, plan, implement, review_autofix, orchestrate, orchestrate_poll, orchestrate_clarify_respond, validate | Disable the optional Git MCP server setup (preloaded diff artifacts remain the fallback). Default reverted to `false` in #1940 alongside `CONTEXT7_DISABLED`; same handshake-probe rationale. |
| `MCP_HANDSHAKE_PROBE_ENABLED` | No | `true` | clarify, plan, implement, review_autofix, orchestrate, orchestrate_poll, orchestrate_clarify_respond, validate | When `true`, `scripts/setup_serena.sh` runs a JSON-RPC `initialize` probe (`scripts/mcp_handshake_probe.py`) against each enabled optional MCP server (Context7, Git) before writing its `[mcp_servers.<name>]` block to `~/.codex/config.toml`. Servers that fail the probe (timeout, EOF mid-handshake, malformed response) are omitted, preventing Codex from emitting a `tools[N]` entry whose `function` field is `undefined` — the failure shape that some OpenRouter back-ends (notably Azure) reject with HTTP 400. Defence-in-depth alongside the `@upstash/context7-mcp@2.1.8` pin from #1705. Set to `false` to bypass the probe (legacy behaviour). |
| `MCP_HANDSHAKE_PROBE_TIMEOUT` | No | `15` | clarify, plan, implement, review_autofix, orchestrate, orchestrate_poll, orchestrate_clarify_respond, validate | Seconds the MCP handshake probe waits for an `initialize` response before declaring the server unhealthy. Must be positive. |
...
| `ALT_EDITOR_MODEL` | No | `anthropic/claude-sonnet-4-6` | test-and-mark-stable (`e2e-alt-model-test` job) | Model used by the release-gate's alternate-model happy-path run. Pinned to a different family than the production `WORKFLOW_EDITOR_MODEL` default so model-coupling regressions surface before stable is tagged. Override per release if you need to test against a specific candidate. Has no effect outside the release gate. |
...
| `LOG_ANALYZER_REASONING` | No | `medium` | test-and-mark-stable (Phase 8 soft-error analyser) | Reasoning effort for `LOG_ANALYZER_MODEL`. `medium` is the default sweet spot between cost and signal. Set to `low` for cheaper runs or `high` if you want deeper triage; values must match what the chosen model accepts. |
...
| `CONTEXT7_DISABLED` | `false` | Disable the optional Context7 MCP server setup in workflows. Default reverted to `false` in #1940 once `MCP_HANDSHAKE_PROBE_ENABLED` was confirmed to gate the malformed-tool-list HTTP 400 from #1754. |
| `GIT_MCP_DISABLED` | `false` | Disable the optional Git MCP server setup in workflows (preloaded diff artifacts remain fallback). Default reverted to `false` in #1940; same handshake-probe rationale. |
| `MCP_HANDSHAKE_PROBE_ENABLED` | `true` | Pre-flight JSON-RPC `initialize` probe (`scripts/mcp_handshake_probe.py`) gates writing each optional MCP server's `[mcp_servers.<name>]` block in `~/.codex/config.toml`. Defence-in-depth against the malformed-tool-list HTTP 400 from #1754 and follow-on Codex/OpenRouter-Azure failures. Set to `false` to skip the probe. |
| `MCP_HANDSHAKE_PROBE_TIMEOUT` | `15` | Seconds the MCP handshake probe waits for an `initialize` response. |

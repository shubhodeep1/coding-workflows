...
| `ORCHESTRATOR_STATE_AUTH_KEYRING` | **Yes for orchestrator deployments** | orchestrate, orchestrate_poll, review_autofix conflict preparation and behavioural-smoke signing, validate behavioural-smoke verification, promote-main-to-stable cycle | Dedicated state, promotion-marker, and behavioural-smoke provenance signing keyring JSON. Schema: `{"schema_version":"orchestrator_state_auth_keyring.v1","active_key_id":"<id>","keys":[{"key_id":"<id>","key_base64":"<base64>"}]}`. IDs are unique, 1–64 safe characters; 1–8 decoded keys of 32–64 bytes are accepted. New signatures use only the active key, while verification accepts retained previous keys. Keep the previous key until every active project, cached behavioural-smoke bundle, and promote cycle has completed under the new active key. Missing or malformed configuration pauses state mutations, rejects comprehensive-cycle tracking bindings, prevents restored behavioural-smoke artifacts from executing, and stops the daily promote cycle before marker dispatch. |
...
| `MODEL_PROVIDER_BROKER_MAX_PROMPT_PRICE` | No | `10` | all model-facing phases | Maximum provider prompt price in USD per million input tokens. A request may lower but cannot raise this server-controlled ceiling. Malformed, negative, or non-finite values prevent broker startup. |
| `MODEL_PROVIDER_BROKER_MAX_COMPLETION_PRICE` | No | `30` | all model-facing phases | Maximum provider completion price in USD per million output tokens. A request may lower but cannot raise this server-controlled ceiling. Malformed, negative, or non-finite values prevent broker startup. |
| `MODEL_PROVIDER_BROKER_MAX_REQUEST_PRICE` | No | `0.10` | all model-facing phases | Maximum provider price in USD per request. A request may lower but cannot raise this server-controlled ceiling. Malformed, negative, or non-finite values prevent broker startup. |
| `MODEL_PROVIDER_BROKER_MAX_IMAGE_PRICE` | No | `1` | all model-facing phases | Maximum provider price in USD per image. A request may lower but cannot raise this server-controlled ceiling. Malformed, negative, or non-finite values prevent broker startup. |
| `MODEL_PROVIDER_BROKER_MAX_INPUT_TOKENS` | No | `16777216` | all model-facing phases | Per-request input-token ceiling enforced by `scripts/model_provider_broker.py` (#4090). The broker conservatively reserves one token per normalized UTF-8 byte and rejects a request over the ceiling with HTTP 400 before forwarding it. The default equals the 16 MiB request-body cap, so it never bites below what the body cap already permits. Valid range `1..16777216`. |
| `MODEL_PROVIDER_BROKER_MAX_TOTAL_INPUT_TOKENS` | No | `1677721600` | all model-facing phases | Aggregate input-token budget per broker session (#4090). Each admitted request reserves its conservative input estimate and is settled to the provider-reported `usage.prompt_tokens` / `usage.input_tokens`; an exhausted budget returns HTTP 429 and the phase fails closed. The default is `MODEL_PROVIDER_BROKER_MAX_REQUESTS x MODEL_PROVIDER_BROKER_MAX_INPUT_TOKENS` (`100 x 16777216`). Must be at least the per-request ceiling and at most `1677721600`. |
| `MODEL_PROVIDER_BROKER_MAX_TOTAL_COST_USD` | No | derived | all model-facing phases | Aggregate decimal cost budget per broker session in USD (#4090). Each admitted request reserves its worst-case cost at the configured price ceilings (input estimate x `MAX_PROMPT_PRICE` + output ceiling x `MAX_COMPLETION_PRICE`, per million tokens, + recognized image inputs x `MAX_IMAGE_PRICE` + `MAX_REQUEST_PRICE`) and is settled to the provider-reported usage. An observed upstream error response or pre-forward failure settles to zero; an ambiguous transport failure after forwarding starts keeps the full reservation. When unset the broker derives the budget from both total token budgets, the request-body-bound maximum image count, and `MAX_REQUESTS x MAX_REQUEST_PRICE`, so it never bites below what those limits already permit. Set it to make the spend control binding; must be a positive finite decimal. |
...
| `REVIEW_CONSOLIDATOR_MODEL` | No | `openai/gpt-6-sol` | review_autofix | Model used by the consolidator stage. |
| `REVIEW_CONSOLIDATOR_REASONING` | No | `high` | review_autofix | Reasoning effort for the consolidator stage. |
...
| `ENABLE_REVIEW_BLOCKED_JUDGE` | No | `true` | review_autofix | When true, non-orchestrator PRs that exhaust autofix iterations invoke a judge (LLM) to recommend merge as-is, push a fix commit, merge_with_followup, or close and reissue. `fix` remains automated; terminal recommendations require an exact `/review-blocked-approve <request-id> <decision-digest>` PR comment from a GitHub User whose current repository role is `maintain` or `admin`, bound to the judged head SHA. Orchestrator-managed PRs are skipped (handled by the poller). PRs without linked issues use the PR title/body as requirement context. |
...
| `CODEX_STALL_GUARD_ENABLED` | No | `false` (`true` by default in review/autofix) | implement, orchestrate_poll, review_autofix, validate | Enables event-idle Codex stall killing in `scripts/codex_stall_guard.sh`. Review/autofix now enables kill mode by default unless a repo explicitly sets `CODEX_STALL_GUARD_ENABLED=false`; when `false`, the helper stays observe-only and emits `codex_stall_observed` telemetry without terminating the child process. |
| `CODEX_STALL_TIMEOUT_SECONDS` | No | `600` | implement, orchestrate_poll, review_autofix, validate | Seconds of heartbeat inactivity before the stall helper records a stall and, when `CODEX_STALL_GUARD_ENABLED=true`, begins termination. Review/autofix keeps the default at `600` so the shared guard fires before the legacy `900`-second reviewer watchdog and well before the `REVIEW_SOFT_DEADLINE_MINUTES` budget. Invalid values fall back to `600`. |
...
| `LEDGER_TOKENS_LOG_MAX_BYTES` | No | `1048576` | implement, review_autofix, validate | Maximum number of bytes `scripts/ledger_emit_substate.sh` reads from a `--tokens-log-file` when extracting token counts. The helper reads that many bytes from the end of the file, snapped to a line boundary; every parser keeps its last match, so the tail carries the same answer as the whole file. Files at or under the bound are read whole. Set to `0` to read the whole file regardless of size (not recommended: the usage scan is quadratic in document length, so a 30 MB editor log costs over an hour of CPU per call). Invalid values fall back to `1048576`. |
| `LEDGER_EMIT_TIMEOUT_SECONDS` | No | `120` | implement, review_autofix, validate | Wall-clock bound on the `scripts/ai_memory.py record-run-event` call that `ledger_emit_substate.sh` makes. On expiry the helper warns `record-run-event failed (timeout:...)`, leaves the dedupe key unconsumed so a later substate can retry, and returns success — telemetry never fails or stalls the caller. Set to `0` to disable the bound. Invalid values fall back to `120`. |
...
| `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` | No | `promote-main-to-stable.yml` | orchestrate_poll | Workflow the comprehensive release callback (12f) dispatches when a tracking issue labelled `ai:comprehensive-test-pending` completes. The default fast-forwards `stable` to the default branch and runs the full release gate; set `test-and-mark-stable.yml` (with `COMPREHENSIVE_RELEASE_WORKFLOW_REF=stable`) to restore the stable-only patch path. |
| `COMPREHENSIVE_RELEASE_WORKFLOW_REF` | No | `main` | orchestrate_poll | `--ref` passed with `COMPREHENSIVE_RELEASE_WORKFLOW_FILE`. |
| `PROMOTE_CYCLE_ENABLED` | No | `true` | promote-main-to-stable (cycle job) | Kill switch for the daily promote cycle in `.github/workflows/promote-main-to-stable.yml` (schedule `0 0 * * *`). The cycle promotes `main` only after a code change since the `stable` tag, a passing `gate_only` smoke gate, and two apply-analysis orchestrator runs (proving, then verifying). `false` logs `PROMOTE_CYCLE_SKIPPED reason=disabled`. Manual `workflow_dispatch` promotion is unaffected. |
| `PROMOTE_CYCLE_MIN_DOCS` | No | `2` | promote-main-to-stable (cycle job) | Analysis docs that must be available before a cycle starts (one for the proving run, one for the verifying run). Fewer logs `PROMOTE_CYCLE_SKIPPED reason=insufficient_docs`. |
| `PROMOTE_CYCLE_GATE_WAIT_SECS` | No | `18000` | promote-main-to-stable (cycle job) | How long the cycle gives its newly dispatched `gate_only` smoke gate to finish before failing the tick (`PROMOTE_CYCLE_FAILED reason=smoke_gate_timeout`). This budget starts after dispatch, so waiting for an earlier gate does not reduce it. Keep this plus `PROMOTE_CYCLE_GATE_IDLE_WAIT_SECS` below the cycle job's 340-minute cap. |
| `PROMOTE_CYCLE_GATE_IDLE_WAIT_SECS` | No | `1800` | promote-main-to-stable (cycle job) | How long the cycle waits for existing `test-and-mark-stable.yml` runs on any branch to finish before skipping the tick (`PROMOTE_CYCLE_SKIPPED reason=gate_busy`). Keep this plus `PROMOTE_CYCLE_GATE_WAIT_SECS` below the cycle job's 340-minute cap. |
| `PROMOTE_CYCLE_GATE_POLL_SECS` | No | `60` | promote-main-to-stable (cycle job) | Poll interval used both while waiting for the gate workflow to become idle and while waiting for the cycle's dispatched smoke gate to finish. |
| `PROMOTE_CYCLE_MAX_ATTEMPTS` | No | `3` | promote-main-to-stable (cycle job) | How many failed scheduled cycle runs (smoke gate failed or timed out, dispatch failed, job cancelled) may cover the same `main` tip before the tip is held. Below the budget the next daily tick retries the tip, so a transient gate failure recovers on its own; at the budget the tick logs `PROMOTE_CYCLE_SKIPPED reason=no_code_changes_since_failed_run ... attempts=N max=M` until `main` moves with a code change (a workflow-heal fix forward-merged from `stable` is such a move) or an operator promotes by hand. Failed runs are walked newest first and the walk stops at the first one separated from the tip by a code change, so the tick issues at most this many extra compare calls. A cancelled smoke gate (`reason=smoke_gate_cancelled`) is a skip, not an attempt. Must be a positive integer. |
| `APPLY_ANALYSIS_ON_MAIN_ENABLED` | No | `true` | promote-main-to-stable (cycle job), orchestrate_poll | Kill switch for `scripts/apply_analysis_on_main.sh`, the dispatcher that hands one `analysis/workflow-optimization-*.md` doc to the orchestrator for the proving and verifying runs. `false` logs `APPLY_ANALYSIS_SKIPPED reason=disabled`, which ends the cycle before the proving run. |
| `COMPREHENSIVE_PROMOTION_HOLD_MAX_SECS` | No | `21600` | orchestrate_poll | Longest the poller holds a verifying run's final merge while the pinned release runs. Past the cap the merge proceeds, nothing is promoted, and a CRITICAL alert names the release run. |
...
| `COMPREHENSIVE_CYCLE_MARKER_TRUSTED_ASSOCIATIONS` | No | `OWNER,MEMBER,COLLABORATOR` | orchestrate_poll | Compatibility-only setting retained for existing configuration. It no longer grants marker authority. Promote-cycle markers must be authored by immutable GitHub Actions bot user ID `41898282` and carry a valid, tracking-issue-bound `comprehensive_cycle_marker.v1` HMAC envelope signed by `ORCHESTRATOR_STATE_AUTH_KEYRING`; unsigned, tampered, cross-issue replayed, or differently authored markers are untrusted. |
...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | No | `false` | orchestrate_poll | Allow terminal `escalate_human` stall actions; when `false`, both declarative and judged stall actions downgrade `escalate_human` to the nearest prior non-human phase action. Exception: if the stall judge deliberately returns `escalate_human` on `MAX_JUDGE_REPLAY` consecutive evaluations for the same issue, it is honored even when `false`, so a judge that keeps escalating cannot loop forever. |
...
| `MAX_RECOVERY_ATTEMPTS` | No | `3` | orchestrate_poll | Maximum project-level recovery cycles when the judge declares failure. Replaces the previous single-shot `recovery_attempted` boolean with a configurable counter. By default a failed verdict only consumes budget when its normalized fingerprint was already recorded on an earlier failed verdict of the same project (see `RECOVERY_COUNT_DISTINCT_FINDINGS`); a brand-new finding is free, so a project that fixes a different real defect each cycle keeps converging while `MAX_JUDGE_CYCLES` remains the absolute cap. When the budget is exhausted the poller still files the judge's fix-up issues, cites them in the `## Project Failed` comment, and keeps the project `failed` until `/judge_resume`. |
| `JUDGE_REPEAT_FINGERPRINT_MAX` | No | `2` | orchestrate_poll | Circuit-breaker cap on consecutive identical normalized judge-failure fingerprints. When the same normalized failure repeats more than this threshold, the poller escalates to `ai:blocked` and stops spawning additional judge-driven recovery cycles for that project. Operates independently of `MAX_RECOVERY_ATTEMPTS`: this is the loop detector for back-to-back identical findings, while the recovery budget charges any repeat (consecutive or ping-pong) of a finding already in the project's seen-on-failure ledger. |
| `RECOVERY_COUNT_DISTINCT_FINDINGS` | No | `false` | orchestrate_poll | Recovery-budget accounting for judge failures. `false` (default): a failed verdict consumes `MAX_RECOVERY_ATTEMPTS` budget only when its normalized fingerprint is already in the tracking state's `judge_failed_fingerprints` ledger (any earlier failed verdict of this project, consecutive or ping-pong); a never-seen finding is not charged; an empty fingerprint is always charged. `true`: legacy behaviour, every failed verdict consumes one attempt. `/judge_resume --reset-recovery` (or `--force`) also clears the ledger. Added after tele-funtoken-msg-scoring#3928 fixed a distinct defect on each of five consecutive judge cycles and was killed by the budget while the repeat-fingerprint breaker never fired. |
...
| `CONFLICT_DISPATCH_COOLDOWN_SECS` | No | `900` | orchestrate_poll | Minimum seconds between consecutive review/autofix dispatches against the same orchestrator integration-branch final PR and between retryable integration-judge attempts. Prevents the self-healing loop from re-dispatching the resolver or re-invoking a failed judge every poll tick. |
| `INTEGRATION_CONFLICT_MAX_RETRIES` | No | `3` | orchestrate_poll | Circuit-breaker budget for automated integration-branch conflict resolution. The self-healing path attempts the `main -> integration_branch` sync via GitHub's merges API; on an HTTP 409 conflict, the poller dispatches `_dispatch_review_for_conflicts` for the final integration PR. After this many consecutive unresolved ticks, the orchestrator escalates to the judge with full PR context. Retryable judge execution, output-validation, live-head, comment-publication, and redispatch failures preserve conflict state for the next poll and count against `INTEGRATION_CONFLICT_LIFETIME_MAX`; invalid trusted inputs still fail closed. Applies to **non**-`orchestrator/project-*` integration branches; sync conflicts on orchestrator-owned integration branches use the tighter `INTEGRATION_SYNC_CONFLICT_MAX_RETRIES` instead. |
...
| `ORCH_SYNC_CONTRACT_LIST_UNION_ENABLED` | No | `true` | orchestrate_poll | Enable the deterministic pre-resolver for pure two-sided `read_entrypoints` / `write_entrypoints` list appends in `db/contracts/*.yml` and `*.yaml`. Eligible conflicts are validated locally, committed, and pushed without a review/autofix resolver dispatch; unsupported shapes fail open to the existing conflict path. Set to `false` to restore the legacy resolver flow exactly. |

The helper still enforces the current hardened limits: 1 MiB per input, at most 4,096 scalar strings of at most 4,096 characters per entrypoint list, and a fixed 10-second isolated-Python timeout.
...
| `BRANCH_REBUILD_THRESHOLD_HOURS` | No | `24` | orchestrate_poll | Minimum hours the final PR's producer-authenticated `AUTOFIX_RESOLVER_RETRY_STATE_V2.escalated_at` must remain unchanged before the poller may delete/recreate the integration branch from default branch and replay merged sub-PR commits. |
...
| `ALERT_MSG_LEVEL` | No | `DEBUG` (except `update_workflows` → `SILENT`) | clarify, plan, implement, review_autofix, orchestrate, orchestrate_poll, orchestrate_clarify_respond, validate, issue_pr_status, update_workflows, test-and-mark-stable | Minimum Telegram alert level to send. Alerts below this threshold are suppressed. Valid values: `DEBUG`, `WARNING`, `ERROR`, `CRITICAL`. Each alert is prefixed with an icon and level (e.g. `🔍 DEBUG:`, `⚠️ WARNING:`, `❌ ERROR:`, `🚨 CRITICAL:`). New alerts default to `CRITICAL` until explicitly recategorised. **Exception:** `update_workflows.yml` defaults its in-workflow `ALERT_MSG_LEVEL` env to `SILENT` so the per-run `🔍 DEBUG: Workflow wrappers updated in <repo>…` notification is suppressed by default; consumer repos that want the alert back can set `vars.ALERT_MSG_LEVEL=DEBUG` (or pass `alert_msg_level=DEBUG` on `workflow_dispatch`). **Exception:** `review_autofix.yml`'s "PR processed: #N" notification is gated independently by the dedicated `PR_PROCESSED_ALERT_LEVEL` knob (default `SILENT`; set `vars.PR_PROCESSED_ALERT_LEVEL=DEBUG` to re-enable). The remaining `review_autofix.yml` Telegram sends continue to honour `ALERT_MSG_LEVEL`. **Smoke-test precedence:** when a pipeline run detects the `test-and-mark-stable.yml` smoke fixture it exports `ALERT_MSG_LEVEL=SILENT` to the job env, and every Telegram step reads that job-env value first (`${{ env.ALERT_MSG_LEVEL \|\| vars.ALERT_MSG_LEVEL \|\| 'DEBUG' }}`, likewise ahead of `PR_PROCESSED_ALERT_LEVEL` / `CONFLICT_RESOLVED_ALERT_LEVEL`), so the gate's single pass/fail message is the only notification a release run produces. A step-level `env:` value overrides `$GITHUB_ENV`, which is why the step must consult `env.` explicitly; `tests/test_smoke_alert_silencing_contract.py` pins this. `validate.yml` takes the same threshold through its `alert_msg_level` input (the gate passes `SILENT`; empty honours `vars.ALERT_MSG_LEVEL`), and `check_failure_triage.yml` runs SILENT for PRs carrying the `e2e-smoke-test` label. |
...
| `EDITOR_MAX_WALL` | No | `7800` (implement); `3300` (review_autofix, script default in `scripts/review_apply_fixes.sh` — `review_autofix.yml` does not forward the repo var) | review_autofix, implement | Maximum wall-clock seconds per editor attempt (~130 min for implement, 55 min for review_autofix). Budget-aware: auto-capped to remaining job time minus a 2-min buffer. The 3-hour job cap typically allows about one full-length attempt; retries are only possible when earlier attempts finish well under the wall cap. A watchdog kill near the 130-min limit consumes most of the remaining job budget, so undersizing per-issue scope (the orchestrator's 60-minute target) is mandatory. A watchdog kill (wall-time or idle) ends only that attempt: the editor retry loop continues with its remaining attempts and otherwise writes the fallback summary, and the killed attempt's stderr is archived as `editor_attempt_<n>.err` in the failure-log artifact. |
| `EDITOR_ISOLATION_KILL_VERIFY_ATTEMPTS` | No | `20` | review_autofix | Number of 0.1-second survivor checks before isolated-editor cleanup may restore workspace ownership. Must be an integer in `1..100`; invalid values fall back to `20`. Exhaustion fails closed and leaves ownership restoration blocked. |
| `TARGETED_FILE_CONTEXT_MAX_BYTES` | No | `102400` (100 KB ≈ 25k tokens) | implement, review_autofix, orchestrate_poll | Hard UTF-8 byte limit for the complete targeted-file-context block built by `scripts/targeted_file_context.py`, including its header, file wrappers, missing/binary/overflow markers, Semble/read fallback content, summaries, and omission notices. At most 256 normalized paths of at most 1024 UTF-8 bytes each are accepted before filesystem access. Only complete structural fragments are appended; entries that do not fit are omitted or represented by a complete marker when space permits, and output never exceeds this value. Set to `0` for an intentionally empty block. |
| `IMPLEMENT_PROMPT_CODEX_STDIN_CAP_CHARS` | No | `1048576` | implement | Character cap that codex-cli enforces on `turn/start` stdin. `implement.yml` measures the assembled `${CODEX_PROMPT_FILE}` in characters (`wc -m` under `C.UTF-8`; a non-UTF-8 locale degrades to a byte count, which is never smaller and so only warns early), logs that count alongside the byte size, and emits a `::warning::` when the character count is at or over this value. Diagnostic only — it never truncates the prompt and never fails the step. It exists because overshooting the cap fails every attempt before the editor runs, with only an opaque `Input exceeds the maximum length of 1048576 characters` on stderr, which the retry loop then misreads as an ordinary no-actionable-output bail. A non-numeric or non-positive value falls back to the default. Counterpart of `REVIEWER_PROMPT_CODEX_STDIN_CAP_BYTES` in `scripts/review_run_reviewers.sh`, which compares bytes against the same cap; this one compares in the cap's own unit. |
...
| `SECURITY_AUDIT_ENABLED` | No | `true` | security-audit (source repo + consumers via `ai-security-audit.yml`) | Default-on gate for `.github/workflows/security-audit.yml` / `scripts/security_audit.sh` (consumers run it through the synced `workflow-templates/ai-security-audit.yml` wrapper, which needs `OPENROUTER_API_KEY` and optionally `GH_PAT` in the consumer repo). When enabled, the weekly/manual default-branch audit renders `prompts/mode-security-audit.txt`, appends a dated section to the stable `AI Security Audit Tracker` issue (`ai:security-audit`), and opens one `ai:security` follow-up issue for every finding that survives confidence-gate and false-positive-exclusion filtering and has no marked follow-up yet (no per-run or weekly cap). Set `false` per repo to disable; the workflow then exits cleanly before Codex or GitHub side effects. |
| `SECURITY_AUDIT_CRON` | No | `0 8 * * 0` | security-audit (documentation-only) | Documentation mirror for the literal weekly schedule in `.github/workflows/security-audit.yml` and `workflow-templates/ai-security-audit.yml`. GitHub Actions `on.schedule` cannot consume repo vars, so this value is not read at runtime; keep the docs row aligned with the workflows' hard-coded cron. |
| `SECURITY_AUDIT_CONFIDENCE_GATE` | No | `8` | security-audit | Minimum confidence score (1-10) a finding must meet before `scripts/security_audit.sh` surfaces it on the tracker or creates a follow-up issue. Applied before the false-positive exclusion catalog. |
| `SECURITY_AUDIT_FP_EXCLUSIONS` | No | `scripts/security_audit_fp_exclusions.json` | security-audit | Path to the editable JSON false-positive exclusion catalog consumed by `scripts/security_audit.sh`. The weekly/manual workflow resolves relative paths against its SHA-verified support checkout, accepts absolute paths only inside that checkout (never under `audit-data/`), and requires the catalog to be unchanged tracked support content, never a file from the audited branch. The script itself also resolves the path only beneath `SECURITY_AUDIT_SUPPORT_DIR` and canonicalizes it, so traversal and symlink escapes fail closed for every caller. Consumer overrides that previously selected an audited-repo copy must move the catalog into trusted workflow support or the audit fails closed. Missing, malformed, escaping, or matcher-less catalogs fail the run closed rather than suppressing findings. |
| `SECURITY_AUDIT_SKIP_IF_UNCHANGED` | No | `true` | security-audit | Skip the whole audit (log-only, no Codex call, no tracker comment) when the default-branch HEAD equals the last audited commit recorded on the tracker body via the `<!-- ai:security-audit-last-sha:… -->` marker. First runs and non-git checkouts fail open to a full audit. |
| `SECURITY_AUDIT_INCREMENTAL` | No | `true` | security-audit | Scope the audit to commits since the last audited commit: the prompt lists the changed files and the post-filter drops findings citing unchanged files (`suppressed_out_of_scope` in the filter summary). Falls back to a full audit on first runs, history rewrites, or diffs over 200 files. The model keeps read access to the whole repository for cross-file tracing. |
...
|---|---|---|---|
| `THINKING_LEVEL_CLARIFY` | `high` | clarify | Reasoning effort for the clarification phase |
...
| `THINKING_LEVEL_PLAN` | `high` | plan | Reasoning effort for the planning phase |
| `THINKING_LEVEL_IMPLEMENT` | `high` | implement | Reasoning effort for the implementation phase |
| `THINKING_LEVEL_IMPLEMENT_REPAIR` | `high` | implement | Reasoning effort for the post-Codex syntax-repair sub-phase (`MODEL_REPAIR_REASONING_EFFORT`). |
| `THINKING_LEVEL_DIAGNOSE` | `high` | implement | Reasoning effort for the post-Codex diagnose sub-phase (`MODEL_DIAGNOSE_REASONING_EFFORT`). |
...
| `THINKING_LEVEL_REVIEW_BLOCKED_JUDGE` | `high` | review_autofix | Reasoning effort for the review-blocked judge (non-orchestrator PRs) |
| `THINKING_LEVEL_ORCHESTRATE` | `high` | orchestrate | Reasoning effort for project decomposition |
| `THINKING_LEVEL_JUDGE` | `high` | orchestrate_poll | Reasoning effort for judge evaluation |
| `THINKING_LEVEL_CLARIFY_RESPOND` | `high` | orchestrate_clarify_respond | Reasoning effort for auto-answering clarification questions |
| `THINKING_LEVEL_VALIDATE` | `high` | validate | Reasoning effort for runtime validation harness generation and diagnosis |
| `MODEL_REASONING_EFFORT_DISCOVER` | `high` | validate | Per-phase override applied only to the validate-discover step (`.ai/validate.yml` hint generation). Patched into `~/.codex/config.toml` before the discover codex call and restored to `THINKING_LEVEL_VALIDATE` on both the normal exit path and the EXIT trap so abnormal exits cannot leak the override. Accepted values: `xhigh`, `high`, `medium`, `low` (`none` is rejected to match the catalog's advertised levels for the default `gpt-6-sol` model). |
...
A deliberate `BLOCKED: <reason>` verdict from the implementer (credentials, access, an
approval, or a product decision the pipeline cannot supply) parks the issue in `ai:blocked`
with the reason and the resume instructions in a comment, mirroring the plan phase. The
orchestrator poller treats `ai:blocked` as human-gated, so stall recovery and re-issuance do
not re-run the same blocked plan. Resume with `/answer` to re-plan, or remove `ai:blocked`,
...
>
> **Terminal same-head skip** — The partial-finalize path caps same-head
> resume rounds at `REVIEW_MAX_RESUME_ROUNDS` and records the terminal
> outcome (`resume_state=no_progress` or `round_budget_exhausted`,
> `resume_should_continue=false`, keyed on `head_sha=`) in the fenced block
> of its `<!-- REVIEW_AUTOFIX_PARTIAL_V1 -->` PR comment. Because the
> `Internal: AI Review Autofix Sweep` cron re-dispatches every open PR every
> 30 minutes, a terminal head used to be re-run indefinitely: PR #4077 went
> terminal on `0d30bc69` at 2026-09-12T15:55Z and the sweep dispatched about
> 25 more same-head runs over the next 12.5 hours, each restoring the cached
> terminal state and exiting neutral after roughly 6 minutes of runner setup
> (for example run 34703936345). With `AUTOFIX_SKIP_TERMINAL_SAME_HEAD`
> (default `true`) the `gate` job now reads the newest marker authored by the
> account authenticated through `GH_PAT` for the PR's current head on
> `workflow_dispatch` events and sets `should_run=false`
> with `skip_reason=terminal_same_head` when it is terminal, logging
> `AUTOFIX_GATE_SKIP reason=terminal_same_head pr=<n> head_sha=<sha>
...
> from other authors are ignored. An authenticated-user lookup, comments API,
> or parse failure logs `AUTOFIX_GATE_TERMINAL_SAME_HEAD_QUERY_FAILED` and
> falls open to a normal run. `AUTOFIX_GATE_NO_SKIP_TERMINAL_SAME_HEAD` and
> `AUTOFIX_GATE_TERMINAL_SAME_HEAD_UNCHECKED` record the non-skip decisions.
> The head SHA rides on the gate's existing `/pulls/{n}` fetch; eligible
> dispatches add one `/user` read and one paginated `/issues/{n}/comments`
> read. Reusable workflows retain the caller's `github` context, so the
> sweep's `internal-review.yml` dispatch still enters this gate as a
  ...
  workflow_dispatch:
    inputs:
      ...
      alert_msg_level:
        ...
        default: ""
      target_ref:
        description: Branch to validate instead of the default branch (empty = default branch or the tracking issue's integration branch)
        required: false
        type: string
        default: ""
permissions:
...
> **Changelog fragment delivery contract:** `update_workflows.yml` also
> delivers the `changelog.d/` fragment mechanism (`CLAUDE.md` §20) to every
> repo in `.github/ai/consumer_repos.json`. The `Sync changelog fragment
> assets from upstream` step is **additive only** — unlike the `.claude/` and
> root `CLAUDE.md` syncs, it never overwrites consumer content. It creates
> `changelog.d/.gitkeep` and adds a delimited managed block carrying
> `CHANGELOG.md merge=union` to `.gitattributes`, leaving any
> consumer-local attribute rules intact. The sibling `Assemble changelog
> fragments` step then folds accumulated `changelog.d/*.md` fragments into
> `CHANGELOG.md` and deletes them, so the consumer's changelog stays current
> without an operator or a release workflow (consumer repos have neither).
> Both steps run upstream's `scripts/assemble_changelog.py` from the
> `@stable` support checkout, so the consumer never carries a copy of the
> script and the §20 rule cannot drift from the machinery implementing it.
> The assembler auto-detects the repo's changelog layout — Keep a Changelog
> (`## [Unreleased]` + `### Added`/`### Changed`/`### Fixed`) or bare
> `## YYYY-MM-DD` date headings — so no consumer has to convert its history,
> and insertion is purely additive (no existing line is removed or
...
| `issue_pr_status.yml` | `pull_request.closed` | Label/state sync + final lineage closure; executable support is staged only from the reusable workflow's validated immutable `job.workflow_sha` |
...
- **Intake (coding-workflows):** `scripts/workflow_failure_heal_intake.sh`
  ...
  (`WORKFLOW_HEAL_MODEL`, default `openai/gpt-6-sol`, `high`) with
...
|---|---|---|
| `WORKFLOW_EDITOR_MODEL` | `openai/gpt-6-sol` (every phase) | Model for code editing / reasoning tasks. The previous legacy editor split (patch-heavy phases on a separate older slug) was retired after openai/codex#11151. See main table above. |
...
| `REVIEW_SOFT_DEADLINE_MINUTES` | `210` | Default-on soft run budget for review/autofix. The workflow exports `JOB_START_EPOCH` and `CODEX_RUN_BUDGET_*` from it, `CODEX_HEARTBEAT` lines add `budget_elapsed_secs` / `budget_remaining_secs` when the contract is present, `codex_stall_killed` + `REVIEWER_ADVANCE: slot=... model=... reason=... next_action=...` make reviewer stall recovery visible in logs, and the terminal `REVIEW_AUTOFIX_RUN_SUMMARY_V1` record reports the run's final budget totals plus `stall_recovery.{killed_attempts,advanced_slots,recovered_slots,partial_finalize_requested}`. Soft-deadline or recoverable partial-finalize exits stay green only after the existing validation/publish tail can still finish; otherwise the run posts findings only, withholds unpushed edits for safety, and records that fallback in the partial marker/summary. Genuine config/auth/push/input faults still fail red. |
...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | `false` | Allow terminal `escalate_human` stall actions; when `false`, both declarative and judged stall actions downgrade `escalate_human` to the nearest prior non-human phase action. Exception: a judged `escalate_human` repeated on `MAX_JUDGE_REPLAY` consecutive evaluations for the same issue is honored even when `false`, breaking the retrigger loop |
...
| `MAX_RECOVERY_ATTEMPTS` | `3` | Max project-level recovery cycles (judge failure → auto-fix). Charged only on a repeat of an already-seen failure fingerprint unless `RECOVERY_COUNT_DISTINCT_FINDINGS=true`; exhaustion still files the judge's fix-up issues before marking the project `failed` |
...
| `JUDGE_PR_DIFF_MAX_BYTES` | `65536` | Byte cap on each PR diff embedded in the wave judge and inline review-blocked judge prompts, applied after their existing 500-line and 1000-line caps. A diff over the cap is cut to a UTF-8-safe prefix and prefixed with a `[NOTE: PR diff is N bytes ... truncated ...]` line so the judge knows to read the files directly. Both paths log byte and character counts and skip futile codex attempts when the assembled prompt exceeds 1,048,576 characters. Exists because line caps alone do not bound minified bundles with single 40-70 KB lines (run 35425771769, project #3928). Non-numeric or non-positive values fall back to the default |
| `JUDGE_PR_DIFFS_TOTAL_MAX_BYTES` | `524288` | Shared byte budget across every PR diff in one judge prompt. Consumed merged PRs first (sorted issue order, so the cache-stable merged block never depends on an in-flight PR's size), then open PRs get the remainder; a PR that finds the budget exhausted or cannot be truncated safely is replaced by a `[NOTE: PR diff elided ...]` line. The poller logs the prompt's byte and character counts once before the first codex exec, warns above 950,000 characters, and skips futile attempts when the prompt exceeds codex's 1,048,576-character limit. Non-numeric or non-positive values fall back to the default |
| `RECOVERY_COUNT_DISTINCT_FINDINGS` | `false` | `false`: a failed judge verdict consumes recovery budget only when its fingerprint is already in the project's `judge_failed_fingerprints` ledger (new findings are free, empty fingerprints always charged). `true`: legacy accounting, every failed verdict is charged |
...
| `CONFLICT_DISPATCH_COOLDOWN_SECS` | `900` | Min seconds between consecutive resolver dispatches and retryable integration-judge attempts against an integration-branch final PR |
...
| `INTEGRATION_CONFLICT_LIFETIME_MAX` | `10` | Cumulative cap on resolver dispatches plus judge escalation attempts per integration branch across all retry episodes; once reached, `heal_integration_branch_conflict` flips `status=failed` and stops dispatching. Catches resolver/judge alternation and repeated retryable judge failures without terminalizing a project on one transient failure |
...
| `THINKING_LEVEL_CLARIFY` | `high` | Reasoning effort for clarification (`xhigh`, `high`, `medium`, `none`) |
...
| `THINKING_LEVEL_PLAN` | `high` | Reasoning effort for planning |
| `THINKING_LEVEL_IMPLEMENT` | `high` | Reasoning effort for implementation |
| `THINKING_LEVEL_ANALYSIS` | `high` | Reasoning effort for the workflow-log-analysis API-redundancy pass (deep-audit is hardcoded at `xhigh` in the workflow YAML; edit there to override) |
...
| `THINKING_LEVEL_ORCHESTRATE` | `high` | Reasoning effort for project decomposition |
| `THINKING_LEVEL_JUDGE` | `high` | Reasoning effort for judge evaluation |
...
| `EDITOR_ISOLATION_KILL_VERIFY_ATTEMPTS` | `20` | Number of 0.1-second isolated-editor survivor checks before workspace ownership restoration; accepts `1..100`, invalid values fall back to `20`, and exhaustion fails closed |
...
| `THINKING_LEVEL_CLARIFY_RESPOND` | `high` | Reasoning effort for auto-answering clarification questions |
...
| `REVIEW_LEDGER_PATH` | `.ai/review_issue_ledger/pr-${PR_NUMBER}.txt` | Runtime ledger file path used by `scripts/review_issue_ledger.sh`. Per-PR filename isolates concurrent PRs so they never share a file (no cross-PR merge conflicts on main). Gitignored by default; cross-iteration persistence is provided by `actions/cache` restore/save steps in `review_autofix.yml` keyed on `review-ledger-<repo>-pr-<N>-`. Those steps cache a run-independent, repository/PR-scoped staging dir (`${RUNNER_TEMP}/review-ledger-cache/<owner>/<repo>/pr-<N>/…`) rather than the per-run workspace, because actions/cache hashes the path list into the cache version and a per-run path made every restore miss; the stable scope also prevents concurrent PRs on one self-hosted runner from clobbering each other's staging data. The adjacent `Stage … review-issue ledger …` steps copy between the staging dir and the workspace (`REVIEW_LEDGER_CACHE_STAGE_IN` / `REVIEW_LEDGER_CACHE_STAGE_OUT` log lines). Explicit overrides are honored verbatim (legacy single-file path still supported). Malformed prior ledgers fail-open with `ledger_reset=1` and state reset semantics. |
...
| `REVIEW_CONSOLIDATOR_MODEL` | `openai/gpt-6-sol` | Model slug used for the consolidator stage in `review_autofix.yml`. |
| `REVIEW_CONSOLIDATOR_REASONING` | `high` | Reasoning effort for the consolidator model call. |
...
| `ORCH_INTEGRATION_MAX_AHEAD_COMMITS` | `10` | Backpressure **floor**; the effective threshold is `max(this, planned_issue_count + ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN)` so a project's own merges never self-deadlock backpressure. Evaluated against the integration branch's non-merge commits ahead of default (the poller's `chore: sync main` merge commits are excluded; falls back to raw `ahead_by` when the compare commit list is unavailable/truncated). Activates `ai:integration-backpressure`, pauses additional sub-issue merges, and auto-clears once the backlog drops below the effective threshold |
| `ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN` | `20` | Headroom added to a project's planned sub-issue count when deriving the size-aware backpressure threshold (judge merges + fix-up issues; sync-merges no longer count). Raised from `5` to `20` after the #2974 integration over-drift deadlock (22 commits ahead vs the old `max(10, 10+5)=15` threshold). Non-negative integer; script-level fallback `5` |
...
| `REVIEWER_SLOT_RETRYABLE_FAILURE_LIMIT` | `3` | Default-on slot-local cap for retryable reviewer recovery. The cheaper-reasoning retry still runs first when enabled, then any mapped same-family failback model can consume the remaining bounded attempts; once the slot limit is exhausted, the slot ends as `failed` or `skipped_budget` and the workflow falls through to the next reviewer slot/model instead of burning the whole soft deadline. |
| `REVIEWER_SLOT_BACKOFF_BASE_SECS` | `2` | Base seconds for full-jitter exponential backoff between retryable reviewer attempts. |
| `REVIEWER_SLOT_BACKOFF_CAP_SECS` | `30` | Per-retry ceiling (seconds) for reviewer-slot backoff after exponential growth and budget caps are applied. |
| `REVIEWER_SLOT_BACKOFF_BUDGET_RATIO` | `0.05` | Maximum share of the overall soft-deadline budget a single reviewer slot may spend sleeping in retry backoff before the slot is marked budget-exhausted / skipped. |
...
| `SECURITY_AUDIT_ENABLED` | `true` | Default-on gate for the weekly/manual `security-audit.yml` workflow (source repo, and consumers via the `ai-security-audit.yml` wrapper); when enabled it posts surviving default-branch security findings to a stable tracker issue and opens one follow-up issue per new finding. Set `false` per repo to disable |
...
| `SECURITY_AUDIT_FP_EXCLUSIONS` | `scripts/security_audit_fp_exclusions.json` | Editable JSON false-positive catalog for the weekly/manual audit; resolves only to unchanged tracked content in the verified workflow-support checkout, not to the audited branch (existing audited-repo overrides fail closed). The script also rejects catalogs outside `SECURITY_AUDIT_SUPPORT_DIR`, path escapes, and matcher-less rules |
...
| `SECURITY_AUDIT_SKIP_IF_UNCHANGED` | `true` | Skip the audit entirely (log-only) when HEAD equals the last audited commit recorded on the tracker body marker |
...
- For the Phase O shared-prelude rollout, migrated mode prompts keep their legacy bodies in `prompts/mode-*.txt` while `prompts/_templates/*.txt` plus `scripts/assemble_prompt.sh` provide the flag-gated include expansion path. `scripts/render_prompt.sh` uses that path only when `PROMPT_PRELUDE_REFACTOR_ENABLED=true`; the acceptance default remains `false`. Both shell entrypoints always launch renderer Python through an allowlisted `env -i` environment with `python3 -I -B`; `PYTHON_ISOLATED_MODE` remains compatible but cannot disable isolation. Checkout-sensitive inline and module Python launches in the clarify, plan, implement, review/autofix, orchestrate, orchestrator clarification-response, and validate workflows use isolated mode; helper-backed parsers also receive only their declared file or status inputs.
...
OpenCode is installed by `.github/workflows/opencode-live-smoke.yml`, a `workflow_dispatch` rollout gate, and by `.github/workflows/review_autofix.yml`, which warms the models.dev cache before running the complete review/autofix model pipeline. The smoke generates role-scoped OpenRouter configuration for all six reviewer models plus the primary and fallback editor slots, runs each entry twice with the same stdin request, verifies non-empty ANSI-stripped output, intended provider/model stream evidence, and provider-reported reasoning usage, and fails if any entry fails. Production reviewer, summariser, interim-judge, behavioural-smoke synthesiser, editor, review-blocked judge/fix, consolidator, and conflict-resolver calls use isolated OpenCode configurations; OpenCode failures do not fall back to Codex.

Both paths default `OPENCODE_VERSION` to `1.18.23`. `CODEX_VERSION` remains independently pinned at `v0.114.0` for production paths outside the migrated review/autofix model pipeline.
...
2b. **Unknown-scope serialization (partition guard):** A sibling issue whose `files_touched` list is empty or missing is treated as *unknown scope* by `validate_wave_file_partition` and paired with every other sibling in its wave (overlap type `unknown_scope`, `files: []`), so `auto_serialize_file_overlaps` pushes it into a later wave exactly like a declared overlap would. An empty list gives the guard nothing to prove the siblings disjoint, so the only safe assumption is that they collide. This closes the bypass used by binance-blessings project #249, whose decomposition omitted `files_touched` on three phases that all edit `twap_router.py`, `README.md` and the same `db/contracts/*.yml` "because the authoritative parallel phases necessarily share" them — every sibling merge then conflicted the remaining PRs, the late resolver ran O(n²) times and deleted a tracked file on the way (see 2c and 4a). `prompts/mode-orchestrate.txt` now tells the decomposer never to leave the list empty and to declare `dependency_edges` when phases must share files.
...
9. **Review-blocked resolution:** When a PR exhausts its autofix iterations (`ai:review-blocked`), the poller invokes a credential-sanitized, read-only review-blocked judge with full bounded PR context. The judge emits only a validated advisory decision record. A bounded `fix` remains automated. Terminal `merge`, `merge_with_followup`, and `close_and_reissue` recommendations instead create a SHA- and digest-bound approval request; only an exact `/review-blocked-approve <request-id> <decision-digest>` PR comment from a GitHub User whose current repository role is `maintain` or `admin` can unlock the deterministic head/check/mergeability gates. Pending requests are reused without rerunning the model, stale-head and consumed requests are rejected, and replacement creation precedes source-PR closure.
...
10j. **Already-satisfied `BLOCKED:` verdicts route to success-no-op (not failure):** Not every `BLOCKED:` line is an obstacle. When the approved plan requires no edits because the requested state is already present on the branch, the verdict describes the one outcome the pipeline already has a success path for (10f). Before the 10i failure bail, the "Run Codex implementation" step classifies the extracted reason line with two named regexes defined above the retry loop: `BLOCKED_ALREADY_SATISFIED_REGEX` (positive — "requires no edits", "no changes are required", "nothing to do", "already implemented/present/satisfied/applied/enforced/…") and `BLOCKED_REAL_OBSTACLE_REGEX` (negative veto — `scope-lock-violation`, `cannot`, `unable`, `unavailable`, `inaccessible`, `denied`, `missing`, `fail*`, `error`, `conflict`, `ambiguous`, `unclear`, `insufficient`, `needs human`, `blocked by`, `out-of-scope`, `timeout`). The positive half reads the whole reason; the negative veto reads only the verdict's **first conservatively detected sentence**, held in `codex_blocked_first_sentence`. A boundary is `.`, `!` or `?` followed by whitespace and an uppercase letter or digit; `;` and `,` do not end the sentence, and a boundary after a dotted or common abbreviation falls back to checking the whole reason. Extraction errors or empty extraction results also fall back to the whole reason. The model states its blocker up front and appends diagnostics afterwards, so obstacle words in a trailing aside such as `Full suite has 6 unrelated failures.` no longer flip an already-satisfied verdict to red, while `requires no edits; pytest is unavailable` and `no changes are required, e.g. DigitalOcean token is unavailable` are still vetoed. Only a reason matching the positive half **and** whose retained first sentence does not match the negative half is reclassified: the step writes `${RUNTIME_DIR}/codex_success_noop.flag` (never `codex_blocked.flag`), sets `implement_succeeded=true`, emits the `Succeeded` substate, and breaks — so Guard 0 of "Handle no-op implementation" closes the issue with `ai:closed` and the ✅ "Already implemented" comment instead of posting implementation-failed diagnostics. An obstacle retained in the first-sentence input keeps the 10i failure behaviour; later diagnostic sentences are intentionally outside the veto. `prompts/mode-implement.txt`'s `completeness_contract` was the upstream cause and now states that `BLOCKED:` is reserved for real obstacles and that a no-op outcome must end with `No changes needed.` on its own line, which the 10f success-no-op regex already recognises. Motivating case: issue #3972 (run 33711184784) asked for an `author_association` gate that `main` already carried; Codex answered `BLOCKED: Approved plan requires no edits; all actor gates exist and the focused contract test passes (5/5).` and the run failed instead of closing the issue. First-sentence scoping and the `already contains` / `already holds` vocabulary came from tele-funtoken-msg-scoring issues #4180, #4184, #4189, #4191, #4192, #4193 and #4213 (runs 34162543282, 34162832548, 34163478446, 34163977712, 34163969134, 34164219124, 34166170045): one SES error opened thirteen auto-heal issues, each implement run answered `BLOCKED: Approved plan requires no repository changes; HEAD already contains the fix. Targeted tests pass (148). Full suite has 6 unrelated failures.` or similar, and `failures` in the trailing aside vetoed every one of them.
11. **Auto-recovery:** On failure, the judge can revert problematic PRs and create fix-up issues. Those fix-up issues include the standard orchestrator metadata block (`Tracking issue`, `Integration branch`, `Local ID`, `Managed by`) in the issue body. Recovery is attempted up to `MAX_RECOVERY_ATTEMPTS` (default 3) times. By default only a *repeat* of an already-seen failure fingerprint consumes an attempt (`RECOVERY_COUNT_DISTINCT_FINDINGS=false`); a brand-new finding is dispatched without charging the budget, so a project that fixes a different real defect each cycle keeps converging (`MAX_JUDGE_CYCLES` stays the absolute cap and `JUDGE_REPEAT_FINGERPRINT_MAX` still breaks identical-finding loops). When the budget is exhausted the poller still creates the judge's fix-up issues, tracks them in the current wave, cites them in the `## Project Failed` comment, marks the project `failed`, and notifies the operator via Telegram; `/judge_resume` then lets the poller merge and re-judge those issues with no further diagnosis by hand.
...
12a. **Integration branch delivery:** Orchestrator projects now create a per-project integration branch (`orchestrator/project-<tracking_issue>`). All orchestrator child issues include `Integration branch` metadata so implementation PRs target the integration branch instead of `main`. Branch resolution order is strict: child issue metadata footer first, then tracking issue metadata, and default-branch fallback only when no integration metadata exists. A human-authored issue may name its branch with `Target branch:` instead (``**Target branch:** `orchestrator/project-<n>` `` — trailing prose after the closing backtick is allowed, and the plain unbackticked form is a single token); the resolver (`scripts/resolve_integration_ref.sh`, `scripts/orchestrate_lib.py`) treats it as an alias of `Integration branch:` in child and tracking issue bodies, and the canonical line wins when both are present. If metadata exists but the branch is invalid/missing, the poller fails safe instead of silently falling back to default branch. The poller periodically syncs default branch changes into this branch via the merge API.
...
12c. **Integration self-healing:** If a periodic `main` → integration-branch sync returns HTTP 409 (real conflict), the poller routes recovery through `heal_integration_branch_conflict`: it (a) ensures/creates the final integration→default PR (eagerly, if it does not yet exist), (b) dispatches the review/autofix workflow through `_dispatch_review_for_conflicts` against that PR to run the existing Codex conflict resolver on a clean runner, and (c) records the attempt in new tracking-state fields (`integration_sync_status`, `integration_sync_last_error`, `integration_conflict_dispatch_count`, `integration_conflict_dispatch_ts`, `integration_conflict_unresolved_ticks`). Dispatches are throttled by `CONFLICT_DISPATCH_COOLDOWN_SECS` (default 900s). The retry budget is **branch-aware**: head refs matching `orchestrator/project-*` honour `INTEGRATION_SYNC_CONFLICT_MAX_RETRIES` (default `1` — single resolver shot, then judge), while non-orchestrator integration branches honour `INTEGRATION_CONFLICT_MAX_RETRIES` (default `3`). After the effective budget is exhausted the orchestrator escalates by invoking the judge with full PR context via `codex exec`. Only after both the automated resolver *and* the judge escalation fail is the project marked terminally `failed`. The same healing flow is triggered from `finalize_integration_merge_if_needed` whenever the final PR is observed with `mergeable=false`, so the project no longer halts on first conflict. Before dispatching the resolver, `sync_contract_list_union_merge` deterministically keeps both sides of eligible contract entrypoint-list appends and records `sync.last_sync_outcome = merged-deterministic` after the validated merge is pushed.

Additional hardening keeps retryable judge/model/API failures in the cumulative conflict budget, requires both sync boundaries to target `orchestrator/project-<tracking issue>`, and sends oversized, nested, non-scalar, or timed-out contract inputs to the existing resolver path.

12c-i. **Integration-sync intent fingerprints:** When a sub-issue PR merges into an orchestrator integration branch, the poller captures `must_contain` / `must_not_contain` regex fingerprints from the merged diff and persists them under `merged_issue_fingerprints[<issue_num>]` in the orchestrator state comment. Every new state write carries an `orchestrator_state_auth.v2` HMAC-SHA-256 envelope bound to the repository, tracking issue, canonical integration branch, immutable numeric producer ID, key ID, and monotonic generation. Signing uses the active dedicated `ORCHESTRATOR_STATE_AUTH_KEYRING` key; retained previous keys support rotation, while producer login and `GH_PAT` rotation do not affect V2 verification. The poller can accept V1/GH-token or unsigned exact-producer state only for migration and immediately republishes it as V2. The conflict resolver expands state-derived scope only from valid V2 state. Associated users and unrelated `*[bot]` accounts are not state authorities. The implementing PR is resolved by `scripts/orchestrate_poll_process.sh::_subissue_closing_pr_number`, which selects the most-recently-merged PR on the orchestrator's conventional `ai/issue-<n>` head branch and, only if that lookup is empty, falls back to the newest merged cross-referenced PR whose body carries a closing keyword (`Closes`/`Fixes`/`Resolves`, in `#N` or issue-URL form) targeting the issue. It deliberately does **not** pick the most-recent cross-reference: a `Refs #N` cross-reference from an unrelated infrastructure PR is not an implementation and must not be fingerprinted. When no implementing PR can be identified, capture is skipped rather than fingerprinting an arbitrary PR. Capture applies net-no-op and substring-overlap filters before persisting. The `review_autofix.yml` resolver step uses three affordances on top of these fingerprints when the PR head ref matches `orchestrator/project-*`:
- **Authenticated state anti-replay.** Each signed state increments a bounded monotonic `state_auth.generation`. The poller and privileged conflict resolver select the highest available generation before verification, preserving newest-write ordering for equal-generation retries while preventing an older authentic state body reposted by the designated producer account from rewinding project state. Unsigned and pre-generation state enter as generation zero and are republished by the poller on the next canonical write.
...
- **Trusted-runner index completion.** Conflict preparation preserves the initially unmerged paths separately from fingerprint-only working-set expansions. After the isolated resolver returns, trusted runner code stages only paths the resolver actually changed, treats each staging failure as fatal, and requires `git diff --name-only --diff-filter=U --` to be empty before either a no-change decision or the `[ai-merge-resolve]` commit. Marker-free modify/delete conflicts therefore cannot be silently accepted from working-tree checks alone. An attempted resolver run that does not set `CONFLICT_RESOLVED=true` is reported in `REVIEW_AUTOFIX_RUN_SUMMARY_V1` as `conflict_resolver_failed`, rather than the misleading `clean_review_no_commit` fallback.
...
Capture is **going-forward only**: sub-issues merged before fingerprinting was enabled have no entries in `merged_issue_fingerprints` and are silently skipped by the verifier (fail-open). Capture and verification are tunable via `FINGERPRINT_PER_FILE_CAP` / `FINGERPRINT_MIN_PATTERN_CHARS`. The resolver safety scripts (`orchestrate_state_v2.py`, `verify_integration_fingerprints.py`, `review_conflict_prepare.sh`, `review_conflict_resolve.sh`) are bootstrapped via the historically named `MAIN_PRIMARY_BOOTSTRAP_SCRIPTS` list from the gate-verified reusable-workflow support commit (`review_support_sha`). No PR, `main`, or `stable` executable fallback is allowed; missing optional support fails open only within that pinned source, while missing required support fails closed. Operationally: a verification rejection always surfaces in the workflow run log with `::error::Aborting [ai-merge-resolve] commit: integration fingerprint verification rejected the resolver output.`, the resolver step exits 1, the conflict resolver workflow lands without a commit, and the orchestrator's next poll tick takes the judge path because `unresolved_ticks` was already incremented by the dispatch.

- **Resolver escape valve, authenticated comment state, and tier ladder.** `scripts/review_conflict_resolve.sh` prepares retry-state fields after a fingerprint failure, then the trusted actuator signs and persists one `AUTOFIX_RESOLVER_RETRY_STATE_V2` issue comment bound to the repository, tracking issue, integration branch, final PR, producer ID, and head SHA. It records the normalized failure signature, `consecutive_failure_count`, `verification_tier`, `escalation_threshold`, and `escalated` / `escalated_at`, emits `FINGERPRINT_TIER_DOWNGRADED_V1` when the tier moves to `ratio`, `count_only`, or `warn_only`, and applies `ai:resolver-escalated` to the **final PR issue** only after the same head and normalized failure signature exhaust the warn-only tier. The poller accepts only a valid producer-authenticated V2 envelope and suppresses further first-line redispatch until the head SHA changes; legacy V1 PR-body blocks remain recognizable for diagnostics but carry no authority.
- **Adaptive quarantine + scheduled drift audit.** `scripts/verify_integration_fingerprints.py` tracks unchanged pre-existing drift in ai-memory; after `FINGERPRINT_QUARANTINE_RUNS_M` consecutive unchanged observations it skips that `fp_key` and emits `FINGERPRINT_QUARANTINED_V1`. `.github/workflows/drift-audit.yml` runs daily at `03:00 UTC`, stays inert unless `DRIFT_AUDIT_ENABLED=true`, and uses `scripts/drift_audit.sh` to scan recent review/autofix logs for `PRE_EXISTING_FINGERPRINT_DRIFT_V1` / `FINGERPRINT_QUARANTINED_V1` clusters, then create or refresh tracker issues for persistent drift. Every enabled run also posts a Telegram run summary linking to the workflow run and writes a GitHub Actions job-summary report. Completed runs whose conclusion is `cancelled` or `skipped` (e.g. review runs superseded by a newer push via the concurrency group) routinely upload no logs; when their log fetch fails the audit records them separately (`unscannable` in the `DRIFT_AUDIT_COVERAGE` log line, "Cancelled/skipped runs without logs" in the job summary) instead of counting them as missing coverage, so the Telegram summary only escalates to WARNING for genuinely missing logs. Cancelled/skipped runs that did upload logs are still scanned normally.
- **Periodic default-branch security audit.** `.github/workflows/security-audit.yml` runs weekly at `08:00 UTC` and on `workflow_dispatch`, stays inert unless `SECURITY_AUDIT_ENABLED=true`, and runs `scripts/security_audit.sh` with `prompts/mode-security-audit.txt` from a verified support commit checked out at the workspace root (protected `main` for this repository; the `stable` tag or an immutable pin for consumers), installing Codex from that commit's local action. The audited branch head is checked out separately under `audit-data/` and treated only as untrusted data; audit-owned Python, including prompt rendering and model-broker setup, runs in isolated mode with an allowlisted environment, and final findings packaging runs from the runner-owned audit runtime directory. False-positive exclusions resolve only inside canonical immutable support, and every rule needs an effective exact or substring matcher. Surviving findings are appended under dated sections on the stable `AI Security Audit Tracker` issue (`ai:security-audit`, marker `<!-- ai:security-audit-tracker:v1 -->`), and one `ai:security` follow-up issue is opened for every non-excluded finding that meets the confidence gate, with no per-run or weekly cap. Findings are deduplicated against the `<!-- ai:security-finding:<id> -->` marker of every existing `ai:security` issue, open or closed, read with one paginated REST listing (`gh api --paginate repos/<repo>/issues?labels=ai:security&state=all`, one call per 100 issues). `SECURITY_AUDIT_CRON` is documentation-only because GitHub Actions schedule cron is literal YAML, not repo-variable-driven.

#### Security audit engine modes

`scripts/security_audit.sh` defaults to `SECURITY_AUDIT_OUTPUT_MODE=issues`, preserving the periodic audit workflow. The scheduled orchestrator poller invokes `findings-json` by default for the mandatory security pass; setting `ENABLE_SECURITY_PASS=false` leaves this orchestrator path inert. Internal callers may pass one positional argument containing the path to a readable project-specification file. That content is appended to the model prompt between `UNTRUSTED PROJECT SPECIFICATION` delimiters and is valid only in `findings-json` mode.

Two optional inputs support the orchestrator's delta re-audits. `SECURITY_AUDIT_DIFF_SINCE` (requires the explicit `SECURITY_AUDIT_DIFF_BASE`/`SECURITY_AUDIT_DIFF_HEAD` range) keeps only range files that also changed in `SINCE..HEAD`; it fails closed when the commit does not resolve or is not an ancestor of the head. `SECURITY_AUDIT_PRIOR_FINDINGS` (findings-json mode only) names a JSON array of earlier findings: files they cite stay in scope, and the list is appended to the prompt with instructions to re-emit persisting findings under the same `finding_id` and to report every remaining instance of the same defect class. It fails closed on anything that is not an array of objects citing repository-relative files. The run log reports `security-audit: prior-findings=N (M cited files kept in scope)`. `SECURITY_AUDIT_FIX_CYCLE_DIFFS` (findings-json mode only) names a JSON array of fix-cycle diff entries `{cycle, since_sha, head_sha, files}`: their files stay in scope and the added/modified hunks of `since_sha..head_sha` for each file are appended to the prompt between `=== BEGIN/END UNTRUSTED FIX-CYCLE CODE ===` fences as newly introduced code, followed by rules to audit it as fresh attack surface for new defect classes. Unlike the prior and waived findings it fails open: a missing or malformed file, a broken entry, an unresolvable or non-ancestor SHA, or a git failure logs a `::warning::` and the audit continues without that input. The run log reports `security-audit: fix-cycle-diffs=N entries (M files kept in scope, hunks for K files: L lines, B bytes, O files over the cap)`.

| Input | Default | Contract |
|---|---|---|
| `SECURITY_AUDIT_OUTPUT_MODE` | `issues` | Accepts `issues` or `findings-json`. `issues` preserves tracker discovery and updates, labels, uncapped follow-up creation, last-audited-SHA bookkeeping, and the existing weekly behavior. |
| `SECURITY_AUDIT_FINDINGS_OUT` | _(empty)_ | Required in `findings-json` mode. Its parent directory must exist and the destination must be writable; a successful run atomically replaces this file. |
...
| `SECURITY_AUDIT_WAIVED_FINDINGS` | _(empty)_ | Optional, findings-JSON-only path to a JSON array of accepted findings. Only rows with a valid `security_defect_context.v1` `defect_fingerprint` enter the prompt or suppress output, and suppression requires one unique exact fingerprint match on each side. Legacy rows without fingerprints are ignored. Using it in `issues` mode or passing a malformed file fails before model execution. |
| `SECURITY_AUDIT_WAIVER_LINE_WINDOW` | `40` | Compatibility-only non-negative integer. It remains accepted but no longer authorizes suppression. |
...
The findings output has schema version `security_audit_findings.v1`, a `findings` array containing every surviving severity, and a `counts` object with `kept`, `suppressed_excluded`, `suppressed_invalid`, `suppressed_low_confidence`, `suppressed_out_of_scope`, and `suppressed_waived`. Findings-JSON mode does not initialize GitHub label helpers, read or mutate tracker/follow-up issues, update last-SHA markers, or send notifications; it exits immediately after atomic output publication. Invalid input, invalid refs, model/filter errors, or publication errors return non-zero without replacing a pre-existing findings file.
...
12f. **Promote cycle callbacks (poller-owned):** Tracking issues labeled `ai:comprehensive-test-pending` belong to the daily promote cycle started by `.github/workflows/promote-main-to-stable.yml`. `scripts/apply_analysis_on_main.sh` signs a bounded, initially unbound `comprehensive_cycle_marker.v1` envelope with `ORCHESTRATOR_STATE_AUTH_KEYRING`, binding the source document, role, originating dispatcher run, smoke run and actor IDs, expected smoke inputs/title/workflow/event/conclusion/head, promotion SHA, and proving merge SHA. After `orchestrate.yml` creates the tracking issue, it verifies that envelope from immutable support, requires its signed smoke actor to match the authenticated workflow dispatcher, binds the new tracking-issue number into the HMAC, and only then reposts it with `github.token`; a copied envelope cannot authorize another issue. All readers require that exact issue binding, immutable GitHub Actions bot user ID `41898282`, and a valid signature. Before promotion the poller re-fetches the signed smoke run and verifies the API-reported workflow path, event, display title/input correlation, actor, conclusion, and head SHA. Unsigned, tampered, wrong-producer, stale, cross-issue, or API-mismatched evidence cannot dispatch or promote; the poller automatically removes `ai:comprehensive-test-pending` so the next scheduled cycle retries with fresh evidence instead of waiting for manual label removal. Operational marker lookup or parsing failures instead keep the label and defer callback processing; pre-dispatch verifying merges use the existing bounded hold, proving and legacy merges proceed, and an already-dispatched promotion continues from persisted state without another marker read. The proving/verifying dispatch, quiescence gate, pinned promotion, and bounded release hold otherwise retain their existing behavior.
13. **Stall detection and self-healing:** Every poll cycle, the poller tracks how long each issue has been in its current pipeline phase. Stall thresholds are **adaptive per phase**: lightweight phases (clarification, planning, approval, merge) default to 60 minutes, while heavy phases (implementation, review/autofix) default to 120 minutes. Each threshold is independently configurable via `STALL_THRESHOLD_<PHASE>_MINUTES` env vars, with `STALL_THRESHOLD_MINUTES` as the global fallback. Before stall checks, the poller reconciles managed-issue labels and state truth (labels + issue open/closed + linked PR merge state), repairs missing/conflicting phase labels, and persists reconciled statuses every cycle. Closed/terminal issues are hard-guarded out of retrigger paths; stale `no_labels` on closed issues is healed (label/state repair) instead of retriggered. Issues carrying `ai:needs-human` or a human-gated latch label (`ai:destructive-blocked`, `ai:scope-blocked` — see 10d) are excluded from stall detection entirely: `implement.yml` refuses to redispatch them until a human removes the label, so any recovery action would be a no-op loop; the poller logs `STALL_SKIP … reason=human_gated_latch` for them and takes no action. One latch reason is released by the poller itself: the `ai:needs-human` that `implement.yml`'s staged-support rejection handler sets on `IMPLEMENT_STAGED_SUPPORT_REBASE_CONFLICT` (this repository only). That handler also removes `ai:implementing`, so the issue sits with no phase label and even a human `/approved` is refused with `reason=wrong_phase` (#4113, project #3965 fix cycle 7, run 35349975875) after the cause itself was fixed by `scripts/implement_staged_support_workspace.sh`. The `release_staged_support_needs_human_latches` sweep (after standalone stall recovery, gated by `STAGED_SUPPORT_LATCH_AUTO_RELEASE_ENABLED`, default `true`) lists open `ai:needs-human` issues once per tick and, for each whose latest latch comment carries `<!-- ai:needs-human-latch reason=staged_support_rebase_conflict -->` (or the pre-marker `🚨 **Staged-support restore failed; implementation halted.**` header) and which carries no `ai:destructive-blocked` / `ai:scope-blocked`, removes `ai:needs-human`, adds `ai:awaiting-approval`, and posts `/approved` with a `<!-- ai:needs-human-auto-release reason=staged_support_rebase_conflict engine=<sha> -->` marker, at most once per issue per engine commit and never while a release marker already follows the latest latch comment, so a latch that recurs on the same engine stays for a human. The latest `ai:needs-human` label event must precede that staged-support comment and have the same actor; if the latch was cleared and another human gate was set later, the stale marker cannot release it. Every other `ai:needs-human` reason is untouched. The sweep is skipped when the engine cannot be resolved (`STAGED_SUPPORT_LATCH_RELEASE_SKIPPED reason=engine_unresolved`) or lacks the fix (`reason=engine_lacks_editor_workspace_restore`); per issue it logs `STAGED_SUPPORT_LATCH_RELEASED` or `STAGED_SUPPORT_LATCH_SKIP ... reason=other_human_gated_latch_present|comments_unavailable|label_events_unavailable|no_staged_support_latch_comment|current_latch_not_staged_support|already_released`. The sweep runs only in `shubhodeep1/coding-workflows`, accepts latch and release-marker comments only from an `OWNER`, `MEMBER`, `COLLABORATOR`, or installed `[bot]`, and costs one paginated REST issues read per source-repository tick, plus one paginated comments read and one paginated label-events read per candidate issue, then one label edit and one comment write per released issue; consumer repositories exit before any API call. Early-phase recovery actions (`retrigger_pipeline`, `auto_respond_clarify`, `retrigger_plan`, `auto_approve`, `retrigger_implement`) are additionally guarded against issues that already have an open linked PR. The guard is **state-aware**: (a) if the PR has merge conflicts (`mergeable: false`), the guard dispatches the conflict resolver workflow instead of skipping entirely; (b) if the PR has `CHANGES_REQUESTED` reviews, the guard dispatches the review/autofix workflow; (c) if the PR is clean and progressing, the guard skips the recovery action as before. This prevents re-triggering earlier pipeline phases on issues whose implementation PR already exists, while still routing stuck PRs to the appropriate corrective action. The guard uses the batched GraphQL prefetch cache first (0 extra API calls on hit), with a per-issue REST fallback that reuses the PR JSON already fetched for the merged-PR sub-guard. The review-state check adds at most 1 REST call per stalled issue with an open PR. When an issue exceeds its phase threshold, the poller first selects a declarative action from `STALL_RECOVERY_ACTIONS` by `stall_recovery_count` (per issue): `no_labels` → `retrigger_pipeline`, `ai:clarification` → `auto_respond_clarify`, `ai:planning` → `retrigger_plan`, `ai:awaiting-approval` → `auto_approve`, `ai:implementing` → `retrigger_implement`, `ai:done` → `retrigger_review`, `ai:ready-to-merge` → `attempt_merge`, with each phase ladder ending in `escalate_human`. `retrigger_implement` first moves the issue from `ai:implementing` back to `ai:awaiting-approval` (shared helper `_reset_implementing_to_awaiting_approval_for_retrigger`, one `gh issue edit` call) and only then posts `/approved`: `implement.yml`'s precheck skips with `reason=already_implementing` while `ai:implementing` is present, so without the swap the re-triggered run ends in seconds with conclusion `success`, no PR and no diagnostics. Both the orchestrator-managed arm (`execute_stall_recovery_action`) and the standalone arm (`run_standalone_stall_recovery`) perform the swap; a failed swap logs a `::warning::` and the `/approved` comment is still posted. Backward-compatible default behavior keeps human terminalization disabled: with `ENABLE_STALL_HUMAN_TERMINALIZATION=false` (default), any terminal `escalate_human` result (declarative or judged) is downgraded to the nearest prior non-human phase action. Set `ENABLE_STALL_HUMAN_TERMINALIZATION=true` to allow terminal human escalation. As a loop backstop, a **judged** `escalate_human` that the stall judge returns on `MAX_JUDGE_REPLAY` consecutive evaluations for the same issue is honored even when terminalization is disabled: the input-hash judge-replay cap can be defeated when the chosen recovery action churns the PR head SHA (e.g. `ai:done` → `retrigger_review` pushes an empty commit each tick, changing the judge cache key and pinning `replay_count` at 0), so a separate counter keyed on the judge's *decision* (reset whenever the judge picks any non-escalate action) surfaces the issue to `ai:needs-human` instead of retriggering forever. Honoring it only adds the `ai:needs-human` label (highest-priority phase label — pauses further stall recovery) and a CRITICAL notification; it never closes or discards the PR. If `ENABLE_STALL_JUDGE=true` and `stall_recovery_count >= STALL_JUDGE_TRIGGER_COUNT` (while still below `MAX_STALL_RECOVERIES_PER_ISSUE`), the recovery action switches to `run_stall_judge` for diagnostics-driven action selection. The stall judge may choose targeted actions including `resolve_merge_conflict`; that path attempts GitHub `update-branch` for the target PR and then dispatches `_dispatch_review_for_conflicts`. If stall-judge execution fails, output parsing fails, or the returned action is unsupported, the poller fail-opens to the same declarative ladder action for that phase/recovery count. If `stall_recovery_count` exceeds the phase ladder length, the final declarative action is repeated until the max budget is hit. After `MAX_STALL_RECOVERIES_PER_ISSUE` (default 5) attempts, the issue is skipped (`ai:closed`) so the wave can advance; the judge evaluates the gap at wave completion and decides whether to reissue, accept, or fail. When `ENABLE_STALL_JUDGE=false`, or when `STALL_JUDGE_TRIGGER_COUNT` is effectively unreachable within the configured recovery budget, recovery remains on the declarative `STALL_RECOVERY_ACTIONS` ladder without judge escalation. All stall recoveries trigger Telegram notifications. Standalone AI issues (not linked to any active orchestrator tracking state) also use the same stall recovery engine and the same human-terminalization gate when `ENABLE_STANDALONE_STALL_RECOVERY=true`; standalone recovery state is persisted per issue in a hidden marker comment. Additionally, all orchestrator-created issues (Wave 1, deferred waves, reissues, and judge fix-ups) now receive the `ai:clarification` label at creation time, ensuring they enter the pipeline immediately without relying solely on the `issues.opened` event trigger.
13a. **Missing state recovery:** If the orchestrate.yml workflow creates issues but fails before posting the initial state comment (e.g. due to a transient API error or timeout), the poller automatically reconstructs the state. It parses the tracking issue body to extract the wave structure and dependency graph, searches for child issues that reference the tracking issue, and builds a new state object. The reconstructed state is posted as a comment so subsequent poll cycles operate normally. State authority requires the exact numeric producer ID represented by the active workflow credential plus a valid repository/tracking/branch-bound `state_auth` HMAC; broad bot suffixes and repository associations grant no authority. For migration only, the poller accepts unsigned legacy state from that exact producer and immediately republishes signed V2. Invalid signed state or unavailable producer authentication pauses state processing rather than reconstructing over uncertain data, while markers from other identities are ignored and cannot veto reconstruction from trusted project inputs. This prevents projects from being permanently stuck when the initial orchestration run fails mid-execution. Reconstruction is guarded by two safety checks so a transient read failure can never rewind a live project: (a) it runs **only when the tracking-issue comments fetch actually succeeded** — if the paginated comments API failed (e.g. `HTTP 503` on a long-lived tracking issue with hundreds of comments), the poller treats state as *temporarily unreadable* rather than *absent*, skips the issue for that cycle, and retries on the next poll (matching the fail-open handling used elsewhere for unavailable comments); and (b) `rebuild_tracking_state` **refuses to rebuild** (raising `ReconstructionUnsafeError`, which the poller logs as `State reconstruction failed … skipping`) when the tracking body marks a sub-issue complete (`[x]`) that the child-issue search cannot map, since a from-scratch rebuild would reset `current_wave` to 1 and re-create already-merged work as duplicate issues. The genuine crash-before-first-state case is unaffected because none of its sub-issues are complete yet. As a third, independent layer, the **deferred-creation path itself will not mint a duplicate**: before creating a current-wave issue whose in-state `github_issue` is null and which is absent from issue_number_map, the poller runs one live child-issue search (`"Tracking issue: #N" in:body`) and, when an issue already carrying that `Local ID` exists on GitHub (open **or** closed), adopts it into the wave/state instead of creating a second one. This covers the case where a stale but *readable* state snapshot presents an already-created issue as uncreated without ever triggering reconstruction. If that existence lookup itself fails for the cycle, creation is skipped and retried on the next poll (fail-closed) rather than risking a duplicate. A pure-adoption cycle heals the state map silently and does **not** post a "Deferred Issue Creation … Created them now" comment.
...
**Staged-support sweep revalidation contract:** This supersedes the earlier API-cost and skip-reason sentence in item 13. Per candidate issue, the sweep uses one paginated comments read, two paginated label-events reads, and one paginated labels read before any transition; `latch_revalidation_unavailable` and `latch_changed_during_release` explain live-state revalidation skips.
...
15. **Completion:** When validation is disabled and `ENABLE_SECURITY_PASS=true` (the default), completion requires a successful SHA-bound security pass. Setting `ENABLE_SECURITY_PASS=false` restores the legacy judge-driven immediate route.

### Merge train and pre-review conflict resolution

Two mechanisms in `review_autofix.yml` stop a burst of same-file PRs from
turning into a storm of `[ai-merge-resolve]` commits (2026-09-07: 4 resolver
rounds across coding-workflows #4031 and binance-blessings #255, then 14
across seven tele-funtoken-msg-scoring PRs in two hours, one Telegram ping
each).

**Pre-review conflict resolution** (`PRE_REVIEW_CONFLICT_RESOLVE_ENABLED`,
default `true`). The `Pre-review deterministic merge-topology gate` already
replays `git merge --no-commit origin/<base>` before the reviewers run. When
that replay leaves unmerged paths, the gate now sets `MERGE_CONFLICT=true` and
`AUTOFIX_PRE_REVIEW_RESOLVE=true`; every reviewer/editor-phase step (from
`Run reviewer models` through `Validate editor no-op disposition`) carries
`env.AUTOFIX_PRE_REVIEW_RESOLVE != 'true'` and is skipped, and the run
proceeds straight to the existing `Detect merge conflicts` → `Prepare
merge-conflict resolver prompt` → `Run Codex resolver` → `Push all pending
commits` → `Re-trigger review via workflow_dispatch` tail. The re-dispatched
run reviews the merged head. Nothing else changes: the same resolver, the same
`CONFLICT_RESOLVED` push/notify/re-trigger gating, the same
...
instead of N². The orchestrator poller's managed and standalone conflict loops
and both `ai:done` stall-recovery paths skip `ai:merge-queued` PRs without
...
- Review support is checked out once at the gate-verified reusable-workflow support commit (`review_support_sha`; source-repository PRs use protected `main`'s commit instead of the PR head). PR-declared dependencies are installed, and the OpenCode writer runs, only inside a disposable review sandbox container (`scripts/review_untrusted_sandbox.sh`) with no checkout Git metadata, runner command files, repository or Telegram credentials, or state-auth keys; model traffic goes through a fixed-path relay that keeps the provider key on the host, and only validated edits return to the checkout. Dependency-sensitive local validation may therefore be limited; CI/check-run evidence remains authoritative.
- Model-facing issue analysis, orchestrator polling, implementation and repair, validation, check-failure triage, security audits, source and consumer workflow retros, review consolidation, review editing, review-blocked judging/fixing, workflow-log analysis, and merge-conflict resolution use `scripts/model_provider_broker.py`, a bounded loopback proxy that retains the real OpenRouter credential. Model children receive only an ephemeral per-run broker token in a clean environment; same-UID launches also run inside a private PID namespace with a fresh `/proc`, so agents cannot inspect the secret-bearing workflow or broker environments. Implementation and syntax-repair writers additionally run as a dedicated UID with no runner command-file, immutable-support, or Git-metadata access; validation and self-heal model calls use the same separate-UID boundary read-only. Support bundles enforce their complete shell/Python dependency closure and carry model instruction files and meta-prompts from the validated workflow SHA; checkout documentation is fenced as untrusted data rather than treated as instructions. Missing isolation prerequisites fail closed before model launch. The broker authorizes only the phase's configured model IDs, limits active clients to 8 with a 30-second read timeout, normalizes endpoint-specific output limits, and injects server-controlled provider price ceilings. `MODEL_PROVIDER_BROKER_MAX_REQUESTS` defaults to `100` (valid range `1..100`), `MODEL_PROVIDER_BROKER_MAX_OUTPUT_TOKENS` defaults to `16384`, and `MODEL_PROVIDER_BROKER_MAX_TOTAL_OUTPUT_TOKENS` defaults to `1638400` (`100 x 16384`, so the total never bites below what the request cap already permits); either exhausted budget returns HTTP 429 and the phase fails closed. Input spend is budgeted the same way (#4090): `MODEL_PROVIDER_BROKER_MAX_INPUT_TOKENS` (default `16777216`, one token per byte of the 16 MiB body cap) bounds one request's conservative input reservation, `MODEL_PROVIDER_BROKER_MAX_TOTAL_INPUT_TOKENS` (default `1677721600`, `100 x 16777216`) bounds the session, and `MODEL_PROVIDER_BROKER_MAX_TOTAL_COST_USD` bounds the worst-case USD cost of all admitted requests at the price ceilings (derived from the token budgets and the request-body-bound maximum image count when unset). Input reservations and cost are settled to the provider-reported `usage.prompt_tokens` / `usage.input_tokens` alongside the output true-up, so the defaults change no phase's behaviour and operators tighten the vars to make the control binding. Price defaults are `MODEL_PROVIDER_BROKER_MAX_PROMPT_PRICE=10` USD per million input tokens, `MODEL_PROVIDER_BROKER_MAX_COMPLETION_PRICE=30` USD per million output tokens, `MODEL_PROVIDER_BROKER_MAX_REQUEST_PRICE=0.10` USD per request, and `MODEL_PROVIDER_BROKER_MAX_IMAGE_PRICE=1` USD per image. Request-supplied prices can only lower these ceilings; malformed, negative, or non-finite configured values prevent broker startup. The total token budget is charged against provider-reported usage: each request reserves its per-request ceiling only while in flight and is settled to the `usage` output-token count when its response completes (a usage-less or cut-off response keeps the full reservation; an observed upstream error response or pre-forward failure settles to zero, while an ambiguous transport failure after forwarding starts keeps the full reservation), so a multi-turn agent session is bounded by tokens actually generated rather than by `total / per-request` turns. Broker termination is attempted before ACL restoration, so a restore failure cannot leave the credential-bearing process alive. There is no direct-provider fallback when broker startup fails.
- Clarify and the sandboxed review editor reach the provider through `scripts/clarify_openrouter_broker.py` instead: it keeps the provider key on the host and applies the same `MODEL_PROVIDER_BROKER_MAX_*` request, token, and cost budgets and price ceilings, per clarify or editor attempt. A missing budget module or an invalid budget value stops the attempt before any model call.
- Integration-ref resolution defaults to the repository's default branch only when no issue/tracking metadata declares an integration branch. Failure to stage the SHA-pinned resolver, a missing resolver helper, an API error, or a declared-but-missing branch fails the phase before checkout rather than validating or implementing the wrong ref.
- Resolver retry authority is stored only in producer-authenticated `AUTOFIX_RESOLVER_RETRY_STATE_V2` comments bound to repository, project branch, PR, and current head SHA. Legacy V1 PR-body blocks are ignored as authority. The trusted actuator stores the verified marker comment ID as an untrusted commit status on the signed head SHA; the poller searches up to 10 combined-status pages for that context, fetches the referenced comment directly, and still verifies its producer, context, and signature before use. An incomplete status lookup defers mutation rather than treating a missing first-page result as absence. Legacy PR-body locator hints remain readable but are no longer written. A non-escalated locator cannot override the trusted `ai:resolver-escalated` label: the poller falls back to highest-generation selection before deciding whether to dispatch. If the locator is absent or invalid, the poller selects the highest verified generation from a complete fallback scan bounded to 10 pages and 32 MiB and refreshes the status locator; if a short page cannot prove completeness within either bound, mutation is deferred instead of using partial history. The actuator reuses the review pipeline's collected comment snapshot rather than issuing a second unbounded fetch. Locator persistence no longer replaces the PR body, so concurrent description edits are outside this bookkeeping path. GitHub labels, comments, and force-tick dispatches run afterward in `scripts/review_conflict_actuate.sh`, which revalidates the live head before signing or acting. Lookup order is fixed (#4091): the PR-body `AUTOFIX_RESOLVER_RETRY_COMMENT_ID_V1` hint is tried first as untrusted metadata, and when it is absent, stale, or fails producer/signature verification the poller consults the `ai/resolver-retry-state-locator` commit status on the head SHA before falling back to bounded comment discovery, so a forged body locator can no longer skip the trusted lookup and force every tick to defer inside a comment flood.
- Workflow-log model output is written under `RUNNER_TEMP` and must pass `scripts/workflow_log_output_contract.py` before a trusted step publishes or appends it. The validator enforces UTF-8/control-byte limits, mode-specific ordered headings, UTC date headings, byte bounds, non-symlink destinations, and atomic publication.
- On exhaustion the per-PR review-blocked judge runs and decides `merge` (auto-merge into the integration branch), `fix` (the trusted actuator posts the bounded guidance and pushes a no-tree-change `[judge-fix]` commit only when the live head still matches the judged SHA, then redispatches review; this resets the autofix counter and lets the editor run, capped at `MAX_REVIEW_BLOCKED_RETRIES`, default `2`), `merge_with_followup` (auto-merge AND open a follow-up issue tracking a deferred-but-non-blocking gap — preferred over `close_and_reissue` at IS_FINAL when the PR is shippable; follow-up issue inherits `ai:orchestrator-managed` from the parent and carries the parent's `Tracking issue:` / `Integration branch:` lineage lines — taken from the parent issue metadata, or from an orchestrator integration PR base when metadata is absent), or `close_and_reissue` (close the sub-issue PR and create a refined issue). Terminal actions additionally require an exact approval from a current `maintain` or `admin` repository role. Existing successors are reused only when their immutable creator ID, required labels, action/type, approved head, and title/body digests match a producer-authenticated `REVIEW_BLOCKED_SUCCESSOR_V1` intent. Legacy refusal markers remain visible, but only a matching keyring-signed `REVIEW_BLOCKED_DECISION_REFUSED_V2` envelope suppresses another judge attempt.
- The PR merges into the integration branch only when the existing orchestrator merge gate clears: `mergeable=true` AND `_pr_checks_completed` AND `ai:ready-to-merge` label set. The current-wave merge path then re-fetches the head and binds its automatic and direct attempts with `--match-head-commit` so a concurrent push remains queued for another review cycle.
...
- At the end of each `review_autofix.yml` run, look for `REVIEW_AUTOFIX_RUN_SUMMARY_V1` in the workflow log or the final `Review Autofix Run Summary` section in the GitHub job summary.
- A run that fails in "Preflight: Verify required files before reviewer invocation" with `REVIEW_EDITOR_PREFLIGHT result=fail` stopped before any reviewer or editor call: the staged editor script would have failed a precondition (`REVIEW_EDITOR_PREFLIGHT_ENABLED`). The failing `check=` line names it, and the heal report carries `failure_reason=editor_preflight_failed`.
- `completed_phases` / `skipped_phases` show which terminal workflow phases ran, `slot_results` carries per-slot `attempt_count` / `status` / `failure_class`, and reviewer slots now also report `stall_kill_count` / `stall_recovery_next_action` / `stall_recovered` (`stall_kill_count` counts `codex_stall_killed` events for that slot, `stall_recovery_next_action` captures the last stall-related `next_action`, and `stall_recovered` is true when a killed slot later succeeds).
- Reviewer slot summaries also include retry/backoff/cache telemetry: `retryable_failure_count`, `retryable_failure_classes[]`, `backoff_sleep_secs_total`, `slot_retry_budget_exhausted`, `fallback_model_used`, `cache_status`, `cache_reuse_attempted`, and `cache_read_input_tokens_total`. `stall_recovery` still aggregates run-level `killed_attempts` / `advanced_slots` / `recovered_slots` / `partial_finalize_requested`, `budget_*` reports elapsed vs total vs remaining seconds, `finalize_reason` explains why the run stopped, `partial_finalize` / `partial_finalize_reason` / `partial_finalize_phase` / `partial_finalize_validation_tail_can_complete` / `partial_finalize_edits_withheld_for_safety` / `partial_finalize_withheld_reason` / `partial_comment_posted` / `incomplete_phases` describe any partial-completion path, `edits_pushed` shows whether the run successfully pushed its pending commit/merge, and `resume_round` is the current same-head resume attempt count for the `(PR, head SHA)` key.
- The summary also reports `resume_restored`, `resume_head_sha`, `resume_round_limit`, `resume_state`, `resume_should_continue`, `resume_source_reason`, `resume_source_phase`, `resume_comment_posted`, `resume_completed_scope[]`, and `resume_incomplete_scope[]` so later runs and operators can tell whether work was restored from cache, whether the same-head cycle is terminal, and which phases remain.
- Reviewer retry/recovery telemetry is greppable in raw logs as `codex_stall_killed`, `REVIEWER_ADVANCE: slot=... model=... reason=... next_action=...` (with optional `next_attempt=` / `next_model=` when a new attempt is scheduled), `REVIEWER_BACKOFF: ...`, `REVIEWER_CACHE: ...`, `REVIEWER_SLOT_STATE: ...`, and the normalized `INFO: openrouter usage ... cache_read_input_tokens=...` lines; the structured summary is the durable end-of-run record for those events.
- Partial-push safety contract: a partial finalize may publish edits only when the run still has enough hard-timeout headroom to finish the existing validation/publish tail. If that headroom is missing, `review_commit_changes.sh` resets the local editor diff, pushes nothing, and the partial comment/summary report `validation_tail_can_complete=false`, `edits_withheld_for_safety=true`, and `withheld_reason=insufficient_budget_for_validation_tail`.
- When `partial_finalize=true`, the normal editor-summary comment is skipped and the workflow posts a dedicated `<!-- REVIEW_AUTOFIX_PARTIAL_V1 -->` PR comment with machine-readable `reason`, `phase`, `completed_scope`, `incomplete_scope`, `validated_edits_committed`, `edits_pushed`, `validation_tail_can_complete`, `edits_withheld_for_safety`, `withheld_reason`, `head_sha`, `resume_round`, `resume_round_limit`, `resume_state`, and `resume_should_continue` fields.
- The same partial path persists `.ai/review_runtime/pr-<PR>/round-<N>/partial_finalize.json` alongside the usual ledger/runtime cache so a later same-head resume/no-progress consumer can continue from structured state instead of parsing prose. The JSON currently stores `schema_version`, `partial_finalize`, `reason`, `phase`, `head_sha`, `completed_scope[]`, `incomplete_scope[]`, `validated_edits_committed`, `edits_pushed`, `validation_tail_can_complete`, `edits_withheld_for_safety`, `withheld_reason`, `resume_round`, `resume_round_limit`, `resume_state`, `resume_should_continue`, `progress_fingerprint`, and `comment_posted`.
- Genuine finalize-tail faults (bad config/auth, push rejection, malformed inputs, or other hard failures after the partial request) still leave the run red instead of being reclassified as a successful partial finalize.
- Same-head `no_progress` and `round_budget_exhausted` outcomes stay green/neutral: they stop the partial-resume loop, suppress any synthetic continuation dispatch, and leave the remaining incomplete scope visible in both the PR comment and the terminal summary.
- Later resume/stall/finalize phases may populate additional values inside this record, but they extend the existing keys rather than renaming or dropping them.
...
Dynamic operator-data labels are the current exception: when `SCOPE_LOCK_LABEL_ENABLED=true`, the implement workflow recognizes per-issue `ai:scope:<glob>` labels. Because the suffix is runtime glob data, those labels are intentionally not enumerated in `label_contract.v1.json` and are not subject to contract-driven label repair.
...
**Linked-PR closure (`close_linked_pr`).** Every stall-recovery `close_and_reissue` invocation (main and standalone) enumerates every open PR linked to the stalled issue via three independent lookups, then closes only the candidates that pass the implementation-PR filter described below:
...
reports an absent or unknown latch in its issue comment. Genuine three-way rebase conflicts add the
machine-readable `<!-- ai:needs-human-latch reason=staged_support_rebase_conflict -->` marker the
poller's `release_staged_support_needs_human_latches` sweep matches; missing ledgers, baselines,
unsafe paths, and merge-tool failures omit it and remain human-gated. The handler also sends a
CRITICAL Telegram alert and suppresses generic diagnose/re-issue handling. The
...
- When `EVENTS_JSONL_ENABLED=true`, supported stable-prefix emitters also append a fail-open JSONL mirror at `.events/run-<run_id>.jsonl` after the original text line/comment marker emits. Current collectors still treat the text stream as authoritative; the JSONL mirror is additive machine-readable trace data.
...
2. Reset counters only when explicit flags are present: `--reset-stall` (stall only), `--reset-recovery` (recovery only), or `--force` (both). `--reset-recovery` / `--force` also clear the `judge_failed_fingerprints` ledger that `RECOVERY_COUNT_DISTINCT_FINDINGS=false` charges repeats against.
3. Transition the project status from `failed` to `in_progress`.
4. Resume normal wave processing immediately — including any fix-up issues the exhausted recovery arm filed from the final judge verdict (listed in the `## Project Failed` comment), which the poller tracks, merges, and re-judges without further manual diagnosis.
...
| `ORCH_INTEGRATION_MAX_AHEAD_COMMITS` | `10` | Backpressure **floor**; the effective threshold is `max(this, planned_issue_count + ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN)` so a project's own planned merges never self-deadlock. Evaluated against the integration branch's non-merge commits ahead of default (the poller's own `chore: sync main` merge commits are excluded; raw `ahead_by` is the fallback when the compare commit list is unavailable or truncated at 250). Activates `ai:integration-backpressure`, pauses additional sub-issue merges, and auto-clears once the work-commit count drops below the effective threshold. Must be a positive integer; invalid values fall back to `10`. |
| `ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN` | `20` | Headroom added to a project's planned sub-issue count when deriving the size-aware backpressure threshold (absorbs judge-added non-merge fix-up work; merge commits no longer count, including the `main`→integration sync merges that self-deadlocked project tele-funtoken-msg-scoring#3928). Raised from `5` to `20` after project #2974 self-deadlocked at 22 commits ahead. Must be a non-negative integer; the `orchestrate_poll` workflow sets `20` and invalid values fall back to the script-level `5`. |
...
| `ORCH_WORKTREE_REGISTRY_ENABLED` | `false` | Truthy values enable the additive `.worktrees/index.json` registry and the existing-cadence `scripts/worktree_gc.sh` cleanup path. Any other value leaves the current bare `git worktree` behavior unchanged. |
| `ORCH_WORKTREE_TTL_SECS` | `3600` | TTL gate for stale worktree-registry GC. Values must be integers in `300..86400`; invalid values clamp to `3600`. GC reaps only entries older than this TTL whose `owner_run_id` is absent from the reused queued/in-progress actions-runs snapshot/cache. |
| `ENABLE_SECURITY_PASS` | `true` | Truthy values enable the mandatory current-integration-head security pass before validation or finalization. Set to `false` for the immediate operator kill switch and legacy completion route. |
| `MAX_SECURITY_PASS_CYCLES` | `5` | Maximum completed consolidated fix cycles before persistent security findings force terminal `ai:security-pass-failed`. Must be a positive integer; invalid values fall back to `5`. Resets to `0` when an advancing integration head invalidates a recorded clean pass. |
| `MAX_SECURITY_PASS_FIX_REISSUES` | `2` | Maximum times the poller closes and re-issues a consolidated security-fix issue that `implement.yml` left in `ai:implementation-failed` (post-Codex validation failure or no-op implementation) within one fix cycle. Re-issue waits while the failure's fix-up blocker issues are still open. Exceeding the cap fails the pass as `ai:security-pass-failed`; `/re-security-pass` resets it. Must be a non-negative integer; invalid values fall back to `2`. |
| `SECURITY_PASS_CONFIDENCE_GATE` | `8` | Minimum confidence score for blocking security-pass findings. Must be an integer from 1 to 10; invalid values fall back to `8`. |
| `SECURITY_PASS_EXHAUSTION_JUDGE_ENABLED` | `true` | When `MAX_SECURITY_PASS_CYCLES` is spent with findings still open, consult the security-pass exhaustion judge (`prompts/mode-judge-security-pass-exhaustion.txt`) instead of terminalizing. Per remaining finding it chooses `accept_with_followup` (waiver recorded in state plus one non-blocking `ai:security` follow-up issue; the project completes once nothing else blocks), `keep_fixing` (one more consolidated fix issue for those findings), or `fail` (the pre-judge terminal `ai:security-pass-failed`). Any judge failure falls back to the terminal path. Set `false` to restore terminal failure on exhaustion. |
...
- If `.ai/validate.yml` is absent, validation runs a lightweight discovery phase (or reuses a cached hints file) and materializes the result into `.ai/validate.yml` in the runner's working tree so the template renderer can consume it. The cache uses a run-independent, repository/fingerprint-scoped staging path under `${RUNNER_TEMP}/validate-hints-cache/`; stage-in/out steps bridge that path to the per-run workspace so the cache version stays reusable when `WORKSPACE_REUSE_ENABLED=false`. The materialized file is never pushed back to the consumer repo; commit your own `.ai/validate.yml` for deterministic, no-codex behavior.
...
- `python-mongo-repo-checks` (like `python-mongo-flask`) installs the consumer's Python dependencies into the app image with `pip install -r` after copying the repo in, so repo checks and unit tests can import the consumer's modules. The file defaults to `requirements.txt`; override it with `slots.requirements_file` in `.ai/validate.yml`. The step is skipped when the file does not exist, and a requirements file that fails to install fails the image build.
...
| `VALIDATION_DISCOVERY_MODEL` | `openai/gpt-6-sol` | Codex model id. |
| `VALIDATION_DISCOVERY_REASONING_EFFORT` | `high` | Codex reasoning effort. |
...
Two automations take the operator out of that loop by default:

- **The daily promote cycle** in [`promote-main-to-stable.yml`](.github/workflows/promote-main-to-stable.yml)
  (schedule `0 0 * * *`, `cycle` job running `scripts/promote_main_cycle.sh`). A tick promotes `main`
  ...
  4. the verifying run: dispatched by the poller when the proving run merges (12f), on the next doc,
     with `promote_sha` = `main` as of that merge;
  5. the promotion: at the verifying run's ready-to-merge point the poller checks nothing untested
     landed since the smoke-tested commit, dispatches this workflow with `target_sha=promote_sha`
     (full release gate), and holds the verifying merge until the release finishes so `main` HEAD is
  ...
  Manual `workflow_dispatch` promotion stays available as the override and
...
- [`auto-release-stable.yml`](.github/workflows/auto-release-stable.yml) runs every 6 hours (at
  ...
  workflow shares the `promote-main-to-stable` concurrency group so it never races the promote job,
  and every stable-tag writer (`test-and-mark-stable.yml` and the legacy `mark-stable.yml`) runs its
  `release` job under the shared `stable-release-<repo>` lock, checks out the dispatched commit rather
  ...
  promote-main-to-stable run is active (the gate's E2E job cancels concurrent siblings). A gate that
  failed or timed out on the current tip is dispatched again on the next tick until
  `AUTO_RELEASE_STABLE_MAX_ATTEMPTS` (default 3) such runs have failed on that tip; only then does
  the tip hold (`AUTO_RELEASE_SKIPPED reason=last_gate_failed ... attempts=N max=M`) until the branch
  moves or an operator re-runs the gate. A cancelled gate is retried on the next tick and never
  counts as an attempt. Kill switch: `AUTO_RELEASE_STABLE_ENABLED=false`.

...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | No | `false` | orchestrate_poll | Opt-in gate for terminal stall escalation to human intervention (`escalate_human`). Default behavior remains legacy autonomous ladder (`close_and_reissue`/auto-retry paths), including fail-open fallback paths. |
...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | `false` | Opt-in: allow terminal stall escalation to human intervention; default keeps legacy autonomous recovery ladder |

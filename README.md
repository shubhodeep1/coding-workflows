...
| --- | --- | --- |
| `gate` | `review_autofix.yml` (before reviewers) | Lists open PRs on the PR's base (1 REST call, oldest first); for each **older** `ai/issue-*` PR (lower number, not draft, not `ai:review-blocked` / `ai:closed` / `ai:merged`, at most `MERGE_TRAIN_MAX_OLDER_PRS`) fetches its file list and intersects it with this PR's paths (taken from the diff `review_collect_pr_metadata.sh` already fetched — zero extra calls). Any overlap → label `ai:merge-queued`, one upserted `<!-- merge-train:queued -->` comment naming the blockers and files, `AUTOFIX_MERGE_QUEUED=true` + `AUTOFIX_STALE_BASE_SKIP=true` in `$GITHUB_ENV` so the rest of the run soft-exits. No overlap → continue, and drop a stale `ai:merge-queued` label if one is present. |
| `release` | `cancel_on_pr_close.yml` (every merged PR, `BASE_BRANCH` = the merged PR's base) and `orchestrate_poll.yml` (every tick, before the tracking-issue gate so it also runs in repos with no active project) | Lists open PRs (1 REST call); for each `ai:merge-queued` PR recomputes its blockers with the same rule (file lists cached per run); when none remain, removes the label, re-dispatches the review workflow (`ai-review.yml` → `internal-review.yml` → `review_autofix.yml`, first that accepts) with `pr_number`, and upserts a `<!-- merge-train:released -->` comment. A PR whose dispatch fails keeps the label and is retried on the next event or tick. |
...
instead of N². The orchestrator poller's in-progress conflict loop skips
`ai:merge-queued` PRs; its feature-sweep `update-branch` fast-forward still
applies, so a released PR is usually already up to date. Everything is

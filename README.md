...
- For the Phase O shared-prelude rollout, migrated mode prompts keep their legacy bodies in `prompts/mode-*.txt` while `prompts/_templates/*.txt` plus `scripts/assemble_prompt.sh` provide the flag-gated include expansion path. `scripts/render_prompt.sh` uses that path only when `PROMPT_PRELUDE_REFACTOR_ENABLED=true`; the acceptance default remains `false`. Both shell entrypoints always launch renderer Python through an allowlisted `env -i` environment with `python3 -I -B`; `PYTHON_ISOLATED_MODE` remains compatible but cannot disable isolation. Inline Python parsers in the clarify, plan, implement, review/autofix, and orchestrate workflows use the same isolation boundary and receive only their declared file or status inputs.

...
| `REVIEW_CONSOLIDATOR_MAX_TOKENS_OUT` | `16000` | Output cap (tokens; internally multiplied by ~4 to derive the byte truncation limit) for the consolidator. Output above the derived byte cap is truncated with a `TRUNCATED_BY_OUTPUT_CAP` marker; the parser tolerates the truncation cleanly. |

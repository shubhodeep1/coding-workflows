...
| `UNATTENDED_IDENTITY_REINJECT_ENABLED` | `false` | Opt-in prompt-render flag for the Phase B identity-recall block. When enabled, the canonical renderer `scripts/render_prompt.py` parses the canonical `prompts/mode-*.txt` role/goal opening paragraph and injects a rendered `<identity-recall>` block immediately after the opening paragraph of rendered phase prompts; `scripts/render_prompt.sh` injects the same block on its shell path and clears `UNATTENDED_IDENTITY_REINJECT_ENABLED` only after a successful shell injection so the Python backend avoids duplication while remaining the fail-open fallback when the shell path cannot inject. Parse failures fail open: rendering continues unchanged and logs `IDENTITY_REINJECT_PARSE_FAIL`. |

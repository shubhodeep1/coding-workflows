...
| `ORCHESTRATOR_MAX_CLARIFY_CYCLES` | No | `3` | orchestrate_clarify_respond | Maximum orchestrator clarification auto-answer cycles per issue. Once exceeded, or when a clarify hash repeats, auto-answering is blocked and the issue is escalated to `ai:blocked`. |
...
| `MAX_STALL_RECOVERIES_PER_ISSUE` | No | `5` | orchestrate_poll | Maximum stall recovery attempts per individual issue. Declarative recovery uses `STALL_RECOVERY_ACTIONS` by `stall_recovery_count` (index clamped to the last action; current default ladders are phase action, phase action, then `close_and_reissue` for most phases, while `ai:awaiting-approval`/`ai:ready-to-merge` repeat their phase action). When `ENABLE_STALL_HUMAN_TERMINALIZATION=true`, the alternate ladder allows third-step `escalate_human` for all pipeline phases. When `run_stall_judge` is active, judge failures/invalid output/unsupported actions fail-open to that same declarative action. At this limit, the next action becomes `skip` (`ai:closed`) so the wave can advance; the judge evaluates the gap at wave completion. |
...
| `MAX_IMPL_NOOP_REISSUES` | No | `2` | orchestrate_poll | Maximum automatic re-issues for an `ai:implementation-failed` issue before the poller closes it as likely already implemented and defers final verification to the wave-completion judge. Must be a positive integer; invalid values fallback to `2`. |
...
| `ORCHESTRATOR_MAX_CLARIFY_CYCLES` | `3` | Maximum orchestrator clarify auto-answer cycles before escalating to `ai:blocked` |
...
| `MAX_STALL_RECOVERIES_PER_ISSUE` | `5` | Max stall recovery attempts per issue; declarative ladder is clamped by `stall_recovery_count` (defaults end in `close_and_reissue` for most phases and repeat action for `ai:awaiting-approval`/`ai:ready-to-merge`; `escalate_human` is opt-in via `ENABLE_STALL_HUMAN_TERMINALIZATION=true`), judge path fail-opens to same declarative action, then next action is `skip` (`ai:closed`) |
...
| `MAX_IMPL_NOOP_REISSUES` | `2` | Max automatic re-issues for `ai:implementation-failed` before closing as likely already implemented and deferring to judge verification |

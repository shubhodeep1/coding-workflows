...
- `completed_phases` / `skipped_phases` show which terminal workflow phases ran, `slot_results` carries per-slot `attempt_count` / `status` / `failure_class`, `budget_*` reports elapsed vs total vs remaining seconds, `finalize_reason` explains why the run stopped, `partial_finalize` / `partial_finalize_reason` / `partial_finalize_phase` / `partial_comment_posted` / `incomplete_phases` describe any partial-completion path, `edits_pushed` shows whether the run successfully pushed its pending commit/merge, and `resume_round` is a placeholder-safe counter for future resume loops.
- When `partial_finalize=true`, the normal editor-summary comment is skipped and the workflow posts a dedicated `<!-- REVIEW_AUTOFIX_PARTIAL_V1 -->` PR comment with machine-readable `reason`, `phase`, `completed_scope`, `incomplete_scope`, `validated_edits_committed`, `edits_pushed`, and `resume_round` fields.
- The same partial path persists `.ai/review_runtime/pr-<PR>/round-<N>/partial_finalize.json` alongside the usual ledger/runtime cache so a later resume/no-progress consumer can continue from structured state instead of parsing prose. The JSON currently stores `schema_version`, `partial_finalize`, `reason`, `phase`, `completed_scope[]`, `incomplete_scope[]`, `validated_edits_committed`, `edits_pushed`, `resume_round`, and `comment_posted`.

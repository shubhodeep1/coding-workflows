...
| `MODEL_PROVIDER_BROKER_MAX_TOTAL_COST_USD` | No | derived | all model-facing phases | Aggregate decimal cost budget per broker session in USD (#4090). Each admitted request reserves its worst-case cost at the configured price ceilings (input estimate x `MAX_PROMPT_PRICE` + output ceiling x `MAX_COMPLETION_PRICE`, per million tokens, + `MAX_REQUEST_PRICE`) and is settled to the provider-reported usage; an upstream error settles to zero. When unset the broker derives the budget as both total token budgets at the price ceilings plus `MAX_REQUESTS x MAX_REQUEST_PRICE`, so it never bites below what those limits already permit. Set it to make the spend control binding; must be a positive finite decimal. |

...
| `ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN` | No | `5` | orchestrate_poll | Headroom added to a project's planned sub-issue count when deriving the size-aware backpressure threshold (see `ORCH_INTEGRATION_MAX_AHEAD_COMMITS`). Absorbs the non-sub-issue commits a healthy project still accrues on its integration branch before draining — `main`→integration syncs and a handful of judge-added fix-up issues. Raise it for projects that routinely spawn many fix-ups; lower it to tighten the gate. Must be a non-negative integer; invalid values fall back to `5`. |
...
| `ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN` | `5` | Headroom added to a project's planned sub-issue count when deriving the size-aware backpressure threshold (syncs + fix-up issues). Non-negative integer |
...
| `ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN` | `5` | Headroom added to a project's planned sub-issue count when deriving the size-aware backpressure threshold (absorbs `main`→integration syncs and judge-added fix-up issues). Must be a non-negative integer; invalid values fall back to `5`. |

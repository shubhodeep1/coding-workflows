...
| `ORCH_INTEGRATION_BACKPRESSURE_PROJECT_MARGIN` | `20` | Headroom added to a project's planned sub-issue count when deriving the size-aware backpressure threshold (absorbs judge conflict-merges and judge-added fix-up issues; `main`→integration sync-merges no longer count since project tele-funtoken-msg-scoring#3928 self-deadlocked on 15 of them). Raised from `5` to `20` after project #2974 self-deadlocked at 22 commits ahead. Must be a non-negative integer; the `orchestrate_poll` workflow sets `20` and invalid values fall back to the script-level `5`. |

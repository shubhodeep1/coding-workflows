...
| `ENABLE_CLOSE_MERGED_ISSUES` | No | `true` | orchestrate_poll | Enable the per-cycle sweep that closes open GitHub issues carrying `ai:merged` once at least one cross-referenced PR is verified merged via the GitHub REST API (current implementation processes up to 200 open matches per cycle). Applies to both orchestrator-managed child issues and standalone (non-orchestrator) issues. Tracking issues (`ai:orchestrator-tracking`) are intentionally skipped — they are closed by the orchestrator project completion path. If an issue has `ai:merged` but no merged PR can be verified on its timeline, the sweep leaves it open and sends a Telegram `WARNING` alert instead of guessing. |
...
11b. **Atomic final merge:** When a project is complete (or validated), the poller creates/reuses a final PR from integration branch to default branch and squash-merges it. If conflicts occur, state moves to `merge_conflict` with an actionable tracking comment and no `complete` transition until resolved.

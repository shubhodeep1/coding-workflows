...
- [`auto-release-stable.yml`](.github/workflows/auto-release-stable.yml) runs every 6 hours and
  ...
  and the gate's `release` job holds its own `stable-release-<repo>` lock and refuses to tag a
  checkout that is no longer the branch tip (`RELEASE_STALE_TIP`), so a gate that finishes late can
  never move the `stable` tag backwards. It skips while a gate run is queued or running and after the gate failed on the

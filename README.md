...
| `BULK_DELETE_THRESHOLD` | No | `3` | implement | Maximum number of file deletions allowed in a single AI implementation commit when **any** staged deletion is a non-`.md` file. This is the strict cap that catches accidental source-tree wipes. Set higher for legitimate large refactors, or bypass on a per-run basis via `ALLOW_BULK_DELETE=true`. See "Destructive-commit guard" below. |
...
| `ORCH_INTEGRATION_STALE_ALERT_HOURS` | `6` | Hours an integration branch may remain ahead of default after the last squash before the poller emits `INTEGRATION_STALE_ALERT_SENT` and a `WARNING` alert |
...
| `ORCH_INTEGRATION_MAX_AHEAD_COMMITS` | `10` | Ahead-of-default threshold that activates `ai:integration-backpressure`, pauses additional sub-issue merges into the integration branch, and auto-clears once the backlog drops below the threshold |
...
| `ORCH_INTEGRATION_STALE_ALERT_HOURS` | `6` | When the integration branch remains ahead of default for at least this many hours since the last successful squash, the poller emits `INTEGRATION_STALE_ALERT_SENT` and sends a `WARNING` alert. Must be a positive integer; invalid values fall back to `6`. |
...
| `ORCH_INTEGRATION_MAX_AHEAD_COMMITS` | `10` | Ahead-of-default threshold that activates `ai:integration-backpressure`, pauses additional sub-issue merges into the integration branch, and auto-clears once `ahead_by` drops below the threshold. Must be a positive integer; invalid values fall back to `10`. |

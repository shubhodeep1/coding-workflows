...
| `VALIDATION_USE_TEMPLATES` | `true` (script-level default `false`; `.github/workflows/validate.yml` passes `true` when unset) | Truthy values (`1/true/yes/on`, case-insensitive) switch `scripts/validate_process.sh` Phase 1 to template renderer mode (`scripts/render_validation_templates.py`) instead of freehand Codex harness generation. Missing manifest/renderer/schema/template assets fail with `raw_status=harness_error`; there is no silent fallback when opt-in is enabled. |

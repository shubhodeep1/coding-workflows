...
| `EDITOR_MAX_WALL` | No | `7800` | review_autofix, implement | Maximum wall-clock seconds per editor attempt (~130 min). Budget-aware: auto-capped to remaining job time minus a 2-min buffer. The 3-hour job cap allows ~1 full attempt; combined with the orchestrator's 60-minute per-issue scope target, this leaves headroom for one retry on watchdog kill. |

...
9b. **Targeted vs legacy post-Codex failure flow:** Targeted post-Codex pre-commit no-op failures follow the `ai:implementation-failed` lane above (label source issue, close/re-issue, retry). Other implement workflow failures remain on the legacy path (`failure()`/`cancelled()` handling in `implement.yml`), with failure comments/alerts and no automatic relabel to `ai:implementation-failed` unless the no-op condition is hit.

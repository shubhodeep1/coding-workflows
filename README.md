...
> (default `true`) the `gate` job now reads the newest marker for the PR's
> current head on `workflow_dispatch` events and sets `should_run=false`
...
> newer non-terminal marker for the same head reopens it, and a comments API

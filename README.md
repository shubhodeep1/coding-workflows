...
| `SECURITY_AUDIT_FP_EXCLUSIONS` | No | `scripts/security_audit_fp_exclusions.json` | security-audit | Path to the JSON false-positive exclusion catalog within immutable workflow support. Relative paths resolve only beneath `SECURITY_AUDIT_SUPPORT_DIR`; absolute paths, traversal, and symlinks must still canonicalize inside that directory. Audited-repository copies are never trusted. Missing, malformed, escaping, or matcher-less catalogs fail the run closed rather than suppressing findings. |
...
| `SECURITY_AUDIT_FP_EXCLUSIONS` | `scripts/security_audit_fp_exclusions.json` | JSON catalog of false-positive suppression rules loaded only from canonical immutable workflow support; checkout overrides, path escapes, and matcher-less rules fail closed |

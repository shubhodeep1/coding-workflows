...
| `SECURITY_PASS_ADVISORY_FOLLOWUP_CAP` | No | `5` | orchestrate_poll | Maximum rows filed per tick from the untruncated severity/confidence-ordered advisory queue. `0` disables filing without making advisories blocking. |
...
| `SECURITY_PASS_ADVISORY_FOLLOWUP_CAP` | `5` | Maximum pre-existing-code advisory issues filed per poll tick from the untruncated authoritative-state queue; `0` keeps them queued without blocking. |
...
| `SECURITY_PASS_ADVISORY_FOLLOWUP_CAP` | `5` | Maximum advisory rows filed per tick from the untruncated severity/confidence-ordered queue. `0` files nothing and never turns advisories into blockers. |

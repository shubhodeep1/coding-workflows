...
as `/implement-plan-claude`, sized to one issue: a single-phase plan and PR,
then conformance, security pass, runtime validation, the completion PR (which
closes the issue), verify-activation, and `/deploy-activate` when needed.
Claude sessions act instead of asking (CLAUDE.md §28) and record every
judgement call as an assumption in the plan and PR.
...
4. The routine follows `.claude/commands/claude-issue-dispatch.md` and starts
   ...
   That session writes `docs/plans/issue-<N>-<topic>-plan.md` and the progress
   log, opens the phase PR with `Refs #N`, and arms the Sonnet checker. From
   then on every stage is `/implement-plan-claude` in issue mode.
   `review_autofix.yml` reviews and auto-merges the PRs as usual.
...
Telegram ERROR. A Claude session that hits a CLAUDE.md §28.C hard blocker
(exhausted cap, failed security/validation run, an ask-first operation, a
missing credential) comments once on the issue, labels it
`ai:claude-blocked`, and sends a push notification. If `claude_issue_route.py`

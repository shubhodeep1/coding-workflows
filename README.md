...
- Model-facing issue analysis, orchestrator polling, implementation and repair, validation, check-failure triage, security audits, source and consumer workflow retros, review consolidation, review editing, review-blocked judging/fixing, workflow-log analysis, and merge-conflict resolution use `scripts/model_provider_broker.py`, a bounded loopback proxy that retains the real OpenRouter credential. Model children receive only an ephemeral per-run broker token in a clean environment; read-only phases use Codex's `read-only` sandbox, writer phases retain only their required workspace permissions, and the conflict writer runs as an unprivileged user with `.git` inaccessible. The broker authorizes only the phase's configured model IDs, normalizes endpoint-specific output limits, and injects server-controlled provider price ceilings. `MODEL_PROVIDER_BROKER_MAX_REQUESTS` defaults to `100` (valid range `1..100`), `MODEL_PROVIDER_BROKER_MAX_OUTPUT_TOKENS` defaults to `16384`, and `MODEL_PROVIDER_BROKER_MAX_TOTAL_OUTPUT_TOKENS` defaults to `65536`; either exhausted budget returns HTTP 429 and the phase fails closed. There is no direct-provider fallback when broker startup fails.

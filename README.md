...
- Reviewer stall recovery is greppable in raw logs as `codex_stall_killed` plus `REVIEWER_ADVANCE: slot=... model=... reason=... next_action=...` (with optional `next_attempt=` / `next_model=` when a new attempt is scheduled); the structured summary is the durable end-of-run record for those same events.

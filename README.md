...
| `TARGETED_FILE_CONTEXT_MAX_BYTES` | No | `102400` (100 KB ≈ 25k tokens) | implement, review_autofix, orchestrate_poll | Total byte budget for the targeted-file-context block built by `scripts/targeted_file_context.py`. The mechanism pre-loads files named in the plan (implement) / `LAST_RUN_CHANGED_FILES_FILE` (autofix) / conflicted-file allowlist (resolver) so the model's first turn is a write rather than a recon read. Files are processed in source order until the budget is exhausted; a file that would push the cumulative byte count over the cap is NOT head-truncated, it gets a `(NN bytes; would overflow total budget — read with read tool, max_bytes=…, used=…)` marker so the model uses its native targeted-read flow rather than being misled by a truncated head. Every path the caller passes is reported, either inlined or as a marker — there is no separate file-count cap. The budget is charged with **source content**, meaning inlined file bytes *and* overflow representations (semble chunk retrieval, read-fallback heads): an overflow representation is rendered only when it fits the remaining headroom, so source content never exceeds the cap. Once the headroom is gone the semble subprocess is skipped entirely and the remaining files get markers (telemetry: `SEMBLE_FALLBACK target=overflow … reason=budget-exhausted`). Per-entry framing (`FILE` / `END FILE` lines, markers, the summary line) is not charged; it is a small overhead of roughly 200 bytes per entry, so leave a reserve when sizing an assembled prompt against a hard model stdin cap. Set to `0` to disable inlining entirely (block becomes a single-line "(disabled)" marker). Lower this on cost-sensitive repos; raise on small-file repos to fit more verbatim content. |
| `IMPLEMENT_PROMPT_CODEX_STDIN_CAP_BYTES` | No | `1048576` | implement | Codex-cli's `turn/start` stdin cap, in **characters**. After the implement prompt is assembled, `implement.yml` logs its size and emits a workflow warning when it is at or over this value, naming the knobs that shrink it (`TARGETED_FILE_CONTEXT_MAX_BYTES`, the static prefix, memory and issue-comment context). The prompt is measured with `wc -c`, i.e. bytes; a UTF-8 byte count is never lower than the character count, so the comparison is conservative and can only warn early, never miss an overshoot. The value must be a positive integer; anything else is coerced back to `1048576` with a warning, because an invalid value used to disarm the guard silently. Override only when the codex-cli build in use changes its cap. The warning is diagnostic: it does not fail the step, the editor run itself fails on the cap. |

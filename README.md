...
| `ORCHESTRATOR_STATE_AUTH_KEYRING` | **Yes for orchestrator deployments** | orchestrate_poll, review_autofix conflict preparation | Dedicated state-signing keyring JSON. Schema: `{"schema_version":"orchestrator_state_auth_keyring.v1","active_key_id":"<id>","keys":[{"key_id":"<id>","key_base64":"<base64>"}]}`. IDs are unique, 1–64 safe characters; 1–8 decoded keys of 32–64 bytes are accepted. New `orchestrator_state_auth.v2` state uses only the active key, while verification accepts retained previous keys. Keep the previous key until every active project has republished under the new active key. Provision this secret in every orchestrator consumer before syncing the release: an absent secret prevents the reusable poller workflow from starting, while malformed configuration reaches the in-script gate and pauses project mutations. |
...
- **Periodic default-branch security audit.** `.github/workflows/security-audit.yml` runs weekly at `08:00 UTC` and on `workflow_dispatch`, stays inert unless `SECURITY_AUDIT_ENABLED=true`, and uses `scripts/security_audit.sh` plus `prompts/mode-security-audit.txt` to audit the checked-out default branch with OWASP Top 10 + STRIDE framing. Surviving findings are appended under dated sections on the stable `AI Security Audit Tracker` issue (`ai:security-audit`, marker `<!-- ai:security-audit-tracker:v1 -->`), and up to three weekly `ai:security` follow-up issues are opened for non-excluded findings that meet the confidence gate. `SECURITY_AUDIT_CRON` is documentation-only because GitHub Actions schedule cron is literal YAML, not repo-variable-driven.

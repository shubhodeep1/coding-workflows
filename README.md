...
| `MAX_AUTOFIX_ITERATIONS` | No | `3` | review_autofix | Maximum consecutive autofix rounds before the review loop stops and marks the PR `ai:review-blocked`. **Orchestrator override:** when `ORCH_PR_AUTOFIX_FLOW_ENABLED=true` (default) the retrigger guard reclassifies the PR by base/head branch and overrides the cap to `1` for orchestrator intermediate PRs (sub-issue PR → `orchestrator/project-*`), skipping the judge entirely so the PR auto-merges into the integration branch as soon as CI is green + mergeable; orchestrator final PRs (`orchestrator/project-*` → default branch) keep the `3`-iteration inner loop. Non-orchestrator PRs are unaffected. See [Orchestrator PR autofix flow](#orchestrator-pr-autofix-flow). |
| `ORCH_PR_AUTOFIX_FLOW_ENABLED` | No | `true` | review_autofix, orchestrate_poll | Master switch for the orchestrator-aware PR autofix flow. When `true`, `review_autofix.yml`'s retrigger guard classifies the PR (`orch_intermediate` / `orch_final` / `other`) by base/head branch and applies per-mode autofix caps + judge gating; `orchestrate_poll_process.sh` bypasses the `MAX_JUDGE_CYCLES` cap while the integration→default-branch final PR is open and pending merge so the final PR can run unlimited 3-autofix→judge cycles until mergeable. Set to `false` to revert to the legacy uniform 3-autofix→judge→cap-25 flow on every PR (rollback path with no code changes). See [Orchestrator PR autofix flow](#orchestrator-pr-autofix-flow). |
...
| `CHECK_RUNS_AUTOFIX_ENABLED` | No | `true` | review_autofix | When `true` (default), the workflow snapshots failed and incomplete GitHub check-runs on the PR head SHA into `${PR_CHECK_RUNS_CONTEXT_FILE}` and feeds it to reviewers + editor so CI / lint failures are detected and fixed on every run. The "Collect PR check-run failures" step in `.github/workflows/review_autofix.yml` issues exactly one `GET /repos/{repo}/commits/{sha}/check-runs?per_page=100` call per poll iteration; reviewers see the file as a numbered context section, and the editor prompt elevates failed entries to the top of the WILL_FIX priority order (see `scripts/review_apply_fixes.sh` "CI / LINT CHECK-RUN FAILURES" block). Fail-open: any API error writes a sentinel file (`collection_status: api_error`) and the autofix pipeline continues — reviewers/editor are explicitly told to treat the absence-of-failures signal as unknown rather than confirmed-passing. Set to `false` to disable check-run collection entirely (the file still gets written with `collection_status: disabled` and zero counts so preflight always passes). |
| `CHECK_RUNS_WAIT_TIMEOUT_SECS` | No | `600` | review_autofix | Maximum seconds the "Collect PR check-run failures" step waits for in-progress / queued check-runs to complete before snapshotting. When the timeout trips, the snapshot proceeds with whatever data exists and emits a `::warning::CHECK_RUNS_WAIT_TIMEOUT` log line. Integer in `0..3600`; invalid or out-of-range values clamp to `600`. Set to `0` to skip waiting entirely (snapshot whatever is currently completed). |
| `CHECK_RUNS_POLL_INTERVAL_SECS` | No | `20` | review_autofix | Sleep interval between check-run status polls in the wait loop. Integer in `5..300`; invalid values clamp to `20`. Each poll issues one `GET /repos/{repo}/commits/{sha}/check-runs` call. |
...
| `XPOLL_SUMMARISER_REASONING` | No | `medium` | review_autofix | Reasoning effort (`xhigh` / `high` / `medium` / `low`) applied to the summariser model via its isolated `CODEX_HOME` config.toml. Default is `medium` — the summariser task is cross-reviewer dedup, which does not require maximum reasoning depth. Isolated config guarantees the override cannot leak into the editor's codex-cli invocation. |
...
Orchestrator-managed PRs follow a per-PR-kind autofix contract that differs from the legacy uniform `3-autofix → judge → cap-25` flow. The contract is enforced jointly by `.github/workflows/review_autofix.yml` (per-PR autofix loop) and `scripts/orchestrate_poll_process.sh` (orchestrator-level judge cycle cap). It is gated by the master switch `ORCH_PR_AUTOFIX_FLOW_ENABLED` (default `true`) — set to `false` to revert every PR to the legacy uniform flow without code changes.
...
| Mode | Detection | Autofix cap | Judge | Orchestrator cap |
|---|---|---|---|---|
| `orch_intermediate` | head matches pattern AND base matches pattern (sub-issue PR → integration branch) | **1** iteration | **Skipped entirely** (`skip_judge=true`) | n/a |
| `orch_final` | head matches pattern AND base does NOT match pattern (integration branch → default branch) | **3** iterations (legacy default) | Runs normally | **Bypassed** (unlimited cycles while `final_merge_status=pending`) |
| `other` | neither head nor base matches pattern (non-orchestrator PR) | `MAX_AUTOFIX_ITERATIONS` (default `3`) | Runs normally | `MAX_JUDGE_CYCLES` (default `25`) |

**Intermediate PR behavior** (`orch_intermediate`):
- Run 1 (user push): reviewer + editor run; if fixes are needed, reviewer commits one `[ai-autofix]` commit (which also addresses CI / lint check-run failures via the existing `CHECK_RUNS_AUTOFIX_ENABLED=true` path).
- Run 2 (triggered by the `[ai-autofix]` commit): `autofix_count=1 ≥ effective_max=1` → `max_iterations_reached=true`. Because `skip_judge=true`, the rb_judge step, the `ai:review-blocked` label markers, and the Telegram review-blocked alert are all skipped. The "Mark linked issues ready to merge" + "Enable auto-merge on PR" + "Telegram success" steps fire (their gates were widened to `(max_iterations_reached != 'true' || skip_judge == 'true')`).
- The PR merges into the integration branch only when the existing orchestrator merge gate clears: `mergeable=true` AND `_pr_checks_completed` AND `ai:ready-to-merge` label set. CI is **not** bypassed — the 1 autofix round is your single chance to fix CI; if it stays red, the merge gate blocks the merge until CI is fixed by another path (force-push, manual fix, stall-recovery).
- The `force_rb_judge` stall-recovery path (dispatched by the orchestrator stall poller for issues stuck at `ai:review-blocked` past the threshold) is **unchanged** — it forces the rb_judge step to run regardless of mode. The retrigger guard suppresses `skip_judge` (forces it to `false`) when `FORCE_RB_JUDGE=true` so this escape hatch keeps working on stuck intermediate PRs.

**Final PR behavior** (`orch_final`):
- Inner loop matches the legacy `MAX_AUTOFIX_ITERATIONS=3` cycle: 3 consecutive `[ai-autofix]` commits → judge runs → judge may push `[judge-fix]` → autofix resumes → repeat.
...
**Non-orchestrator PRs** (`other`): unchanged. `MAX_AUTOFIX_ITERATIONS=3`, judge runs after exhaustion, `MAX_JUDGE_CYCLES=25` cap applies.

**Failure modes**:
- **Intermediate PR with persistent CI failure**: 1 autofix round attempts CI fix; if CI stays red, `_pr_checks_completed` returns false and the orchestrator does not merge. Existing stall-recovery contracts (`STALL_THRESHOLD_DONE_MINUTES`, `STALL_THRESHOLD_REVIEW_BLOCKED_MINUTES`) recover the issue.
- **Final PR with bad judge verdict loop**: with the cap bypassed, the loop continues indefinitely as long as judge keeps producing `[judge-fix]` commits. Operator intervention path: set `ORCH_PR_AUTOFIX_FLOW_ENABLED=false` to restore the cap, then take action against the PR.
- **Branch naming mismatch**: if your orchestrator pushes to a branch that does not match `^orchestrator/project-`, the classifier falls through to `other` and the legacy flow applies. Override `ORCH_INTEGRATION_BRANCH_PATTERN` to match your naming.
...
- To disable / roll back: set repo var `ORCH_PR_AUTOFIX_FLOW_ENABLED=false`. Both `review_autofix.yml` and `orchestrate_poll_process.sh` honour the kill switch and revert to legacy behavior on the next workflow run / poll tick.
...
| `VALIDATION_USE_TEMPLATES` | `true` | Truthy values (`1/true/yes/on`, case-insensitive) run `scripts/validate_process.sh` Phase 1 through the template renderer (`scripts/render_validation_templates.py`). This is the default path. Setting `VALIDATION_USE_TEMPLATES=false` now returns `raw_status=harness_error` because freehand Codex harness generation was removed. Missing manifest/renderer/schema/template assets also fail with `raw_status=harness_error`; there is no fallback path. |

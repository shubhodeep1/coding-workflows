...
| `STAGED_SUPPORT_LATCH_AUTO_RELEASE_ENABLED` | No | `true` | orchestrate_poll | In this source repository only, release the `ai:needs-human` latch that `implement.yml`'s staged-support rejection handler sets on `IMPLEMENT_STAGED_SUPPORT_REBASE_CONFLICT` once the engine carries `scripts/implement_staged_support_workspace.sh` (the fix). The poller accepts only a matching latch comment from an `OWNER`, `MEMBER`, `COLLABORATOR`, or installed `[bot]`, restores `ai:awaiting-approval`, and posts `/approved`, at most once per issue per engine commit, when no other human-gated latch is present. A failed `/approved` write restores `ai:needs-human`; every other latch reason and every consumer repository stay human-cleared. Set to `false` to keep every latch strictly human-cleared. |
...
reports an absent or unknown latch in its issue comment (which now opens with the machine-readable
`<!-- ai:needs-human-latch reason=staged_support_rebase_conflict -->` marker the poller's
`release_staged_support_needs_human_latches` sweep matches) and CRITICAL Telegram alert, and suppresses generic diagnose/re-issue handling. The
...
| `STAGED_SUPPORT_LATCH_AUTO_RELEASE_ENABLED` | `true` | In `shubhodeep1/coding-workflows` only, truthy values let the poller release the staged-support `ai:needs-human` latch (marker `<!-- ai:needs-human-latch reason=staged_support_rebase_conflict -->` or the pre-marker header) when the comment author is an `OWNER`, `MEMBER`, `COLLABORATOR`, or installed `[bot]`. It restores `ai:awaiting-approval` and posts `/approved` once per issue per engine commit when the engine carries `scripts/implement_staged_support_workspace.sh`; a failed comment write restores the human latch. Any other value keeps every latch human-cleared. |

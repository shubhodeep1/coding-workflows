...
**Workflow-support overlay drift is never committed (source repo).** The "Stage workflow support files" step installs the `SCRIPT_REF` copies of the runtime helper scripts (`scripts/gh_helpers.sh`, `scripts/codex_helpers.sh`, `scripts/memory_helpers.sh`, `scripts/targeted_file_context.py`, …) over the checked-out tree. In `shubhodeep1/coding-workflows` those helpers are tracked, so when the implementation branch is cut from an integration branch that has diverged from `main` in any of them (an `orchestrator/project-*` branch), the overlay leaves those files modified vs `HEAD` before Codex runs, and `git add -u` used to stage that drift as if the model had authored it — the PR then silently reverted the base branch's own helper changes (PR #4079 dropped the `model_provider_broker_*` functions from `scripts/codex_helpers.sh`, and every AI review run on that PR died with `model_provider_broker_start: command not found`). The "Run Codex implementation" step now snapshots the blob of every tracked path that is already dirty before the first Codex attempt (`PRE_CODEX_DIRTY_TRACKED_FILE`, `<blob>\t<path>` per line, in `RUNTIME_DIR`). On the source repo only, both the preflight destructive/scope guard and `scripts/implement_commit_changes.sh` exclude any such path whose content is still exactly the pre-Codex blob from staging (`:(exclude,literal)<path>` pathspecs), log them under `Workflow-support overlay drift excluded from staging`, and keep them out of the no-op handler's `remaining_changes` report. Paths Codex did edit are committed as before, with a `::warning::Codex edited <path> on top of the workflow-support overlay copy` because that edit sits on the `SCRIPT_REF` content rather than the base branch's. The excluded overlay copies stay in the worktree — later steps in the job source the `SCRIPT_REF` helper versions from them — and are recorded in `RUNTIME_DIR/support_overlay_excluded.tsv` (`<stored blob>\t<mode>\t<path>`) so the "Push branch" step can park them (`git restore --source=HEAD`) while its non-fast-forward `git rebase` fallback needs a clean tree and re-materialise them from the stored blobs on exit. Consumer repos are unaffected: their fetched helpers are already kept out of commits by the generated `scripts/.gitignore`. Regression test: `tests/test_implement_support_overlay_drift.py`.

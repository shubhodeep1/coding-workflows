...
| `MAX_POST_CODEX_REPAIR_ATTEMPTS` | No | `1` | implement | Maximum in-job post-Codex syntax-repair attempts after `Validate syntax of changed files` fails. Must be a positive integer; invalid values fallback to `1`. |
...
| `MAX_POST_CODEX_REPAIR_ATTEMPTS` | No | `1` | implement | Maximum in-job post-Codex syntax-repair attempts after `Validate syntax of changed files` fails. Must be a positive integer; invalid values fallback to `1`. The repair loop runs only for syntax-validator failures, enforces an allow-list scope guard, and then falls back to the existing diagnose/fix-up path when attempts are exhausted. |
...
| `MAX_POST_CODEX_REPAIR_ATTEMPTS` | `1` | Max in-job post-Codex syntax-repair attempts in `implement`; must be positive integer (invalid values fallback to `1`) |

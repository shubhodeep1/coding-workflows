...
| `ORCHESTRATE_SHORTCIRCUIT_MAX_CHARS` | No | `1200` | orchestrate | Pre-LLM short-circuit threshold for creating a single plain issue. The evaluation first resolves any repo-file references (e.g. `docs/plan.md`) found in the description and includes their content in the effective length and marker checks. If the effective length is `<=` this value and no multi-step markers match (`step\s*\d|phase\s*\d|wave\s*\d|\bthen\b|\bafter\b|\band\s+then\b|multi[- ]?step`), orchestrate skips decomposer/tracking/wave dispatch and opens one unlabeled plain issue from the description. |
...
| `ORCHESTRATE_SHORTCIRCUIT_MAX_CHARS` | `1200` | Pre-LLM short-circuit threshold; resolves repo-file references in the description before evaluating length and multi-step markers |
...
1. **Pre-LLM short-circuit (cost guard):** The evaluation first resolves repo-file references found in the description (e.g. `docs/plan.md`) and includes their content in both the length and marker checks (capped at 50 KiB per file / 200 KiB total). If the effective length is short (`<= ORCHESTRATE_SHORTCIRCUIT_MAX_CHARS`, default `1200`) and has no multi-step markers (`step\s*\d|phase\s*\d|wave\s*\d|\bthen\b|\bafter\b|\band\s+then\b|multi[- ]?step`), orchestrate skips decomposer entirely and creates exactly one plain issue from the description. This path does not apply orchestrator labels and does not create tracking issue, integration branch, state comment, or wave dispatch.

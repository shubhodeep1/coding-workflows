...
| `STAGED_SUPPORT_LATCH_AUTO_RELEASE_ENABLED` | No | `true` | orchestrate_poll | Release the `ai:needs-human` latch that `implement.yml`'s staged-support rejection handler sets on `IMPLEMENT_STAGED_SUPPORT_REBASE_CONFLICT` once the engine carries `scripts/implement_staged_support_workspace.sh` (the fix): the poller restores `ai:awaiting-approval` and posts `/approved`, at most once per issue per engine commit, only for issues whose latest latch comment is the staged-support one and that carry no other human-gated latch. Every other `ai:needs-human` reason stays human-cleared. Set to `false` to keep every latch strictly human-cleared. |
...
| `STAGED_SUPPORT_LATCH_AUTO_RELEASE_ENABLED` | `true` | Release the staged-support `ai:needs-human` latch once the engine no longer produces the re-base conflict; `false` keeps every latch human-cleared. |
...
| `STAGED_SUPPORT_LATCH_AUTO_RELEASE_ENABLED` | `true` | Truthy values let the poller release the staged-support `ai:needs-human` latch (marker `<!-- ai:needs-human-latch reason=staged_support_rebase_conflict -->` or the pre-marker header) by restoring `ai:awaiting-approval` and posting `/approved`, once per issue per engine commit, when the engine carries `scripts/implement_staged_support_workspace.sh`. Any other value keeps every latch human-cleared. |

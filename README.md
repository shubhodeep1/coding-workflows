...
| `JUDGE_PR_DIFF_MAX_BYTES` | `65536` | Byte cap on each PR diff embedded in the wave judge prompt, applied after the existing 500-line cap. A diff over the cap is cut to a UTF-8-safe prefix and prefixed with a `[NOTE: PR diff is N bytes ... truncated ...]` line so the judge knows to read the files directly. Exists because codex rejects any prompt over 1,048,576 characters before the model runs, and the line cap alone let PRs that commit minified bundles (single 40-70 KB lines) push the prompt past it (run 35425771769, project #3928). Non-numeric or non-positive values fall back to the default |

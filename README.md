...
11c. **Atomic final merge:** When a project is complete (or validated), the poller creates/reuses a final PR from integration branch to default branch and squash-merges it. If conflicts occur, state moves to `merge_conflict` with an actionable tracking comment and no `complete` transition until resolved.

...
- Model-facing issue analysis, workflow-log analysis, and merge-conflict resolution use `scripts/model_provider_broker.py`, a bounded loopback proxy that retains the real OpenRouter credential. Model children receive only an ephemeral per-run broker token in a clean environment; read-only phases use Codex's `read-only` sandbox, while the conflict writer runs as an unprivileged user with `.git` inaccessible. `MODEL_PROVIDER_BROKER_MAX_REQUESTS` defaults to `100` (valid range `1..100`); exhaustion returns HTTP 429 and the phase fails closed. There is no direct-provider fallback when broker startup fails.

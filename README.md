...
| `OPENROUTER_PROMPT_BUDGET_TOKENS` | No | `160000` | opt-in callers of `scripts/openrouter_prompt_cache.py::compact_if_over_budget` | Default token budget for the dormant prompt-compaction helper. The helper uses the repo's shared `~4 chars/token` approximation when future callers pass `budget_tokens=None`; no current workflow path reads this variable yet. |

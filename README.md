...
| `ENABLE_REVIEWER_TWO_PASS` | No | `true` | review_autofix | When true, reviewers run two passes per iteration: pass 1 at `xhigh` reasoning (broad sweep), then pass 2 at the scheduled reasoning level with a cross-pollination summary of pass 1 findings. Set to `false` to use a single pass at the scheduled reasoning level. |
...
- **No-progress detection + step wall-clock cap + immediate-judge dispatch.** Three small additions close the failure mode where a structurally-impossible integration merge (sub-issues with logically contradictory `must_contain` / `must_not_contain` fingerprints) consumed an entire 180-min job budget on three resolver attempts that all reproduced the identical fingerprint violation set. (1) **No-progress detection** inside the retry loop — after each attempt's soft validation, when `IS_INTEGRATION_SYNC=true` and the current `${RUNTIME_DIR}/resolver_fp_violations.txt` is byte-identical to the previous attempt's snapshot at `${RUNTIME_DIR}/resolver_fp_violations_prev.txt` (compared via `sort` then `cmp -s` so verifier output reordering is not a false-negative trigger), the attempt counter is promoted to `INTEGRATION_SYNC_RESOLVER_MAX_ATTEMPTS` so the existing exhaustion block runs immediately. Same `CONFLICT_RESOLVED=false` + `exit 1` shape as natural exhaustion, so downstream tooling (telegram alert, `ai:integration-judge-failed` transitions, immediate dispatch trap below) sees the same signal whether the loop bailed early or ran to MAX_ATTEMPTS. Restricted to integration-sync runs because only fingerprint violations carry stable per-pattern identity; residual-marker presence/absence is not a reliable progress signal. (2) **Step wall-clock cap + per-attempt cap** — the `Run Codex resolver, validate, stage, commit` step in `.github/workflows/review_autofix.yml` carries `timeout-minutes: 60`, sized so a single hung `codex exec` cannot eat the full job budget but a well-behaved 3-attempt run still completes (one observed worst-case attempt ran ~58 min). The step inherits the job's 180-min cap as the previous behaviour; the explicit step cap is the safety net. Inside the retry loop each `codex exec` invocation is additionally wrapped in `timeout --signal=TERM --kill-after=30s "${CONFLICT_RESOLVER_PER_ATTEMPT_TIMEOUT_SECS:=1080}"` (default 18 min) so a runaway first attempt cannot exhaust the full 60-min step budget before the retry loop iterates — without this cap, `INTEGRATION_SYNC_RESOLVER_MAX_ATTEMPTS=3` is structurally meaningless on hangs because attempt 1 alone can absorb the entire step cap on a large multi-file conflict set (observed on `bitsafe.io` PR #103 / run 25602538415, where attempt 1 was still issuing `apply_patch` calls at 59 min in before SIGKILL). 3 attempts x 18 min = 54 min fits inside the step cap with ~6 min headroom for soft validation, commit, and the EXIT-trap dispatch. The wrapper's non-zero exit on timeout flows through the existing `if ! codex …; then attempt=$((attempt+1)); continue; fi` branch in `scripts/review_conflict_resolve.sh` unchanged; the `--kill-after=30s` backstop covers codex children that ignore SIGTERM. Override `CONFLICT_RESOLVER_PER_ATTEMPT_TIMEOUT_SECS` (in seconds) for per-PR tuning when a particular conflict set legitimately needs longer attempts; values above the 1800s upper bound (30 min) are clamped down with a `::warning::` log because letting one attempt exceed half the 60-min step budget would starve the 3-attempt retry loop, defeating the point of the cap. (3) **Immediate orchestrator-poll dispatch** via an EXIT trap in `scripts/review_conflict_resolve.sh` (`_dispatch_integration_judge_now`) — on any non-zero exit from the resolver script when `IS_INTEGRATION_SYNC=true`, the script fires `gh workflow run ${ORCHESTRATE_POLL_WORKFLOW_FILE:-internal-orchestrate-poll.yml}` against the current repo so the orchestrator integration judge picks up on the next concurrency slot rather than waiting up to 5 min for the `*/5` cron tick. Dedup: skipped when a poller run is already `in_progress` or `queued` (the poller's `concurrency.group: ai-orchestrate-poll-${{ github.repository }}` + `cancel-in-progress: false` already serialises runs across the repo). Idempotent within a single script invocation. Fail-open: missing `GH_PAT`/`GITHUB_REPOSITORY`, an `gh` rate-limit, or an unknown workflow filename on a consumer repo logs `::warning::` and falls through; the cron tick remains the safety net so unattendedness is preserved. Exit-0 paths (resolver succeeded in deciding no commit was needed) intentionally do not fire — no escalation is warranted. Consumer repos that ship the orchestrator poller under a non-default filename can override via the `ORCHESTRATE_POLL_WORKFLOW_FILE` env var.
- **Outcome-aware retry reflexion.** When the per-attempt `timeout` wrapper SIGTERMs (`exit 124`) or SIGKILLs (`exit 137`) Codex before it produces any patch — or when Codex exits non-zero for any other reason (panic, transient network/auth, rate-limit) — the soft-validation gates never run, so the violation files (`resolver_marker_violations.txt`, `resolver_fp_violations.txt`) stay empty. Today's reflexion prelude in that state would tell the model "your previous attempt produced output that failed post-resolve validation. Files still containing unresolved Git conflict markers (0): (none). Fingerprint regressions (0): (none)." — actively misleading, since the previous attempt produced *nothing* (not "nothing-wrong"). `scripts/review_conflict_resolve.sh` now records the previous attempt's outcome (`timeout` / `error` / `ran`) in `_prev_attempt_outcome` and forwards it to `_build_retry_prompt`, which on `timeout` prepends a `*** TIMEOUT NOTICE ***` block above the prelude body explaining (a) why the violation lists are empty, (b) that the working tree is at the post-`git merge` state identical to attempt 1, and (c) actionable steps to spend the next per-attempt budget on `apply_patch` first rather than re-enumerating duplicate helpers across the file. On `error` the function prepends an analogous `*** PREVIOUS ATTEMPT EXITED NON-ZERO ***` block describing the no-output condition and naming the typical causes (codex panic, network/auth, rate-limit) so the next attempt does not over-correct against violations that never existed. **The misleading violations body itself is also structurally suppressed** on timeout/error: the prelude template wraps lines 3–50 of the standard "produced output that failed validation" framing in `{{#IF_VIOLATIONS}}`/`{{/IF_VIOLATIONS}}` markers, and the python renderer in `_build_retry_prompt` strips the entire region (markers + body) when `_prev_attempt_outcome` is `timeout` or `error`. The rendered prompt on those paths is just *header + outcome notice + ORIGINAL TASK*, with no leftover boilerplate to anchor the model on the wrong frame. This also belt-and-suspenders the truncation-fail case at the retry-loop site (`: > "${RESOLVER_*_VIOLATIONS_FILE}"` failing silently after a non-zero codex exit): the stale `MARKER_VIOLATION_COUNT` / `MARKER_VIOLATION_FILES` / `FINGERPRINT_VIOLATION_COUNT` / `FINGERPRINT_VIOLATION_DETAILS` substitutions happen *inside* the stripped region, so a truncation failure cannot poison the prelude. The retry banner echo (`Conflict resolver retry N/M: rebuilt reflexion prompt (prev markers=… , prev fingerprint_violations=…)`) is similarly defended by forcing both counts to `0` when outcome is `timeout`/`error` instead of reading the (possibly stale) violation files. Originated by runs `25627236793` (PR `tele-funtoken-msg-scoring#2874`) and `25627316961` (PR `#2867`) on the `orchestrator/project-2840` stack, where the resolver hung in extended-reasoning at `xhigh` on a file with duplicate function bodies outside the conflict markers and got SIGTERMed on all three attempts with `markers=0, fingerprint_violations=0` on every retry — the unambiguous signature of `timeout` firing pre-patch. The notice + suppression are integration-sync only (the prelude template is integration-sync only); non-integration runs continue to retry with the original prompt verbatim. The placeholder `{{PREVIOUS_OUTCOME_NOTICE}}` is empty when the previous attempt ran (whether output was empty or full), and the `{{#IF_VIOLATIONS}}` markers are stripped without removing their body, so the rendered prelude is byte-identical to the pre-PR shape on the post-validation retry path.

...
| `ENABLE_CLEAN_WAVE_JUDGE_SKIP` | No | `false` | orchestrate_poll | When `true`, the poller skips judge invocation for a cleanly completed non-final wave (`wave_complete=true`, no failed issues, not stuck, project not complete), increments `judge_cycle`, and mechanically advances `current_wave`. Deferred future-wave entries in `pending_issue_defs` do **not** block this skip path. |

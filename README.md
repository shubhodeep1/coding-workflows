...
9. **Review-blocked resolution:** When a PR exhausts its autofix iterations (`ai:review-blocked`), the poller invokes a credential-sanitized, read-only review-blocked judge with full bounded PR context. The judge emits only a validated advisory decision record. A bounded `fix` remains automated. Terminal `merge`, `merge_with_followup`, and `close_and_reissue` recommendations instead create a SHA- and digest-bound approval request; only an exact `/review-blocked-approve <request-id> <decision-digest>` PR comment from a trusted repository User can unlock the deterministic head/check/mergeability gates. Pending requests are reused without rerunning the model, stale-head and consumed requests are rejected, and replacement creation precedes source-PR closure.

...
- **Periodic default-branch security audit.** `.github/workflows/security-audit.yml` runs weekly at `08:00 UTC` and on `workflow_dispatch`, stays inert unless `SECURITY_AUDIT_ENABLED=true`, and uses SHA-pinned, out-of-tree copies of `scripts/security_audit.sh` and `prompts/mode-security-audit.txt` for both source-repository and consumer runs. The audited default-branch checkout is treated only as untrusted data; audit-owned Python, including prompt rendering and model-broker setup, runs in isolated mode with an allowlisted environment, and final findings packaging runs from the runner-owned audit runtime directory. Surviving findings are appended under dated sections on the stable `AI Security Audit Tracker` issue (`ai:security-audit`, marker `<!-- ai:security-audit-tracker:v1 -->`), and up to three weekly `ai:security` follow-up issues are opened for non-excluded findings that meet the confidence gate. `SECURITY_AUDIT_CRON` is documentation-only because GitHub Actions schedule cron is literal YAML, not repo-variable-driven.

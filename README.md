...
| `EDITOR_MAX_WALL` | No | `6600` | review_autofix, implement | Maximum wall-clock seconds per editor attempt (~110 min). Budget-aware: auto-capped to remaining job time minus a 2-min buffer. The 3-hour job cap allows ~2 full attempts; combined with the orchestrator's 60-minute per-issue scope target, this leaves headroom for one retry on watchdog kill. |
...
| `LOG_ANALYZER_MODEL` | No | `openai/gpt-5.4-mini` | test-and-mark-stable (Phase 8 soft-error analyser) | Lightweight model used by the release-gate post-run log analyser (`scripts/analyze_soft_errors.py`) to summarise soft failures (rate-limit recoveries, codex fallbacks, summariser hard-fails, editor no-ops) into the Telegram release notification. Non-blocking; analyser failures fall back to a stub report rather than failing the gate. |
...
| `EDITOR_MAX_WALL` | `6600` | Max wall-clock seconds (~110 min) per editor attempt; auto-capped to remaining job budget |

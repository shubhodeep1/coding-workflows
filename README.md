...
| `UNATTENDED_NAG_REMINDER_ENABLED` | No | `false` | review_autofix, orchestrate_poll | Opt-in wrapper-level `<reminder>` injection after repeated silent reviewer/editor/judge turns. Default off; when disabled the wrappers keep the existing retry behavior unchanged. |
| `UNATTENDED_NAG_SILENT_ROUNDS` | No | `3` | review_autofix, orchestrate_poll | Consecutive silent-turn threshold for nag-reminder injection in the review editor, review reviewers, and the main orchestrate-poll judge path. Must be an integer in `1..10`; invalid or out-of-range values clamp conservatively to `3`. |

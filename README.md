...
| `REVIEW_SOFT_DEADLINE_MINUTES` | `210` | Default-on soft run budget for review/autofix. The workflow exports `JOB_START_EPOCH` and `CODEX_RUN_BUDGET_*` from it, and `CODEX_HEARTBEAT` lines add `budget_elapsed_secs` / `budget_remaining_secs` when the contract is present. |

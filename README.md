...
| `ORCHESTRATOR_STATE_AUTH_KEYRING` | **Yes for orchestrator deployments** | orchestrate, orchestrate_poll, review_autofix conflict preparation, promote-main-to-stable cycle | Dedicated state and promotion-marker signing keyring JSON. Schema: `{"schema_version":"orchestrator_state_auth_keyring.v1","active_key_id":"<id>","keys":[{"key_id":"<id>","key_base64":"<base64>"}]}`. IDs are unique, 1–64 safe characters; 1–8 decoded keys of 32–64 bytes are accepted. New signatures use only the active key, while verification accepts retained previous keys. Keep the previous key until every active project and promote cycle has completed under the new active key. Missing or malformed configuration pauses state mutations, rejects comprehensive-cycle tracking bindings, and stops the daily promote cycle before marker dispatch. |

...
| `WRITE_GUARDS_ENABLED` | No | `true` | implement, review_autofix, validate | Enable the write-guard policy from `.github/ai/write_guards.v1.json` at the implement commit boundary, review-editor commit boundary, and validate fix/harness boundary. Set to `false` to bypass the guard; bypasses are logged with `WRITE_GUARD_BYPASS_ENV`. |

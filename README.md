...
| `TARGETED_FILE_CONTEXT_MAX_FILES` | No | `10` | implement, review_autofix, orchestrate_poll | Cap on the number of files MENTIONED in the targeted-context block built by `scripts/targeted_file_context.py` (inlined or marker-only count toward the cap). The inlining mechanism pre-loads files named in the plan (implement) / `LAST_RUN_CHANGED_FILES_FILE` (autofix) / conflicted-file allowlist (resolver) so the model's first turn is a write rather than a recon read. Files past this cap are NOT silently dropped — they are listed at the tail under a `Deferred (max_files=N reached, read with read tool if needed)` summary so the model still sees the full set. Set to `0` to disable inlining entirely (block becomes a single-line "(disabled)" marker). |
| `TARGETED_FILE_CONTEXT_MAX_BYTES` | No | `102400` (100 KB ≈ 25k tokens) | implement, review_autofix, orchestrate_poll | Total byte budget across all inlined files in a single prompt. A file that would push the cumulative byte count over this cap is NOT head-truncated — it gets a `(NN bytes; would overflow total budget — read with read tool, max_bytes=…, used=…)` marker so the model uses its native targeted-read flow rather than being misled by a truncated head. Lower this on cost-sensitive repos; raise on small-file repos to fit more verbatim content. |

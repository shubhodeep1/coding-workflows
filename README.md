...
| `XPOLL_SUMMARISER_CALL_TIMEOUT_SECS` | No | `1200` | review_autofix | Per-attempt wall-time timeout for a single codex-cli summariser invocation. On timeout / non-zero exit / empty stdout the summariser retries up to 3 times, then hard-fails the workflow (the job-level "Telegram failure" step surfaces the incident). |
...
- Supported template families currently include `python-mongo-flask`, `node-hardhat-solidity`, and `repo-local-tooling`. Use `repo-local-tooling` for workflow/tooling repositories that should run local renderer/self-test checks instead of booting a web service.

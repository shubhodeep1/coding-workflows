...
| `AUTO_RELEASE_STABLE_ENABLED` | No | `true` | auto-release-stable | Kill switch for `.github/workflows/auto-release-stable.yml`, which every 6 hours dispatches `test-and-mark-stable.yml` on `stable` when the branch tip differs from the `stable` tag commit. `false` logs `AUTO_RELEASE_SKIPPED reason=disabled`. |
...
12f. **Promote cycle callbacks (poller-owned):** Tracking issues labeled `ai:comprehensive-test-pending` belong to the daily promote cycle started by `.github/workflows/promote-main-to-stable.yml` (see Contributing). The label and a marker comment (`apply-analysis-source-doc`, `apply-analysis-role`, `apply-analysis-cycle-baseline-sha`, `apply-analysis-smoke-sha`, `apply-analysis-promote-sha`, `apply-analysis-proving-merge-sha`) are applied by `orchestrate.yml` at tracking-issue creation through its optional `tracking_labels` / `tracking_comment` inputs, so the poller never guesses which project is its own. Two hooks: (1) when a `proving` run completes, `handle_comprehensive_release_callback_if_needed` runs `scripts/apply_analysis_on_main.sh` with role `verifying`, recording `promote_sha` = the default branch tip (which includes the proving merge) and the proving merge commit; the verifying run's own completion does nothing further. (2) When a `verifying` run's final PR is ready to merge, `comprehensive_promotion_gate_before_final_merge` runs first: every commit between the smoke-tested commit and `promote_sha` must be the proving merge, a forward-merge PR merge, or a `COMPREHENSIVE_CYCLE_BOT_LOGINS` commit touching only non-code paths, else the promotion is deferred and the merge proceeds (`COMPREHENSIVE_PROMOTION_DEFERRED`). Clean, it dispatches `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` (default `promote-main-to-stable.yml` on `COMPREHENSIVE_RELEASE_WORKFLOW_REF`, default `main`) with `target_sha=promote_sha` and the full gate, and holds the final merge (state `comprehensive_promotion.status=dispatched`, `FINAL_MERGE_BUDGET_ELIGIBLE=0`) until the release run completes: success promotes and releases the hold, failure or `COMPREHENSIVE_PROMOTION_HOLD_MAX_SECS` releases the hold without promoting, with a CRITICAL alert. A labelled tracking issue without a marker keeps the legacy behaviour: dispatch `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` on completion with `dry_run=false` and any validated `version_tag`/`test_repo` from tracking comments. `.github/workflows/comprehensive-test-and-release.yml` does not apply the label.
...
- [`auto-release-stable.yml`](.github/workflows/auto-release-stable.yml) runs every 6 hours and
  dispatches `test-and-mark-stable.yml` on `stable` when the branch tip differs from the `stable`
  tag commit, so a patch merged straight into `stable` reaches consumers without a manual
  dispatch. It skips while a gate run is queued or running and after the gate failed on the

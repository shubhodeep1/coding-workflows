...
| `REVIEW_RUN_MAX_RUNTIME_MINUTES` | No | `250` | orchestrate_poll | Freshness window the in-flight / zombie guards (`build_active_issue_set`, the `retrigger_review` inline guard, and `_direct_inflight_review_run_on_branch`) apply to **review_autofix** runs, which legitimately run past `STALL_THRESHOLD_MINUTES` up to the codex-agent job's 240-min timeout. A review still editing within this window is treated as active (not a zombie) so stall recovery does not clobber it with an empty commit. Floored at `STALL_THRESHOLD_MINUTES` in-script. |
...
| `REVIEW_RUN_MAX_RUNTIME_MINUTES` | `250` | Freshness window the in-flight/zombie guards apply to review_autofix runs (they run past the stall threshold up to the 240-min job timeout); floored at `STALL_THRESHOLD_MINUTES` |

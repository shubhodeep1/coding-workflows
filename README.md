...
9. **Validation gate:** When the judge says "complete" and `ENABLE_VALIDATION=true`, the poller dispatches `ai-validate.yml`, marks the tracking issue `ai:validating`, and only closes after `ai:validated` is reported. Validation failures or validation dispatch failures are terminal and move the tracking issue to `ai:validation-failed`. The poller also fails fast if validation is in `ai:validation-fixing` but no active fix-up issue numbers are tracked, preventing indefinite polling deadlocks.

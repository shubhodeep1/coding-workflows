...
- **Credentialless model boundary.** Planner, implementation diagnosis, reviewer/editor, resolver, poller judge, and security-audit model processes run through `scripts/untrusted_process_sandbox.sh`. The sandbox exposes only a loopback provider proxy, hides Git metadata, strips GitHub/runner credentials, and keeps read-only roles from writing the checkout. The proxy accepts only trusted configured models and defaults to 64 requests, one concurrent request, 65,536 output tokens, and a conservative USD 25 cumulative spend reservation; missing pricing or terminal usage accounting fails closed. Implement helpers are copied and SHA-256-bound outside the worktree before model execution, and authenticated publication verifies that immutable set. Model-produced edits cross `scripts/trusted_git_write.sh`, which disables hooks and validates exact local and remote heads.

- **Causal security waivers.** Python findings receive additive `security_audit_causal_scope.v1` metadata from `scripts/security_audit_causality.py`. The same fail-closed validator is used by the audit engine and poller. Fingerprints include module-level registrations, assignments, imports, decorators, callable references, and bounded reverse callers; changed routing/configuration in other languages invalidates a waiver unless static analysis proves the changed path non-causal.

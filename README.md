...
- [`auto-release-stable.yml`](.github/workflows/auto-release-stable.yml) runs every 6 hours and
  ...
  workflow shares the `promote-main-to-stable` concurrency group so it never races the promote job. It skips while a gate run is queued or running and after the gate failed on the

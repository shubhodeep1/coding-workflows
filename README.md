...
`scripts/security_audit.sh` defaults to `SECURITY_AUDIT_OUTPUT_MODE=issues`, which is the only mode invoked by the current workflows. The inert `findings-json` mode is an internal caller interface for a later orchestrator phase; no workflow or poller invokes it yet. Internal callers may pass one positional argument containing the path to a readable project-specification file. That content is appended to the model prompt between `UNTRUSTED PROJECT SPECIFICATION` delimiters and is valid only in `findings-json` mode.

...
- **Outcome-aware retry reflexion.** When the per-attempt `timeout` wrapper SIGTERMs (`exit 124`) or SIGKILLs (`exit 137`) Codex before it produces any patch — or when Codex exits non-zero for any other reason (panic, transient network/auth, rate-limit) — the soft-validation gates never run, so the violation files (`resolver_marker_violations.txt`, `resolver_fp_violations.txt`) stay empty. Today's reflexion prelude in that state would tell the model "your previous attempt produced output that failed post-resolve validation. Files still containing unresolved Git conflict markers (0): (none). Fingerprint regressions (0): (none)." — actively misleading, since the previous attempt produced *nothing* (not "nothing-wrong"). `scripts/review_conflict_resolve.sh` now records the previous attempt's outcome (`timeout` / `error` / `ran`) in `_prev_attempt_outcome` and forwards it to `_build_retry_prompt`, which on `timeout` prepends a `*** TIMEOUT NOTICE ***` block above the existing prelude body explaining (a) why the violation lists are empty, (b) that the working tree is at the post-`git merge` state identical to attempt 1, and (c) actionable steps to spend the next per-attempt budget on `apply_patch` first rather than re-enumerating duplicate helpers across the file. On `error` the function prepends a shorter `*** PREVIOUS ATTEMPT EXITED NON-ZERO ***` block describing the no-output condition and naming the typical causes (codex panic, network/auth, rate-limit) so the next attempt does not over-correct against violations that never existed. The non-zero-exit branch in the retry loop additionally truncates the violation files before `continue`, so a stale `_marker_count` / `_fp_count` from an earlier successful exec cannot leak into the next iteration's prelude and contradict the notice's "(none)" claim. Originated by runs `25627236793` (PR `tele-funtoken-msg-scoring#2874`) and `25627316961` (PR `#2867`) on the `orchestrator/project-2840` stack, where the resolver hung in extended-reasoning at `xhigh` on a file with duplicate function bodies outside the conflict markers and got SIGTERMed on all three attempts with `markers=0, fingerprint_violations=0` on every retry — the unambiguous signature of `timeout` firing pre-patch. The notice is integration-sync only (the prelude template is integration-sync only); non-integration runs continue to retry with the original prompt verbatim. The placeholder `{{PREVIOUS_OUTCOME_NOTICE}}` in `prompts/integration-sync-conflict-resolver-retry-prelude.txt` is empty when the previous attempt ran (whether output was empty or full), so the existing prelude is unchanged on the post-validation retry path.

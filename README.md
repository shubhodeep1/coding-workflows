...
| `REVIEWER_SLOT_RETRYABLE_FAILURE_LIMIT` | `3` | Default-on slot-local cap for retryable reviewer recovery. The cheaper-reasoning retry still runs first when enabled, then any mapped same-family failback model can consume the remaining bounded attempts; once the slot limit is exhausted, the slot stops retrying instead of burning the whole soft deadline. |

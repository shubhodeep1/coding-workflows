...
- Model-facing issue analysis, orchestrator polling, implementation and repair, validation, check-failure triage, security audits, source and consumer workflow retros, review consolidation, review editing, review-blocked judging/fixing, workflow-log analysis, and merge-conflict resolution use `scripts/model_provider_broker.py`, a bounded loopback proxy that retains the real OpenRouter credential. Model children receive only an ephemeral per-run broker token in a clean environment; read-only phases use Codex's `read-only` sandbox, writer phases retain only their required workspace permissions, and the conflict writer runs as an unprivileged user with `.git` inaccessible. The broker authorizes only the phase's configured model IDs, normalizes endpoint-specific output limits, and injects server-controlled provider price ceilings. `MODEL_PROVIDER_BROKER_MAX_REQUESTS` defaults to `100` (valid range `1..100`), `MODEL_PROVIDER_BROKER_MAX_OUTPUT_TOKENS` defaults to `16384`, and `MODEL_PROVIDER_BROKER_MAX_TOTAL_OUTPUT_TOKENS` defaults to `1638400` (`100 x 16384`, so the total never bites below what the request cap already permits); either exhausted budget returns HTTP 429 and the phase fails closed. The total budget is charged against provider-reported usage: each request reserves its per-request ceiling only while in flight and is settled to the `usage` output-token count when its response completes (a usage-less or cut-off response keeps the full reservation, an upstream error settles at zero), so a multi-turn agent session is bounded by tokens actually generated rather than by `total / per-request` turns. There is no direct-provider fallback when broker startup fails.
- Resolver retry authority is stored only in producer-authenticated `AUTOFIX_RESOLVER_RETRY_STATE_V2` comments bound to repository, project branch, PR, and current head SHA. Legacy V1 PR-body blocks are ignored as authority. The poller selects the highest verified generation from a complete scan bounded to 10 pages and 32 MiB; if a short page cannot prove completeness within either bound, mutation is deferred instead of using partial history. The trusted actuator reuses the review pipeline's collected comment snapshot rather than issuing a second unbounded fetch. GitHub labels, comments, and force-tick dispatches run afterward in `scripts/review_conflict_actuate.sh`, which revalidates the live head before signing or acting.

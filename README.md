...
| `AUTOFIX_BOT_LOGIN` | No | `codex` | review_autofix | GitHub login that the gate job accepts as the authoritative bot identity for the self-triggered autofix skip and terminal same-head marker comments. Compared against `.author.login` / `.committer.login` on the HEAD commit API response and `.user.login` on marker comments; these are GitHub-attributed identities, not user-controlled git metadata or comment text. Override if you run the workflow under a fork of codex that pushes and comments as a different bot account (e.g. `codex-bot`, `my-org-codex`). Unset or empty falls back to the default `codex` (via shell `${AUTOFIX_BOT_LOGIN:-codex}` expansion) — to enable the self-triggered skip, set `AUTOFIX_SKIP_SELF_TRIGGERED=true` instead. |
| `AUTOFIX_SKIP_TERMINAL_SAME_HEAD` | No | `true` | review_autofix | Skip a `workflow_dispatch` review run (sweep cron, poller re-dispatch, manual dispatch) when the PR's current head SHA already carries a terminal `<!-- REVIEW_AUTOFIX_PARTIAL_V1 -->` marker (`resume_should_continue=false`, i.e. `no_progress` or `round_budget_exhausted`) authored by `AUTOFIX_BOT_LOGIN`, and GitHub reports the PR `mergeable=true`. The gate emits `AUTOFIX_GATE_SKIP reason=terminal_same_head ...` and the codex-agent job never starts. Markers from other authors are ignored. `[force-review]` in the title, the `force-review` label, a `force_rb_judge` dispatch, a conflicted/unknown-mergeability PR, and `pull_request` events all bypass the skip; API or parse failures log `AUTOFIX_GATE_TERMINAL_SAME_HEAD_QUERY_FAILED` and run normally. Set to `false` to restore unconditional re-dispatch. |
...
> (default `true`) the `gate` job now reads the newest marker authored by
> `AUTOFIX_BOT_LOGIN` for the PR's current head on `workflow_dispatch` events and sets `should_run=false`
...
> newer trusted non-terminal marker for the same head reopens it, markers from
> other authors are ignored, and a comments API
...
> The head SHA rides on the gate's existing `/pulls/{n}` fetch; the only new
> API call is one paginated `/issues/{n}/comments` read on the dispatch path.
>

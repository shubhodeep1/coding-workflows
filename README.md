...
12f. **Promote cycle callbacks (poller-owned):** Tracking issues labeled `ai:comprehensive-test-pending` belong to the daily promote cycle started by `.github/workflows/promote-main-to-stable.yml`. `scripts/apply_analysis_on_main.sh` signs a bounded `comprehensive_cycle_marker.v1` envelope with `ORCHESTRATOR_STATE_AUTH_KEYRING`, binding the source document, role, originating dispatcher run, smoke run and actor IDs, expected smoke inputs/title/workflow/event/conclusion/head, promotion SHA, and proving merge SHA. Before `orchestrate.yml` reposts that marker with `github.token`, it verifies the envelope from immutable support and requires its signed smoke actor to match the authenticated workflow dispatcher; a collaborator cannot replay a copied envelope through the bot identity. All readers then require immutable GitHub Actions bot user ID `41898282` plus a valid signature. Before promotion the poller re-fetches the signed smoke run and verifies the API-reported workflow path, event, display title/input correlation, actor, conclusion, and head SHA. Unsigned, tampered, wrong-producer, stale, or API-mismatched evidence keeps the cycle held and cannot promote. The proving/verifying dispatch, quiescence gate, pinned promotion, and bounded release hold otherwise retain their existing behavior.

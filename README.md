...
| `MAX_SECURITY_PASS_CYCLES` | No | `3` | orchestrate_poll | Maximum completed consolidated security-fix cycles before persistent findings terminalize the project as `ai:security-pass-failed`. |
...
| `ORCH_SYNC_CONTRACT_LIST_UNION_ENABLED` | No | `true` | orchestrate_poll | Enable the deterministic pre-resolver for pure two-sided `read_entrypoints` / `write_entrypoints` list appends in `db/contracts/*.yml` and `*.yaml`. Each input is capped at 1 MiB; entrypoint lists are limited to 4,096 scalar strings of at most 4,096 characters each; and the helper runs under a fixed 10-second timeout. Eligible conflicts are validated locally, committed, and pushed without a review/autofix resolver dispatch; unsupported, oversized, non-scalar, or timed-out inputs fail open to the existing conflict path. Set to `false` to restore the legacy resolver flow exactly. |
...
| `MAX_SECURITY_PASS_CYCLES` | `3` | Maximum completed consolidated security-fix cycles before terminal `ai:security-pass-failed`. |
...
12e. **Project security pass (default on):** Every real or synthesized judge completion, externally merged final-PR recovery, merge-conflict completion, and validation-complete finalization first audits `merge-base(default, integration)..integration-head` through the read-only findings-JSON security engine. A clean result is valid only for the exact recorded integration SHA. Any finding creates one consolidated `ai:orchestrator-managed` fix issue that re-enters clarify → plan → implement → review; after it merges, the poller invalidates the old SHA and re-audits. Persistent findings after `MAX_SECURITY_PASS_CYCLES` (default `3`) set `ai:security-pass-failed`; `/re-security-pass` resets the bounded loop. If orchestrator stall recovery closes the fix issue and re-issues it, the poller adopts the successor (matched on the tracking-issue and `security-pass-fix-cycle-<K>` body markers, logged as `SECURITY_PASS_FIX_ISSUE_SUCCESSOR_ADOPTED`) and keeps waiting instead of failing the project; a closed fix issue with no successor still terminalizes, and an inconclusive successor lookup retries on the next tick. Missing or malformed engine support fails closed with `SECURITY_PASS_FAILED` and a CRITICAL alert. Setting `ENABLE_SECURITY_PASS=false` is the immediate operator kill switch and preserves the legacy completion path.
...
12c. **Integration self-healing:** If a periodic `main` → integration-branch sync returns HTTP 409 (real conflict), the poller routes recovery through `heal_integration_branch_conflict`: it (a) ensures/creates the final integration→default PR (eagerly, if it does not yet exist), (b) dispatches the review/autofix workflow through `_dispatch_review_for_conflicts` against that PR to run the existing Codex conflict resolver on a clean runner, and (c) records the attempt in new tracking-state fields (`integration_sync_status`, `integration_sync_last_error`, `integration_conflict_dispatch_count`, `integration_conflict_dispatch_ts`, `integration_conflict_unresolved_ticks`). Dispatches are throttled by `CONFLICT_DISPATCH_COOLDOWN_SECS` (default 900s). The retry budget is **branch-aware**: head refs matching `orchestrator/project-*` honour `INTEGRATION_SYNC_CONFLICT_MAX_RETRIES` (default `1` — single resolver shot, then judge), while non-orchestrator integration branches honour `INTEGRATION_CONFLICT_MAX_RETRIES` (default `3`). After the effective budget is exhausted the orchestrator escalates by invoking the judge with full PR context via `codex exec`. Retryable judge/model/API failures preserve the conflict state and consume the cumulative `INTEGRATION_CONFLICT_LIFETIME_MAX` budget instead of terminalizing the project after one transient failure; invalid trusted inputs still fail closed. The same healing flow is triggered from `finalize_integration_merge_if_needed` whenever the final PR is observed with `mergeable=false`, so the project no longer halts on first conflict. Before dispatching the resolver, `sync_contract_list_union_merge` deterministically keeps both sides of eligible contract entrypoint-list appends and records `sync.last_sync_outcome = merged-deterministic` after the validated merge is pushed. Both sync boundaries require the target to equal `orchestrator/project-<tracking issue>`; bounded scalar-list validation and a fixed helper timeout send unsafe inputs to the existing resolver without a deterministic push.
...
- On exhaustion the per-PR review-blocked judge runs and decides `merge` (auto-merge into the integration branch), `fix` (the trusted actuator posts the bounded guidance and pushes a no-tree-change `[judge-fix]` commit only when the live head still matches the judged SHA, then redispatches review; this resets the autofix counter and lets the editor run, capped at `MAX_REVIEW_BLOCKED_RETRIES`, default `2`), `merge_with_followup` (auto-merge AND open a follow-up issue tracking a deferred-but-non-blocking gap — preferred over `close_and_reissue` at IS_FINAL when the PR is shippable; follow-up issue inherits `ai:orchestrator-managed` from the parent), or `close_and_reissue` (close the sub-issue PR and create a refined issue). Terminal actions additionally require an exact approval from a current `maintain` or `admin` repository role. Existing successors are reused only when their immutable creator ID, required labels, action/type, approved head, and title/body digests match a producer-authenticated `REVIEW_BLOCKED_SUCCESSOR_V1` intent. Legacy refusal markers remain visible, but only a matching keyring-signed `REVIEW_BLOCKED_DECISION_REFUSED_V2` envelope suppresses another judge attempt.
...
| `MAX_SECURITY_PASS_CYCLES` | `3` | Maximum completed consolidated fix cycles before persistent security findings force terminal `ai:security-pass-failed`. Must be a positive integer; invalid values fall back to `3`. |

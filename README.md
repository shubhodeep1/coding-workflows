...
**Staged-support restore (this repository only).** The "Stage workflow support files" step of `.github/workflows/implement.yml` installs `SCRIPT_REF`'s copies of the runtime helpers (`scripts/*.sh`, `scripts/*.py`, `prompts/*`, `ai-memory/schemas/*`) into the checkout before Codex runs. In consumer repos those are runtime artifacts that `scripts/.gitignore` and the commit-time pathspec exclusions keep out of the commit; in `shubhodeep1/coding-workflows` they are tracked files, `SCRIPT_REF` is `github.sha` (the default branch), and the self-repo commit path has no `scripts/` exclusion. When the issue's branch is an orchestrator integration branch that diverged from `main`, every implementation commit therefore carried `main`'s copies and silently reverted the branch's own edits to those helpers: implement run 34613019339 committed eight such files into PR #4079 (1,117 deletions), dropping the security-pass fixes from PRs #4029, #4052, #4057 and #4071, and the PR's own review editor then died with `model_provider_broker_start: command not found`; run 34392788763 (PR #4071) did the same and spent eight `[ai-autofix]` rounds restoring them. The staging step now snapshots `git status --porcelain --untracked-files=no` before and after its installs and records every tracked file it overwrote in `STAGED_SUPPORT_LEDGER` (`${RUNTIME_DIR}/staged_support_overwrites.txt`), with the installed content under `STAGED_SUPPORT_BASE_DIR`, logging `IMPLEMENT_STAGED_SUPPORT_LEDGER ref=<sha> overwritten_tracked_files=<n>`. `scripts/implement_commit_changes.sh` reads the ledger before it stages anything: a file whose content still equals the installed copy is put back to `HEAD` (`IMPLEMENT_STAGED_SUPPORT_RESTORED path=…`); a file the editor edited is re-based onto the branch's version with a 3-way `git merge-file` against the installed copy (`IMPLEMENT_STAGED_SUPPORT_REBASED path=…`); a file the editor deleted keeps its deletion; and a 3-way conflict fails the commit step closed (`IMPLEMENT_STAGED_SUPPORT_REBASE_CONFLICT path=… conflicts=<n>`, `staged_support_rebase_conflict=true` plus `staged_support_rebase_conflict_files` in the step outputs) rather than commit a `main`-based copy that drops the branch's edits. The summary line `IMPLEMENT_STAGED_SUPPORT_RESTORE restored=<n> rebased=<n> conflicts=<n>` is emitted on every self-repo run; consumer repos never set the ledger and are unaffected. The helpers themselves still execute from the `SCRIPT_REF` copies for the rest of the job, so `main`-side fixes to the implement tooling keep applying to wedged integration branches; only what gets committed changes.

...
| `SECURITY_PASS_LINE_OWNERSHIP` | No | `project-lines` | orchestrate_poll | Route only blame- and causal-diff-proven base-owned findings to advisories; deleted guards and ambiguity remain blocking. Set to `file` for per-file blocking. |
| `SECURITY_PASS_OWNERSHIP_CONTEXT_LINES` | No | `3` | orchestrate_poll | Number of surrounding lines (0-50) included in project-line ownership classification. Invalid values fall back to `3`. |

...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | No | `false` | orchestrate_poll | Controls whether stall-judge decisions are allowed to execute `escalate_human`. When `false` (default), any stall-judge `escalate_human` decision is converted to the same non-human declarative fallback action for the issue phase/recovery count. |

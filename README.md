...
| `SECURITY_AUDIT_ENABLED` | No | `false` | security-audit (source repo only) | Opt-in gate for `.github/workflows/security-audit.yml` / `scripts/security_audit.sh`. When `true`, the weekly/manual default-branch audit renders `prompts/mode-security-audit.txt`, appends a dated section to the stable `AI Security Audit Tracker` issue (`ai:security-audit`), and opens up to three weekly `ai:security` follow-up issues after confidence-gate and false-positive-exclusion filtering. When `false`, the workflow exits cleanly before Codex or GitHub side effects. |

...
| `IMPLEMENT_PROMPT_CODEX_STDIN_CAP_BYTES` | No | `1048576` | implement | Character cap that codex-cli enforces on `turn/start` stdin. `implement.yml` logs the assembled prompt size after building `${CODEX_PROMPT_FILE}` and emits a `::warning::` when the prompt is at or over this value. Diagnostic only — it never truncates the prompt and never fails the step. It exists because overshooting the cap fails every attempt before the editor runs, with only an opaque `Input exceeds the maximum length of 1048576 characters` on stderr, which the retry loop then misreads as an ordinary no-actionable-output bail. A non-numeric or non-positive value falls back to the default. Mirrors `REVIEWER_PROMPT_CODEX_STDIN_CAP_BYTES` in `scripts/review_run_reviewers.sh`. |

...
| `WORKFLOW_EDITOR_FALLBACK_MODEL` | No | `openai/gpt-5.4` | plan, implement, review_autofix (editor) | Capacity-fallback editor model. Each codex editor retry loop switches `--model` to this slug on its **final** attempt only, so a sustained OpenRouter/OpenAI per-model TPM saturation on the primary (`WORKFLOW_EDITOR_MODEL`, `gpt-5.5`) can be ridden out — the fallback sits in a different capacity bucket (issue #3515 / run 28640359211). The default `openai/gpt-5.4` is declared in `scripts/codex_model_catalog.json` (fields mirror `gpt-5.5`), so `apply_patch`/verbosity resolution stays intact when the switch fires. Set to the primary slug (or clear it) to disable the switch; point at any catalog-declared slug to change the fallback. The implement **repair** pass and the parallel reviewer models are unaffected. |
...
| `REVIEW_CONSOLIDATOR_MODEL` | No | `openai/gpt-5.5` | review_autofix | Model used by the consolidator stage. |
...
- **Diagnosis:** for same-repo PRs, the repo is checked out at the failing head
  ...
  `openai/gpt-5.5`, `xhigh`) reads the failing check's logs (via
...
|---|---|---|
| `WORKFLOW_EDITOR_MODEL` | `openai/gpt-5.5` (every phase) | Model for code editing / reasoning tasks. The previous legacy editor split (patch-heavy phases on a separate older slug) was retired after openai/codex#11151. See main table above. |
...
| `REVIEW_CONSOLIDATOR_MODEL` | `openai/gpt-5.5` | Model slug used for the consolidator stage in `review_autofix.yml`. |
...
| `WORKFLOW_CHECK_TRIAGE_MODEL` | `WORKFLOW_EDITOR_MODEL` (`openai/gpt-5.5`) | Diagnosis model for check-failure triage. |
...
| `VALIDATION_DISCOVERY_MODEL` | `openai/gpt-5.5` | Codex model id. |

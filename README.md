...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | No | `false` | orchestrate_poll | Compatibility gate for declarative stall ladders: when `false`, human terminalization (`escalate_human` / `ai:needs-human`) is suppressed and the poller repeats the last non-human action; when `true`, legacy human-escalation terminalization is allowed. |
...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | `false` | Compatibility gate for declarative stall ladders: allow (`true`) or suppress (`false`) `escalate_human` terminalization |

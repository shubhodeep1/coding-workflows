...
| `ENABLE_SECURITY_PASS` | No | `false` | orchestrate_poll | Default-off dark-launch gate for the mandatory current-integration-head security pass before validation or finalization. When enabled, every completion route must obtain a clean SHA-bound pass. |
...
| `ENABLE_SECURITY_PASS` | `false` | Enable the scheduled poller's mandatory current-head project security pass before validation or finalization. |
...
`scripts/security_audit.sh` defaults to `SECURITY_AUDIT_OUTPUT_MODE=issues`, preserving the periodic audit workflow. The scheduled orchestrator poller invokes `findings-json` only when `ENABLE_SECURITY_PASS=true`; the default-off path remains inert. Internal callers may pass one positional argument containing the path to a readable project-specification file. That content is appended to the model prompt between `UNTRUSTED PROJECT SPECIFICATION` delimiters and is valid only in `findings-json` mode.
...
15. **Completion:** When validation and the default-off security pass are disabled, completion remains judge-driven and immediate. Enabling `ENABLE_SECURITY_PASS` inserts the SHA-bound security gate before this route.
...
| `ENABLE_SECURITY_PASS` | `false` | Truthy values enable the mandatory current-integration-head security pass before validation or finalization. Any other value preserves the legacy completion route. |

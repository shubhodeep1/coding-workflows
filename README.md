...
| `WORKFLOW_EDITOR_FALLBACK_MODEL` | No | `openai/gpt-5.5` | plan, implement, review_autofix (editor) | Capacity-fallback editor model. Each codex editor retry loop switches `--model` to this slug on its **final** attempt only, so a sustained OpenRouter/OpenAI per-model TPM saturation on the primary (`WORKFLOW_EDITOR_MODEL`, `gpt-5.4`) can be ridden out — the fallback sits in a different capacity bucket (issue #3515 / run 28640359211). The default `openai/gpt-5.5` is declared in `scripts/codex_model_catalog.json` (fields mirror `gpt-5.4`), so `apply_patch`/verbosity resolution stays intact when the switch fires. Set to the primary slug (or clear it) to disable the switch; point at any catalog-declared slug to change the fallback. The implement **repair** pass and the parallel reviewer models are unaffected. |

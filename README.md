...
| `security-audit.yml` dispatch inputs `ref` / `bypass_weekly_cap` | No | `""` / `false` | security-audit (source repo + consumers via `ai-security-audit.yml`) | Workflow inputs, not repo vars; scheduled runs leave both unset. `ref` audits a branch instead of the default branch: the workflow sets `SECURITY_AUDIT_TARGET_REF` and an explicit `SECURITY_AUDIT_DIFF_BASE`..`SECURITY_AUDIT_DIFF_HEAD` range (the branch's merge-base with the default branch .. its head), follow-up issues carry an `Integration branch: <ref>` line so `scripts/resolve_integration_ref.sh` routes their fix PRs onto that branch, and the tracker's default-branch last-audited-commit marker is not moved. `bypass_weekly_cap` (`SECURITY_AUDIT_BYPASS_WEEKLY_CAP`) opens a follow-up for every surviving finding instead of stopping at 3 per UTC week. `/implement-plan-claude` sets both. The run log's summary line reports `deferred_by_weekly_cap=<n>`. |

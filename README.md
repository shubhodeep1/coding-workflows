...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | No | `false` | orchestrate_poll | Opt-in gate for terminal `escalate_human` stall actions. Default `false` preserves legacy autonomous recovery by mapping terminal human escalations to deterministic non-human declarative fallback actions for the same phase/recovery count. |
...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | `false` | Opt-in gate for terminal `escalate_human` stall actions; default keeps legacy non-human fallback behavior |

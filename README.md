...
| `MAX_SECURITY_PASS_CYCLES` | No | `3` | orchestrate_poll | Maximum completed consolidated security-fix cycles before persistent findings terminalize the project as `ai:security-pass-failed`. The counter resets to `0` when a recorded clean pass is invalidated by an advancing integration head, so findings in newly-synced code get their own budget. |
...
9. **Review-blocked resolution:** When a PR exhausts its autofix iterations (`ai:review-blocked`), the poller invokes a credential-sanitized, read-only review-blocked judge with full bounded PR context. The judge emits only a validated decision record. It can: (a) merge the PR as-is if remaining issues are cosmetic or low-risk, (b) request `fix` with a bounded description that the trusted shell publishes before pushing a SHA-bound no-tree-change `[judge-fix]` reset and redispatching the existing review/autofix workflow, (c) close the PR and create a replacement issue with refined guidance, or (d) `merge_with_followup` — merge as-is and open a follow-up issue tracking a deferred-but-non-blocking gap. A valid `fix` decision after the source PR has merged is normalized to `merge_with_followup`; after `MAX_REVIEW_BLOCKED_RETRIES` (default 2), it is normalized to `close_and_reissue` so model noncompliance cannot restart the review loop.
...
| `MAX_SECURITY_PASS_CYCLES` | `3` | Maximum completed consolidated fix cycles before persistent security findings force terminal `ai:security-pass-failed`. Must be a positive integer; invalid values fall back to `3`. Resets to `0` when an advancing integration head invalidates a recorded clean pass. |

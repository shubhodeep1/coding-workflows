...
| `CHECK_RUNS_POLL_INTERVAL_SECS` | No | `20` | review_autofix | Sleep interval between check-run status polls in the wait loop. Integer in `5..300`; invalid values clamp to `20`. Each poll iteration runs one `gh_retry gh api --paginate --slurp "/repos/{repo}/commits/{sha}/check-runs?per_page=100"` call (≥1 underlying GitHub API requests once pagination + `gh_retry` are accounted for). |

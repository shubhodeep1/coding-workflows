...
| `CLAUDE_FIXER_ENABLED` | No | `true` | review_autofix | Claude-fixer mode for `/implement-plan-claude` PRs (head ref `claude/implement-plan-*`). The reviewer panel runs as for every PR, but the GPT editor, the GPT conflict resolver, the push / re-trigger tail, and the review-blocked judge do not: the "Hand review round to Claude session (Claude-fixer mode)" step posts the consensus ledger (or the pre-review conflict) with an `<!-- ai:claude-fixer-handoff:v1 kind=<findings\|conflict> head=<sha> round=<n> -->` comment, and a fresh `/implement-plan-claude` stage session fixes the findings in one `[claude-autofix]` commit per round (counted toward `MAX_AUTOFIX_ITERATIONS` like `[ai-autofix]`) or rejects them with an `ai:claude-fixer-verdict:v1` comment and dispatches the workflow with `claude_fixer_converged_head=<sha>`; the gate verifies the hand-off and the verdict for that exact head and the `claude-fixer-auto-merge` job enables auto-merge. A round with zero ledger entries and no failing check auto-merges in the run, as an editor no-op does. At the cap the PR itself is labelled `ai:review-blocked`. Dispatch re-runs (sweep, continuation) on a head that already has a hand-off are skipped (`claude_fixer_awaiting_session`). Set to `false` to send these PRs through the GPT editor path again. |
...
| `SECURITY_PASS_LINE_OWNERSHIP` | No | `project-lines` | orchestrate_poll | Route only blame- and causal-diff-proven base-owned findings to advisories; deleted guards and ambiguity remain blocking. Set to `file` for per-file blocking. Changing the mode invalidates a cached pass and forces a full-range audit; automatic line-ownership waivers do not suppress findings in `file` mode. |
| `SECURITY_PASS_OWNERSHIP_CONTEXT_LINES` | No | `3` | orchestrate_poll | Number of surrounding lines (0-50) included in project-line ownership classification. Invalid values fall back to `3`; changing the effective value forces a full-range audit, even at an unchanged head. |
| `SECURITY_PASS_ADVISORY_FOLLOWUP_CAP` | No | `5` | orchestrate_poll | Deprecated compatibility input; ignored. All eligible pre-existing advisories are attempted each tick, including when set to `0`; failed rows remain queued for retry. |
...
| `TARGETED_FILE_CONTEXT_MAX_BYTES` | No | `102400` (100 KB ≈ 25k tokens) | implement, review_autofix, orchestrate_poll | Total byte budget for the targeted-file-context block built by `scripts/targeted_file_context.py`. The mechanism pre-loads files named in the plan (implement) / `LAST_RUN_CHANGED_FILES_FILE` (autofix) / conflicted-file allowlist (resolver) so the model's first turn is a write rather than a recon read. Files are processed in source order until the budget is exhausted; a file that would push the cumulative byte count over the cap is NOT head-truncated, it gets a `(NN bytes; would overflow total budget — read with read tool, max_bytes=…, used=…)` marker so the model uses its native targeted-read flow rather than being misled by a truncated head. The Semble overflow fallback (which renders retrieved chunks instead of the marker) is clamped to the same total budget: chunk size is set by the index rather than by the file that triggered the query, so a block is truncated on a line boundary to whatever budget remains and its header says `truncated to NN byte(s) to fit the remaining total budget`; once the budget is exhausted no further query is issued and the remaining paths get the plain marker. This cap is a **total**, never a per-file allowance — the Semble path previously emitted ~129 KB per overflowing file against the 100 KB budget (implement run 33796624872 / issue #3990), which pushed the implement prompt past codex-cli's 1,048,576-character `turn/start` stdin cap and failed every attempt before the editor ran. Every path the caller passes is reported, either inlined or as a marker — there is no separate file-count cap. Set to `0` to disable inlining entirely (block becomes a single-line "(disabled)" marker). Lower this on cost-sensitive repos; raise on small-file repos to fit more verbatim content. |
...
| `SECURITY_AUDIT_FP_EXCLUSIONS` | No | `scripts/security_audit_fp_exclusions.json` | security-audit | Path to the editable JSON false-positive exclusion catalog consumed by `scripts/security_audit.sh`. A copy at this path in the audited repository wins; otherwise a relative path resolves against the staged workflow-source support tree, so consumers inherit the source catalog unless they pin their own. Missing or malformed catalogs fail the run closed rather than silently bypassing exclusions. |
...
| `SECURITY_AUDIT_FP_EXCLUSIONS` | `scripts/security_audit_fp_exclusions.json` | Editable JSON catalog of false-positive suppression rules for the security audit; audited-repo copy wins, staged workflow-source copy is the fallback |
...
| `SECURITY_PASS_LINE_OWNERSHIP` | `project-lines` | Route findings to advisories only when blame plus causal base-to-head diff analysis proves the defect is base-owned; deleted guards and ambiguous control-flow changes remain blocking. `file` restores per-file blocking. |
| `SECURITY_PASS_OWNERSHIP_CONTEXT_LINES` | `3` | Surrounding lines used for ownership classification (0-50). |
| `SECURITY_PASS_ADVISORY_FOLLOWUP_CAP` | `5` | Deprecated compatibility input; ignored. Every eligible pre-existing advisory is attempted on the audit tick; failed rows stay in authoritative state for retry. `0` does not disable filing. |
...
- **Periodic default-branch security audit.** `.github/workflows/security-audit.yml` runs weekly at `08:00 UTC` and on `workflow_dispatch`, stays inert unless `SECURITY_AUDIT_ENABLED=true`, and uses `scripts/security_audit.sh` plus `prompts/mode-security-audit.txt` to audit the checked-out default branch with OWASP Top 10 + STRIDE framing. Surviving findings are appended under dated sections on the stable `AI Security Audit Tracker` issue (`ai:security-audit`, marker `<!-- ai:security-audit-tracker:v1 -->`), and one `ai:security` follow-up issue is opened for every non-excluded finding that meets the confidence gate, with no per-run or weekly cap. Findings are deduplicated against the `<!-- ai:security-finding:<id> -->` marker of every existing `ai:security` issue, open or closed, read with one paginated REST listing (`gh api --paginate repos/<repo>/issues?labels=ai:security&state=all`, one call per 100 issues). `SECURITY_AUDIT_CRON` is documentation-only because GitHub Actions schedule cron is literal YAML, not repo-variable-driven.

- **Credentialless model boundary.** Planner, implementation diagnosis, reviewer/editor, resolver, poller judge, and security-audit model processes run through `scripts/untrusted_process_sandbox.sh`. The sandbox exposes only a loopback provider proxy, hides Git metadata, strips GitHub/runner credentials, and keeps read-only roles from writing the checkout. The proxy accepts only trusted configured models and defaults to 64 requests, one concurrent upstream request, 65,536 output tokens, USD 25 cumulative spend, four fixed HTTP workers, eight queued connections, and a 15-second client read deadline. The sandbox defaults to `TasksMax=128`, `MemoryMax=8G`, no swap, `CPUQuota=200%`, 100M/50M read/write I/O ceilings, `LimitNOFILE=4096`, and a 7,200-second runtime; missing cgroup v2 controllers, invalid overrides, or unsupported mandatory systemd properties fail closed. Implement helpers are copied and SHA-256-bound outside the worktree before model execution, and authenticated publication verifies that immutable set. Model-produced edits cross `scripts/trusted_git_write.sh`, which disables hooks and validates exact local and remote heads.

- **Causal security waivers.** Python findings receive additive `security_audit_causal_scope.v1` metadata from `scripts/security_audit_causality.py`. The same fail-closed validator is used by the audit engine and poller. Fingerprints include module-level registrations, assignments, imports, decorators, callable references, and bounded reverse callers; a nonempty reverse-caller frontier at the depth bound is `indeterminate`, and changed routing/configuration in other languages invalidates a waiver unless static analysis proves the changed path non-causal.

- **Model-writer publication boundaries.** Conflict resolution captures byte anchors around every original marker span plus a deterministic clean-tree mode/blob manifest; missing, binary, delete/modify, or fingerprint-only unbounded conflicts are rejected before model publication. Review-blocked fixes use `review_fix_authorization.v1`, bound to repository, PR, current head, source digests, grounded diff hunks, and stronger multi-reviewer provenance for protected paths. Both the standalone judge and poller stage only selected, span-validated paths; stale, spoofed, malformed, out-of-hunk, mode-changing, deleted, or newly created output is discarded.
...
| `SECURITY_AUDIT_WAIVED_FINDINGS` | _(empty)_ | Optional, findings-JSON-only path to accepted findings. Only a valid engine-generated `waiver_match_key` authorizes suppression; model IDs and location proximity do not. Legacy keyless rows remain prompt-visible but non-authoritative. |
| `SECURITY_AUDIT_WAIVER_LINE_WINDOW` | `40` | Compatibility input retained for existing callers; provenance-key waiver matching does not use a line window. |
...
12f. **Promote cycle callbacks (poller-owned):** Tracking issues labeled `ai:comprehensive-test-pending` belong to the daily promote cycle started by `.github/workflows/promote-main-to-stable.yml` (see Contributing). The label and a marker comment (`apply-analysis-source-doc`, `apply-analysis-role`, `apply-analysis-cycle-baseline-sha`, `apply-analysis-smoke-sha`, `apply-analysis-promote-sha`, `apply-analysis-proving-merge-sha`) are applied by `orchestrate.yml` at tracking-issue creation through its optional `tracking_labels` / `tracking_comment` inputs, so the poller never guesses which project is its own; the bindings are all-or-nothing, and a failed label or comment closes the just-created issue and fails the run (`TRACKING_ISSUE_BINDING_FAILED`) so the next daily tick retries the dispatch. The poller only honours a marker whose comment author is `github-actions[bot]` or has an `author_association` in `COMPREHENSIVE_CYCLE_MARKER_TRUSTED_ASSOCIATIONS` (default `OWNER,MEMBER,COLLABORATOR`); a marker posted by anyone else logs `COMPREHENSIVE_MARKER_UNTRUSTED`, raises one CRITICAL alert, and leaves the callback unhandled with the label in place, so the cycle stays human-gated (`PROMOTE_CYCLE_SKIPPED reason=cycle_in_flight` on later ticks) until a trusted account posts the marker or the label is removed. Two hooks: (1) when a `proving` run completes, `handle_comprehensive_release_callback_if_needed` runs `scripts/apply_analysis_on_main.sh` with role `verifying`, recording `promote_sha` = the default branch tip (which includes the proving merge) and the proving merge commit; the verifying run's own completion does nothing further. A transient dispatcher outcome (`project_in_flight`, `orchestrate_run_in_flight`, `guard_unavailable`, `default_branch_unavailable`, `dispatcher_error`) is retried on later ticks up to `COMPREHENSIVE_VERIFICATION_RETRY_MAX` times. (2) When a `verifying` run's final PR is ready to merge, `comprehensive_promotion_gate_before_final_merge` runs first: every commit between the smoke-tested commit and `promote_sha` must be the proving merge, the merge commit of a merged PR whose head branch is `auto/forward-merge-stable-*` (verified via the API, not by commit subject), or a `COMPREHENSIVE_CYCLE_BOT_LOGINS` commit touching only non-code paths, and the candidate must descend from the smoke-tested commit (compare status `ahead` or `identical`); otherwise the promotion is deferred and the merge proceeds (`COMPREHENSIVE_PROMOTION_DEFERRED`). Clean, it dispatches `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` on `COMPREHENSIVE_RELEASE_WORKFLOW_REF` (default `main`) with `target_sha=promote_sha` and the full gate; the gate fails closed (`COMPREHENSIVE_PROMOTION_FAILED reason=release_workflow_not_pinnable`) if `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` is anything other than `promote-main-to-stable.yml`, because only that workflow honours `target_sha`, and holds the final merge (state `comprehensive_promotion.status=dispatched`, `FINAL_MERGE_BUDGET_ELIGIBLE=0`) until the release run completes: the promotion counts as done only when the `stable` tag moves to (or past) `promote_sha`, which releases the hold, failure or `COMPREHENSIVE_PROMOTION_HOLD_MAX_SECS` releases the hold without promoting, with a CRITICAL alert. A labelled tracking issue without a marker keeps the legacy behaviour: dispatch `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` on completion with `dry_run=false` and any validated `version_tag`/`test_repo` from tracking comments. `.github/workflows/comprehensive-test-and-release.yml` does not apply the label.
...
| `SECURITY_PASS_LINE_OWNERSHIP` | `project-lines` | Route only blame- and causal-diff-proven base-owned findings to advisories; deleted guards and ambiguity remain blocking. Set `file` for per-file blocking. |
| `SECURITY_PASS_OWNERSHIP_CONTEXT_LINES` | `3` | Number of surrounding lines used by ownership classification. Must be an integer from 0 to 50; invalid values fall back to `3`. |
| `SECURITY_PASS_ADVISORY_FOLLOWUP_CAP` | `5` | Deprecated compatibility input; ignored even at `0`. All pre-existing advisory rows are attempted in severity/confidence order; failures remain queued for the next tick. |
...
- `target_ref` (optional; branch to check out and validate instead of the tracking issue's integration branch or the default branch, default empty = unchanged; `/implement-plan-claude` passes its `claude/implement-plan-<slug>` project branch)

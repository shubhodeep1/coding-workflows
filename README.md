...
- **Credentialless model boundary.** Planner, reviewer/editor, resolver, poller judge, and security-audit model processes run through `scripts/untrusted_process_sandbox.sh`. The sandbox exposes only the loopback provider proxy, hides the repository's main, linked-worktree, and nested Git metadata, strips GitHub/runner credentials, and keeps read-only roles from writing the checkout. Model-produced edits are committed and published only by `scripts/trusted_git_write.sh`, which disables hooks, validates the exact local and remote heads, and supplies GitHub credentials through a temporary askpass file without changing the repository remote. Review-time Python packages stay in a secretless Docker volume; post-editor pytest runs only editor-changed Python test targets in a networkless, read-only container, and the volume is removed by the workflow's unconditional cleanup step.

- **Causal security waivers.** Python findings receive additive `security_audit_causal_scope.v1` metadata from `scripts/security_audit_causality.py`. Waivers require a complete matching causal fingerprint covering the enclosing symbol and bounded reverse callers. Added routes/callers, changed decorators or guards, deleted cross-file checks, unsupported languages, parse failures, and ambiguous call graphs keep access-control findings blocking rather than reusing a stale waiver or routing them as pre-existing advisories.

...
| `MODEL_PROVIDER_BROKER_MAX_INPUT_TOKENS` | No | `4194304` | all model-facing phases | Per-request input-token ceiling enforced by `scripts/model_provider_broker.py` (#4090). The broker estimates input tokens from the normalized request body (4 bytes per token, rounded up) and rejects a request over the ceiling with HTTP 400 before forwarding it. The default equals the 16 MiB request-body cap divided by 4, so it never bites below what the body cap already permits. Valid range `1..4194304`. |
| `MODEL_PROVIDER_BROKER_MAX_TOTAL_INPUT_TOKENS` | No | `419430400` | all model-facing phases | Aggregate input-token budget per broker session (#4090). Each admitted request reserves its input estimate and is settled to the provider-reported `usage.prompt_tokens` / `usage.input_tokens`; an exhausted budget returns HTTP 429 and the phase fails closed. The default is `MODEL_PROVIDER_BROKER_MAX_REQUESTS x MODEL_PROVIDER_BROKER_MAX_INPUT_TOKENS` (`100 x 4194304`). Must be at least the per-request ceiling and at most `1000000000`. |

...
- `.ai/review_runtime/pr-${PR_NUMBER}/round-${N}/` is the per-round cache root for the shipped-but-opt-in review_autofix additions: `judge_interim.json`, `verified_rejections.json`, `sticky_findings.json`, `consolidator_parsed.txt`, and behavioural-smoke `synth/` manifests / wrappers. Restore misses and stage failures fail open back to the pre-rollout reviewer/consolidator/editor flow.
...
The rollout gates for these new review-artifact phases remain default-off on this branch: `JUDGE_INTERIM_ENABLED=false`, `CONSOLIDATOR_REJECT_SCHEMA_ENABLED=false`, `CONSOLIDATOR_REJECT_VERIFIER_ENABLED=false`, `STICKY_FINDINGS_ENABLED=false`, `BEHAVIOURAL_SMOKE_FROM_JUDGE_ENABLED=false`, and `REISSUE_PRESERVE_BASELINE_ENABLED=false`. Companion knobs such as `JUDGE_INTERIM_REASONING`, `JUDGE_INTERIM_TIMEOUT_S`, `STICKY_LINE_BUCKET`, `CONSOLIDATOR_REJECT_VERIFIER_REASONING`, `CONSOLIDATOR_REJECT_VERIFIER_BATCH_MAX`, `BEHAVIOURAL_SMOKE_LANG`, and `BEHAVIOURAL_SMOKE_TIMEOUT_S` are tuning-only and matter only when their parent `*_ENABLED` flag is turned on.
...
| `JUDGE_INTERIM_ENABLED` | `false` | Opt-in gate for the per-round judge-interim stage. When enabled it writes `judge_interim.json` under `.ai/review_runtime/pr-${PR_NUMBER}/round-${N}/`; tune only with `JUDGE_INTERIM_REASONING` / `JUDGE_INTERIM_TIMEOUT_S`. |
| `CONSOLIDATOR_REJECT_SCHEMA_ENABLED` | `false` | Opt-in gate for typed `REJECTION_KIND` parsing / enforcement in the review-artifact chain. `CONSOLIDATOR_REJECT_VERIFIER_ENABLED` and its tuning knobs only matter after this schema gate is on. |
| `CONSOLIDATOR_REJECT_VERIFIER_ENABLED` | `false` | Opt-in gate for the reject-verifier pass that writes `.ai/review_runtime/pr-${PR_NUMBER}/round-${N}/verified_rejections.json`. Tune only with `CONSOLIDATOR_REJECT_VERIFIER_REASONING` / `CONSOLIDATOR_REJECT_VERIFIER_BATCH_MAX`. |
| `STICKY_FINDINGS_ENABLED` | `false` | Opt-in gate for cross-round sticky finding carry-over. When enabled, `scripts/review_annotate_sticky.sh` writes `sticky_findings.json` and sticky priors text under `.ai/review_runtime/...`; `STICKY_LINE_BUCKET` is tuning-only. |
| `BEHAVIOURAL_SMOKE_FROM_JUDGE_ENABLED` | `false` | Opt-in gate for judge-driven behavioural smoke synthesis. When enabled, review_autofix populates `.ai/review_runtime/.../synth/` and uses `BEHAVIOURAL_SMOKE_LANG` / `BEHAVIOURAL_SMOKE_TIMEOUT_S` as secondary tuning knobs. |
| `REISSUE_PRESERVE_BASELINE_ENABLED` | `false` | Opt-in gate for `close_and_reissue` / `spot-fix` baseline preservation. When `false`, review-blocked reissues fail open to redo-style issue creation. |

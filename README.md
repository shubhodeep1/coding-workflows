...
| `ENABLE_REVIEWER_XPOLL_SUMMARISER` | No | `true` | review_autofix | When true, each pass-1 reviewer's output is compressed into a compact findings ledger by a cheap OpenRouter summariser model (see `XPOLL_SUMMARISER_MODEL`), spawned in the background the moment the reviewer finishes so summarisation overlaps with the remaining pass-1 fan-out. pass-2 receives the ledger (bounded size) plus a path hint to the full pass-1 files, instead of a 200-line head-truncation of every pass-1 file. Set to `false` to fall back to the legacy `head -n 200` truncation. On summariser HTTP error/timeout/empty content, the reviewer falls back to head-truncation and an aggregated Telegram `WARNING` is sent once at the end of pass-1 — pass-2 is never blocked. |
| `XPOLL_SUMMARISER_MODEL` | No | `openai/gpt-5.4-mini` | review_autofix | OpenRouter model slug used by the cross-pollination summariser. Must be cheap and fast (called once per pass-1 reviewer). Only consulted when `ENABLE_REVIEWER_XPOLL_SUMMARISER=true`. |
| `XPOLL_SUMMARISER_REASONING` | No | `xhigh` | review_autofix | Reasoning effort (`xhigh` / `high` / `medium` / `low`) passed to the summariser model. Keeps strict-summarisation quality high; the summariser is invoked at most once per pass-1 reviewer per iteration, so the cost impact is bounded. |
| `XPOLL_SUMMARISER_LINES_PER_REVIEWER` | No | `160` | review_autofix | Target max output lines per reviewer ledger. Prompt instructs the summariser to collapse related findings (`(N related items)` suffix) rather than drop them when space is tight. |
| `XPOLL_SUMMARISER_CALL_TIMEOUT_SECS` | No | `180` | review_autofix | Per-call HTTP timeout (seconds) for a single summariser request to OpenRouter. Timeouts fall back to head-truncation for that reviewer. |
| `XPOLL_SUMMARISER_WAIT_TIMEOUT_SECS` | No | `300` | review_autofix | Maximum seconds the main workflow waits for all pipelined summarisers to finish after pass-1 completes. On timeout, any missing summaries are synthesised as head-truncated fallbacks and an aggregated Telegram `WARNING` is emitted — pass-2 still starts. |
| `XPOLL_SUMMARISER_MAX_INPUT_LINES` | No | `3000` | review_autofix | Pre-truncation ceiling on the number of lines from a pass-1 reviewer output fed into the summariser prompt. Guards against pathological reviewer outputs exceeding the summariser model context. |
| `XPOLL_SUMMARISER_FALLBACK_LINES` | No | `200` | review_autofix | Lines retained from the head of a pass-1 reviewer output when the summariser fails (HTTP error, timeout, wait-timeout, empty content). Preserves the legacy `head -n 200` behaviour as the degraded mode. |
...
| `Preflight render recovery` | n/a | When template mode is enabled and preflight fails with lint/syntax-class failures, validate_process performs one deterministic renderer recovery pass (`run_template_validation_harness_renderer` + `run_preflight_checks`) before terminal `harness_error`. Recovery is renderer-driven only (no freehand harness generation in this path) and fails open to the existing preflight failure behavior when rerender/recheck cannot recover. |

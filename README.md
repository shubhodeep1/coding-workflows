...
| `JUDGE_PR_DIFFS_TOTAL_MAX_BYTES` | `524288` | Shared byte budget across every PR diff in one judge prompt. Consumed merged PRs first (sorted issue order, so the cache-stable merged block never depends on an in-flight PR's size), then open PRs get the remainder; a PR that finds the budget exhausted is replaced by a `[NOTE: PR diff elided ...]` line. The poller logs `Judge prompt size: N bytes` before every judge exec and warns above 950,000 bytes. Non-numeric or non-positive values fall back to the default |

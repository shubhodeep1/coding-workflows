...
| `APPLY_ANALYSIS_ON_MAIN_ENABLED` | No | `true` | apply-analysis-on-main | Kill switch for `.github/workflows/apply-analysis-on-main.yml`, which hands one pending `analysis/workflow-optimization-*.md` doc to the orchestrator on every push to `main` and labels the tracking issue `ai:comprehensive-test-pending`. `false` logs `APPLY_ANALYSIS_SKIPPED reason=disabled` and dispatches nothing. |
| `APPLY_ANALYSIS_TRACKING_WAIT_SECS` | No | `1800` | apply-analysis-on-main | How long the dispatcher waits for the orchestrator to create the new tracking issue before failing the run (the project still starts; label it by hand). |
...
12f. **Comprehensive release callback (poller-owned):** Tracking issues labeled `ai:comprehensive-test-pending` are handled by `handle_comprehensive_release_callback_if_needed` in the poller, not by a separate workflow phase. The label is applied by `.github/workflows/apply-analysis-on-main.yml`, which on every push to `main` hands one pending `analysis/workflow-optimization-*.md` doc (oldest first) to the orchestrator through `internal-orchestrate.yml`, so a clean end-to-end orchestrator run on the current `main` is the proving run for promotion. On `complete`, the poller dispatches `COMPREHENSIVE_RELEASE_WORKFLOW_FILE` (default `promote-main-to-stable.yml`, which fast-forwards `stable` to `main` and runs the `test-and-mark-stable.yml` gate) on `COMPREHENSIVE_RELEASE_WORKFLOW_REF` (default `main`) with `dry_run=false`, forwarding validated `version_tag`/`test_repo` extracted from tracking comments when present. On `failed` or `validation-failed`, it sends an abort notification and skips dispatch; manual `promote-main-to-stable.yml` remains available as the override. Successful completion and abort paths record `comprehensive_release_callback.{handled,status,handled_at}` and remove `ai:comprehensive-test-pending` best-effort; dispatch failures in the `complete` path leave callback state/label untouched so a later poll cycle can retry. Set `COMPREHENSIVE_RELEASE_WORKFLOW_FILE=test-and-mark-stable.yml` and `COMPREHENSIVE_RELEASE_WORKFLOW_REF=stable` to restore the pre-2026-09 stable-only patch target. `.github/workflows/comprehensive-test-and-release.yml` does not apply the label.
...
Two workflows take the operator out of that loop by default:

- [`apply-analysis-on-main.yml`](.github/workflows/apply-analysis-on-main.yml) runs on every
  push to `main`. It hands exactly one pending `analysis/workflow-optimization-*.md` doc (oldest
  first) to the orchestrator via `internal-orchestrate.yml` and labels the tracking issue
  `ai:comprehensive-test-pending`; when that project completes cleanly the poller's release
  callback (12f) dispatches `promote-main-to-stable.yml`. With no doc, a labelled project still
  in flight, or every remaining doc already dispatched, the push holds (`APPLY_ANALYSIS_SKIPPED
  reason=…`). Processing one doc per push leaves docs for later pushes to execute. The dispatcher
  records `apply-analysis-source-doc: <path>` as a comment on the tracking issue and never
  dispatches a doc that carries that marker or is listed in
  `analysis/recommendation-processing-report.md`, so a project that failed or merged without
  deleting its doc is a human decision rather than a repeated dispatch. Kill switch:
  `APPLY_ANALYSIS_ON_MAIN_ENABLED=false`.

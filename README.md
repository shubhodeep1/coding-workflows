...
| `ENABLE_STALL_HUMAN_TERMINALIZATION` | No | `false` | orchestrate_poll | Legacy compatibility gate for terminal stall escalation policy: when `false`, `escalate_human` is suppressed and the poller repeats the last non-human ladder action; when `true`, human terminalization is allowed. |

...
| `GH_TOKEN` | No | Interactive Claude Code sessions only (CLAUDE.md §23) — Actions workflows get the same variable name from `secrets.GH_PAT` instead | GitHub Personal Access Token. Set as an env var in the Claude Code session environment (not an Actions secret). Lets interactive sessions reach GitHub data the session's built-in tooling cannot — Actions run/job logs, other repos, endpoints absent from the MCP surface — and perform routine repo writes (push, open/update a PR, comment, label) self-serve; merging, force-push, deletions, repo/org administration, and workflow dispatches always require asking the user first. Scopes: classic `repo` + `workflow` (+ `read:org`), or a fine-grained PAT with contents, pull requests, issues, and `actions: read`. The SessionStart hook installs `gh` and reports whether the token authenticates; a missing or invalid token degrades to the `mcp__github__*` tools rather than blocking. |
...
| `WORKFLOW_EDITOR_FALLBACK_MODEL` | No | `openai/gpt-5.5` | plan, implement, review_autofix (editor) | Capacity-fallback editor model. Each codex editor retry loop switches `--model` to this slug on its **final** attempt only, so a sustained OpenRouter/OpenAI per-model TPM saturation on the primary (`WORKFLOW_EDITOR_MODEL`, `gpt-5.6-sol`) can be ridden out — the fallback sits in a different capacity bucket (issue #3515 / run 28640359211). The default `openai/gpt-5.5` is declared in `scripts/codex_model_catalog.json` (fields mirror the primary's OpenAI generation), so `apply_patch`/verbosity resolution stays intact when the switch fires. Set to the primary slug (or clear it) to disable the switch; point at any catalog-declared slug to change the fallback. The implement **repair** pass and the parallel reviewer models are unaffected. |
...
| `REVIEW_CONSOLIDATOR_MODEL` | No | `openai/gpt-5.6-sol` | review_autofix | Model used by the consolidator stage. |
...
| `PROMOTE_CYCLE_GATE_WAIT_SECS` | No | `18000` | promote-main-to-stable (cycle job) | How long the cycle waits for the `gate_only` smoke gate before failing the tick (`PROMOTE_CYCLE_FAILED reason=smoke_gate_timeout`). |
...
| `SECURITY_AUDIT_ENABLED` | No | `true` | security-audit (source repo + consumers via `ai-security-audit.yml`) | Default-on gate for `.github/workflows/security-audit.yml` / `scripts/security_audit.sh` (consumers run it through the synced `workflow-templates/ai-security-audit.yml` wrapper, which needs `OPENROUTER_API_KEY` and optionally `GH_PAT` in the consumer repo). When enabled, the weekly/manual default-branch audit renders `prompts/mode-security-audit.txt`, appends a dated section to the stable `AI Security Audit Tracker` issue (`ai:security-audit`), and opens up to three weekly `ai:security` follow-up issues after confidence-gate and false-positive-exclusion filtering. Set `false` per repo to disable; the workflow then exits cleanly before Codex or GitHub side effects. |
...
|---|---|---|
| `WORKFLOW_EDITOR_MODEL` | `openai/gpt-5.6-sol` (every phase) | Model for code editing / reasoning tasks. The previous legacy editor split (patch-heavy phases on a separate older slug) was retired after openai/codex#11151. See main table above. |
...
| `REVIEW_CONSOLIDATOR_MODEL` | `openai/gpt-5.6-sol` | Model slug used for the consolidator stage in `review_autofix.yml`. |
...
| `SECURITY_AUDIT_ENABLED` | `true` | Default-on gate for the weekly/manual `security-audit.yml` workflow (source repo, and consumers via the `ai-security-audit.yml` wrapper); when enabled it posts surviving default-branch security findings to a stable tracker issue and opens bounded follow-ups. Set `false` per repo to disable |
...
| `WORKFLOW_CHECK_TRIAGE_MODEL` | `WORKFLOW_EDITOR_MODEL` (`openai/gpt-5.6-sol`) | Diagnosis model for check-failure triage. |
...
12g. **Security-pass exhaustion judge and waivers (default on):** When `MAX_SECURITY_PASS_CYCLES` is spent with findings still open, `run_security_pass_inline` no longer terminalizes on sight. `security_pass_exhaustion_judge` renders `prompts/mode-judge-security-pass-exhaustion.txt` with the remaining findings, the reported-findings history, existing waivers, the cycle and round counters, and the project specification, runs it read-only against the audited checkout, and requires exactly one decision per remaining finding. `accept_with_followup` records the finding in `security_pass_waived_findings` (including its versioned `defect_fingerprint`) and files the existing deferred non-blocking follow-up. Waiver suppression requires exactly one current finding and one waiver to share the same `security_defect_context.v1` fingerprint, which hashes the canonical repository path, cited line, normalized category, and a fixed exact code-context window. Finding IDs and line proximity never authorize suppression; legacy fingerprint-less rows remain stored but suppress nothing. `/security-pass-waive <finding_id> [<finding_id> ...]` is all-or-nothing: the caller's current repository `role_name` must be exactly `maintain` or `admin`, every ID must resolve uniquely to a distinct fingerprinted finding reported at the current integration head, and unknown, stale, ambiguous, or fingerprint-less requests are rejected. Permission lookup failures leave the command unconsumed for retry. In `ai:security-pass-failed`, an accepted command resets and re-runs the pass; in `ai:security-pass-fixing`, it persists for the next re-audit. The exhaustion judge, deferred advisory filing, keep-fixing cap, and `/re-security-pass` behavior are otherwise unchanged.
...
|---|---|---|
| `SECURITY_AUDIT_OUTPUT_MODE` | `issues` | Accepts `issues` or `findings-json`. `issues` preserves tracker discovery and updates, labels, bounded follow-up creation, last-audited-SHA bookkeeping, and the existing weekly behavior. |
...
| `VALIDATION_DISCOVERY_MODEL` | `openai/gpt-5.6-sol` | Codex model id. |

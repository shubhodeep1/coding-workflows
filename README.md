...
| `STATE_SNAPSHOT_BRANCH_ENABLED` | No | `false` | orchestrate_poll | When `true`, publish the rolling state-snapshot history to a dedicated branch after writing the artifact. |
...
- `.github/ai/WORKFLOW.md` is opt-in and is loaded by `scripts/load_workflow_overlay.py`. On current HEAD it is validated by `ai-memory/schemas/workflow_overlay.v1.json`, and the shipped schema currently supports only `schema_version` plus `prompt_overrides[]` append/replace entries.
...
- `state-snapshot` is the per-tick orchestrator artifact written by `scripts/build_state_snapshot.py` and uploaded from `orchestrate_poll.yml` when `STATE_SNAPSHOT_ARTIFACT_ENABLED!=false`. Setting `STATE_SNAPSHOT_BRANCH_ENABLED=true` also publishes the rolling history to `STATE_SNAPSHOT_BRANCH_NAME` (default `state-snapshot`) capped by `STATE_SNAPSHOT_HISTORY_DEPTH`.

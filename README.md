...
| `REVIEW_PR_STATE_POLL_INTERVAL_SECS` | No | `10` | review_autofix | Sleep interval in seconds for reviewer watchdog PR-state polling in `scripts/review_run_reviewers.sh`. Must be an integer in `10..3600`; invalid or out-of-range values emit `rate_limit_audit_fallback` warning and fail open to `10`. |
...
| `REVIEW_PR_STATE_POLL_INTERVAL_SECS` | `10` | Sleep interval (seconds) for reviewer watchdog PR-state polls in `scripts/review_run_reviewers.sh`; must be integer `10..3600`, else warn (`rate_limit_audit_fallback`) and fall back to `10` |

...
2. `"done": false` and no `error` → call send_later with delay_minutes 60, initiation own_followup, name "implement-plan <slug>: check-in", message "Check-in: repeat the steps in your first message." Reply `still waiting: <reason>` in one line and end the turn.
...
Waiting on: PR #M | run <id> | issues #a, #b   Check-in: Haiku checker <session id> every hour → next stage "<title>"; safety net <trig_…> (or: fallback / none — resume by re-running the command)

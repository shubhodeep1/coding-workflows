...
4. **Resume, never duplicate.** Run `git ls-remote --heads origin 'claude/implement-plan-issue-<N>-*'` and read each match's `docs/implement-plan/*.md` for a `Source issue: <owner>/<repo>#<N>` line. Also check `docs/completed/issue-<N>-*-plan.md` on the default branch.
   - **This issue's project exists** → if its final PR was closed without merging and the issue is closed `not_planned`, the project was abandoned (see [Issue Mode](.claude/commands/implement-plan-claude.md#issue-mode)): report and stop. Otherwise, if its log's `Check-in:` names a checker session that exists and is neither archived nor failed (`get_session`), report `already in progress: <stage>` and stop. Otherwise follow `/implement-plan-claude` in this session with that project's plan path as `$ARGUMENTS`; it resumes from its log, and reports and stops when the project is already complete.
...
- **Build on the branch the issue names.** A security follow-up is built on its project's branch and a heal issue on `stable` or the named PR branch; only an issue that names no branch, or names one that no longer exists, is built on the default branch. After step 3a the final PR's base is authoritative: workflow failure heal's PR reconcile may re-point it when the heal's source PR merges.

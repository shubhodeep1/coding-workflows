...
2. **Resolve the source.** Everything is read from `shubhodeep1/coding-workflows` at the **`stable`** ref. Resolve that ref to its peeled commit as `UPSTREAM_SHA` and require exactly 40 hexadecimal characters; never accept a caller-supplied SHA without resolving `stable` independently. Also load `scripts/workflow_wrapper_refs.py` from the same ref. Seeding from `main` would hand the consumer files the next sync immediately rewrites; never do it unless the free text explicitly says `@main`, and then say so in the report. If the current session *is* the library repo, `git fetch origin stable` and read from `origin/stable` (never from a dirty working tree), resolving `git rev-parse 'origin/stable^{commit}'`; otherwise resolve/fetch `stable` and each file via `mcp__github__get_file_contents` or `gh api` (§23.D).

...
4. **Compose the seed file set.** Keep non-wrapper assets byte-for-byte. For every wrapper, run the same release renderer as the automatic sync; never hand-edit or duplicate its regex:
   ...
   - The whole `workflow-templates/.claude/` tree → target `.claude/` (commands, `hooks/session-start.sh`, `hooks/pr_merge_status_guard.py`, `hooks/pr_watch_guard.py`, `hooks/pr_check_in_reminder.py`, `scripts/check_in_status.py`, `settings.json`) — the same set the sync's ".claude/ assets" step mirrors afterwards.

...
- **`gh` CLI** — available when `GH_TOKEN` is set (the SessionStart hook installs `gh` and authenticates with it). Use it for one-off reads where the MCP surface is awkward, or for tools that don't have a clean MCP equivalent in this session — e.g. `gh run view --log <run-id>`, `gh run view --log --job <job-id>`, `gh run view --log-failed <run-id>` (failed-step lines only), `gh api repos/<owner>/<repo>/actions/runs/<id>/logs`. The token has read access across the listed repos, so you can fetch run logs, job logs, PRs, issues, commits, and file contents the same way the MCP tools do.

Repo scope: `shubhodeep1/coding-workflows`. If a `gh` call returns 401/403 (auth not actually granted for that resource) or `gh` is missing, fall back to the MCP tool — don't stop the investigation.

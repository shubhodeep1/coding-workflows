...
6. **Screen candidates against in-flight project work (merge-conflict gate).** The recommendation must be startable *now* without colliding with work already in flight from either project runner: the **AI orchestrator** (`/implement-plan-ai`; each project is an open `ai:orchestrator-tracking` issue) and **`/implement-plan-claude`** (each project is a progress log `docs/implement-plan/<slug>.md` plus its `claude/implement-plan-*` PRs):
   ...
   - **Build the in-flight footprint.** Take the union of:
     ...
     - (c) for each in-flight `/implement-plan-claude` project, the files its source plan (the log's `Plan:` line) declares for every phase not yet ticked `[x]` in the log — and, once every phase is ticked (`Stage:` is `conformance`, `security-pass`, `validation`, or `completion`), **every file the plan declares**, because conformance, validation-fix, and blocked-PR fixes land on the plan's files but cannot be predicted in advance. A project with no open PR still counts: its next stage is still incoming changes.
   - **Set aside plans that are already in flight.** A candidate that is itself the source plan of an in-flight project — an open tracking issue names it, or an in-flight `/implement-plan-claude` log's `Plan:` line (or an unlogged PR's slug) points at it — is removed from the candidate list and reported as **in flight** with its owner (`orchestrator issue #N` / `implement-plan <slug>, stage <Stage:>`). It is neither the recommendation nor a conflict-screened candidate: someone is already implementing it.
...
In-flight /implement-plan-claude work: <implement-plan <slug> (log: <Status:>, stage <Stage:>), PR #M (open, files: …)>  (or: none / logs only — GitHub UNAVAILABLE)

In flight — not candidates (only if any):
- <plan> — <orchestrator issue #N / implement-plan <slug>, stage <Stage:>>
...
- <plan> — classified COMPLETE; owned by implement-plan <slug>, stage <Stage:>

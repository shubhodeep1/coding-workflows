...
session to schedule a status read (`send_later`), never to subscribe to PR
activity, and the §25 guard keeps blocking `subscribe_pr_activity`.
"""
...
REMINDER = (
	...
	"unless one is already armed for that PR or the user opted out for this "
	"task: call send_later with delay_minutes=180 and a message naming the "
	"repository, the PR number and URL, and the §26 check-in steps (delegate "
	"the status read to a Sonnet subagent; re-arm silently while the PR is "
	"open; on merged or closed, summarise the next steps, say whether the "
	"session can be closed, and send one PushNotification). Never subscribe "
	"to PR activity (§25)."
)

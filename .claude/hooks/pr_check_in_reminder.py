...
REMINDER = (
	...
	"for the pushed branch exists, make sure it has an hourly status check-in "
	"with this session as its fixer, unless /implement-plan-claude or "
	...
	"\"PR #<n> hand-back\", the PR URL in its prompt). If a live session titled "
	"\"PR #<n> status check-in\" already exists for this repository, register "
	"with it instead of creating another (create_trigger into it two minutes "
	"out, name \"PR #<n> status check-in: subscriber\", naming the hand-back "
	"trigger id, this session's id, and role fixer). Otherwise call "
	"create_session "
	...
	"whose prompt names the repository, the PR number and URL, the subscriber "
	"list (the hand-back trigger id, this session's id, role fixer), the "
	"CLAUDE_FIXER_HANDOFF_AUTHOR_LOGIN value, and the §26.C steps (run "
	".claude/scripts/check_in_status.py --hand-back; while nothing is due, "
	"update_trigger every subscriber's hand-back 7 days ahead and re-arm with "
	"send_later delay_minutes=60; on a due fix on a claude/* head (conflict, "
	"failed check, review hand-off, block label) update_trigger only the "
	"fixer's run_once_at to 1 minute out, never its prompt, so this session "
	"fixes it with /fix-claude-pr, pushes, and registers a new hand-back; on "
	"merged or closed pull every subscriber forward the same way so this "
	"session re-reads the PR state, writes the action-needed report, "
	"renames itself, and sends one PushNotification; confirm delivery "
	"with get_trigger 10 minutes later, and start a fresh Opus 5.5 "
	"/fix-claude-pr session at high effort when the fixer is gone), and "
	"fallback next steps the checker reports itself only if the terminal "
	"hand-back fails. Never use fire_trigger for "

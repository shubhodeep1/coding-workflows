...
1. **Checker** — `create_session` with `source_url` = the repo, `model: claude-haiku-4-5-20251001`, `permission_mode` = this session's mode, `title` = `implement-plan <slug> — waiting: <PR #N | run <id> | issues #a, #b>`, and a standalone prompt built from the [Checker prompt](#checker-prompt) template.
...
- *Workflow run* — `status` is `completed` (any conclusion).
- *Issue list* — every issue is closed or labelled `ai:merged`.

The script issues one REST call per check (at most five when a PR looks stuck), never GraphQL (§15), and prints one JSON line (`done`, `state`, `reason`); exit 2 means the read failed.

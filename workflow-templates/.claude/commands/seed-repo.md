...
2. **Resolve the source.** Everything is read from `shubhodeep1/coding-workflows` at the **`refs/tags/stable`** release tag. Resolve that tag to its peeled commit as `UPSTREAM_SHA` and require exactly 40 hexadecimal characters; never accept a caller-supplied SHA without resolving the tag independently. Also load `scripts/workflow_wrapper_refs.py` from the same commit. Seeding from `main` would hand the consumer files the next sync immediately rewrites; never do it unless the free text explicitly says `@main`, and then say so in the report. If the current session *is* the library repo, run `git fetch --force --no-tags origin refs/tags/stable` and read from the resulting `UPSTREAM_SHA` (never from a dirty working tree), resolving `git rev-parse 'FETCH_HEAD^{commit}'`; otherwise resolve/fetch `refs/tags/stable` and each file via `mcp__github__get_file_contents` or `gh api` (§23.D).
...
4. **Compose the seed file set.** Keep non-wrapper assets byte-for-byte. For every wrapper, run the same release renderer as the automatic sync; never hand-edit or duplicate its regex:
   ...
   - The whole `workflow-templates/.claude/` tree → target `.claude/` (commands, `hooks/session-start.sh`, `hooks/pr_merge_status_guard.py`, `hooks/pr_watch_guard.py`, `hooks/pr_check_in_reminder.py`, `scripts/check_in_status.py`, `scripts/claude_fix_claim.py`, `scripts/stale_routines.py`, `settings.json`) — the same set the sync's ".claude/ assets" step mirrors afterwards.
...
- **`mcp__github__*`** — primary transport (§23.D): `get_file_contents` (`ref=<UPSTREAM_SHA>`) to read templates and probe the target, `create_branch` / `push_files` / `create_pull_request` to land the seed, `list_branches` for the default branch.
...
- **`Read` / `Grep` / `Glob` / `Bash`** — in a library session: read `workflow-templates/` at the resolved `UPSTREAM_SHA`, edit `consumer_repos.json`, and run the local git push for the registration change.
...
- **Stable-rendered wrappers; byte-identical other assets.** Render wrapper refs only through `scripts/workflow_wrapper_refs.py`; copy `.claude/` and `CLAUDE.md` byte-identically from `stable`. No hand edits, no `main`-sourced files (unless explicitly requested), no partial-profile cherry-picking — a manifest subset request other than the three named profiles is a §2 ask.

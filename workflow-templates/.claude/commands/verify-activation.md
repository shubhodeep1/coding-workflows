...
1. **Parse `$ARGUMENTS`.** Extract the issue number / URL, PR refs, plan-doc path, or feature name, and the optional trailing scope (see [Scope](#scope); default full). If there is no concrete reference, stop and ask for one. Restate the parsed reference and the scope in the `Summary`.
...
7. **Verdict.**
   ...
   A narrowed scope uses its own verdict set instead (see [Scope](#scope)): `conformance` grades **CONFORMANT** / **INCOMPLETE** from Steps 4–5 alone; `activation` grades **LIVE** / **DORMANT** from Step 6 alone.

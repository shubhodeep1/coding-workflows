...
case "$(printf '%s' "${SECURITY_AUDIT_ENABLED}" | tr '[:upper:]' '[:lower:]')" in
	1|true|yes|on)
		;;
	*)
...
[ -f "scripts/label_helpers.sh" ] || {
	echo "scripts/label_helpers.sh is required" >&2
	exit 1
}
...
{
	bash scripts/render_prompt.sh prompts/mode-security-audit.txt
	echo
...
python3 - \
	...
	"${MAX_FOLLOWUP_ISSUES_PER_WEEK}" <<'PY'
from __future__ import annotations

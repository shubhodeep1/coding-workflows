...
run_template_validation_harness_renderer()
{
	...
	if ! (cd "${VALIDATE_TRUSTED_SUPPORT_ROOT}" && env -u PYTHONPATH -u GH_TOKEN -u GH_PAT -u GITHUB_TOKEN \
		python3 -E -c 'import yaml, jsonschema, jinja2') >/dev/null 2>&1; then
	...
	if ! renderer_summary="$(env -u GH_TOKEN -u GH_PAT -u GITHUB_TOKEN -u OPENROUTER_API_KEY -u TG_BOT_SECRET -u GITHUB_ENV -u GITHUB_OUTPUT -u GIT_DIR -u GIT_WORK_TREE -u PYTHONPATH "${python3_bin}" "${renderer_script}" \
...
if ! ensure_validate_wrapper; then
  write_result_files "fail" "Unverified validation wrapper" "Custom validation wrapper cannot execute on the host." "harness_error"
  exit 1
fi

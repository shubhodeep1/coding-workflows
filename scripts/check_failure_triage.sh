...
#   CHECK_TRIAGE_CHECK_CONCLUSION  conclusion (failure|timed_out|action_required|...)
...
# Conclusion filter: only act on genuine failures. cancelled / skipped /
# neutral / stale / success are not actionable code-failure signals.
...
PR_JSON="$(gh_retry gh api "repos/${REPO}/pulls/${PR_NUMBER}" 2>/dev/null || echo '')"
if [ -z "${PR_JSON}" ]; then
...
if [ -n "${PARENT_ISSUE}" ]; then
	PARENT_BODY="$(gh_retry gh api "repos/${REPO}/issues/${PARENT_ISSUE}" --jq '.body // ""' 2>/dev/null || echo '')"
...
if [ "${GEN}" -gt "${MAX_DEPTH}" ]; then
	...
	gh_retry gh issue edit "${PR_NUMBER}" --repo "${REPO}" --add-label "${ESCALATED_LABEL}" >/dev/null 2>&1 || true
...
printf '%s' "${PR_JSON}" > "${PR_PAYLOAD_FILE}"
...
if [ -f scripts/collect_pr_check_runs_context.py ]; then
	...
	PYTHONDONTWRITEBYTECODE=1 python3 scripts/collect_pr_check_runs_context.py \
...
if command -v codex >/dev/null 2>&1; then
	cat "${PROMPT_FILE}" | codex --ask-for-approval never \
		-c model_verbosity="${MODEL_VERBOSITY:-low}" \
		-c include_apply_patch_tool=true \
		exec --skip-git-repo-check \
		--model "${MODEL_EDITOR:-openai/gpt-5.4}" \
		--sandbox danger-full-access \
		> "${DIAG_FILE}" 2> >(tee -a "${RUNTIME_DIR}/codex_log.txt" >&2) \
...
if [ ! -s "${DIAG_FILE}" ]; then
	{
		echo "## Summary"
		echo
		echo "Automated diagnosis produced no output. The raw failure context is included below for follow-up."
		echo
		echo "## Evidence"
		echo
		echo '```'
...
# Defensive label creation (idempotent) in case label sync has not run yet.

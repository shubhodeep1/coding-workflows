#!/usr/bin/env bash
# Append the review pipeline iteration summary for review_autofix.yml.
set -euo pipefail
...
if bool_env("AUTOFIX_REVIEW_PIPELINE_FAILURE"):
    ...
    # precedence, sandbox initialization is classified before
    # provider/model recovery failures, "recoverable_failure" is for the
    ...
    elif bool_env("EDITOR_NOOP_SANDBOX_INITIALIZATION_FAILURE"):
        editor_noop_failure_class = "sandbox_initialization_failure"
...
def determine_finalize_reason() -> str:
    ...
    if bool_env("AUTOFIX_REVIEW_PIPELINE_FAILURE"):
        return "review_pipeline_failure"

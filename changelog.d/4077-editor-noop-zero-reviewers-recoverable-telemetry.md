...
The Check 1 regex and its `::warning::` literal (grepped by the e2e poller in `test-and-mark-stable.yml`) are byte-for-byte unchanged; the refusal and recoverable-failure summaries at zero reviewers are both pinned with `EDITOR_NOOP_SUSPICIOUS=true` and their respective specific flags. `determine_finalize_reason` still returns `partial_finalize` ahead of every `editor_noop_*` outcome, so on the real recoverable-failure path the editor slot's `failure_class` is the field to read. Pinned by `tests/test_review_autofix_editor_noop_cascade_contract.py` and `tests/test_review_autofix_review_pipeline_contract.py`; runbook §20.10.2 in `probably_unnecessary_but_read_if_stuck.md` carries the reachability trace.

...
`detect_stalls()` keeps its return shape; the new `detect_stall_latched_issues()` helper feeds the additive `latched` field of the `check-stalls` CLI payload, and `stall_recovery_latch_label()` is the single predicate both call. The standalone loop in `run_standalone_stall_recovery` carries a literal copy of the label list next to a comment pointing at `STALL_RECOVERY_LATCH_LABELS`; keep the two aligned when adding a latch label.

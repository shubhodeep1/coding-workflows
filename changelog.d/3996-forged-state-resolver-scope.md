<!-- changelog: fixed -->
- **Orchestrator state can no longer be forged to expand conflict-resolver write scope.** State comments are authenticated to the active workflow producer and project context, unsigned legacy state is migrated only by the poller, and resolver fingerprints are structurally sanitized before they can add files to the privileged allowlist.

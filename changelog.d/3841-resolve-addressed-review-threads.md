<!-- changelog: added -->
- **The AI review pipeline now resolves the PR review threads it has addressed.** `review_autofix.yml` gains a `Resolve addressed PR review threads` step that marks a reviewer's thread resolved once the editor's `PR comment audit:` section has given that comment a disposition.
...
| New tests | 17 in `tests/test_review_resolve_review_threads.py` |

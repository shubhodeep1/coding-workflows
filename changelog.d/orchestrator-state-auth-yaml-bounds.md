<!-- changelog: security -->
- **Orchestrator state and deterministic contract merges now enforce their trust and resource boundaries.** Tracking state is accepted only from trusted bot or repository-associated authors, non-empty integration branches must match the current tracking issue, and branch mutations repeat that binding check before access.

...
- **Model-facing workflow phases no longer inherit reusable provider or repository credentials.** A bounded loopback broker retains the real OpenRouter key while issue analysis, implementation and repair, validation, workflow audits, and the unprivileged conflict writer receive only an ephemeral run token in clean environments.

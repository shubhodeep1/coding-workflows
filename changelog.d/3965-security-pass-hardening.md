...
- **Model-facing workflow phases no longer inherit reusable provider or repository credentials.** A bounded loopback broker retains the real OpenRouter key while read-only issue/log agents and the unprivileged conflict writer receive only an ephemeral run token in clean environments.

...
The guard parses the staged implementation workflow's helper-staging block, including literal `scripts/...` entries written to `FETCHED_MANIFEST`, rather than carrying another copy of that list, then unions those names with the plan job's own `scripts/.gitignore` entries. `EXCLUDED_RE` is kept as the pre-filter before path extraction, and trailing sentence punctuation is normalized before exact helper matching. The guard had no test coverage before this change; the consumer-owned and implement-only-helper regression cases fail against the previous body.

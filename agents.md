...
- Validation harness template-mode toggle (supersedes the earlier `VALIDATION_USE_TEMPLATES` entry above): `VALIDATION_USE_TEMPLATES` now defaults to `true`; `scripts/validate_process.sh` keeps Phase 1 on deterministic renderer mode (`scripts/render_validation_templates.py`) when truthy (`1|true|yes|on`, case-insensitive), and explicitly setting `VALIDATION_USE_TEMPLATES=false` now returns `raw_status=harness_error` because freehand Codex harness generation was removed. Missing renderer assets still fail with `raw_status=harness_error`; workflow bootstrap continues staging renderer/schema/template assets best-effort for consumer repos.

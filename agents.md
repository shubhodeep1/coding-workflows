...
- **Output**: a deterministic text snapshot at `${PR_CHECK_RUNS_CONTEXT_FILE}` (`${RUNTIME_DIR}/pr_check_runs_context.txt`). Header lines: `PR_CHECK_RUNS_CONTEXT`, `head_sha`, `collection_status` (one of `ready` / `timeout` / `disabled` / `unavailable` / `api_error`), `total_check_runs`, `failed_count`, `incomplete_count`. Failed entries include `name`, `status`, `conclusion`, `app` slug, `html_url`, `details_url`, and the truncated `output.title` / `output.summary`. Incomplete entries are listed with name + status + URL so reviewers know which jobs are still pending.
...
- Disabling the feature (`CHECK_RUNS_AUTOFIX_ENABLED=false`) keeps the file present with `status: disabled` so the preflight check still passes. Do not gate the preflight on `collection_status: ready`.

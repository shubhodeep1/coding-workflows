...
- Orchestrator pre-LLM short-circuit control: `ORCHESTRATE_SHORTCIRCUIT_MAX_CHARS` (default `1200`) allows `orchestrate.yml` to skip decomposer and open one plain issue when description length is below threshold and no multi-step markers are detected. The evaluation resolves repo-file references (e.g. `docs/plan.md`) found in the description and includes their content in both the length and marker checks, so a short body that points to a large planning doc is not incorrectly short-circuited.

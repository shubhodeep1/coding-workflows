...
- `EDITOR_MAX_WALL` default raised from `3300` (55 min) to `6600` (110 min) per editor attempt. The job-level cap (`JOB_TIMEOUT_SECS = 180 * 60`) and the 300s minimum-attempt budget guard remain unchanged, so the retry loop now fits roughly **2** full attempts before the budget guard skips the rest (vs. ~3 short attempts before). This is paired with the orchestrator's per-issue 60-minute scope target (`mode-orchestrate.txt` and `mode-plan.txt`): the planner emits `implementation_minutes_estimate` per issue and refuses to plan for issues estimated above 60 minutes (`BLOCKED: scope-too-large`), so attempt 1 should complete well inside one wall window and attempt 2 exists only as headroom for the watchdog edge case. Consumer repos can still override via `vars.EDITOR_MAX_WALL`.
...
- Worst-case API cost per autofix iteration: `ceil(CHECK_RUNS_WAIT_TIMEOUT_SECS / CHECK_RUNS_POLL_INTERVAL_SECS) + 1` calls (default ≤ 31). Best case (no in-flight checks at start): exactly 1 call. The shared `gh_helpers::gh_retry` wrapper is intentionally not used inside the poll loop — a single transient API error breaks the loop, marks `collection_status: api_error`, and continues fail-open (the goal is not to maximise reliability of an advisory signal at the cost of blocking the autofix run).

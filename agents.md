...
- Review autofix pipelined pass-1 summariser: `ENABLE_REVIEWER_XPOLL_SUMMARISER` (default `true`) replaces the legacy `head -n 200` cross-pollination truncation with a strict LLM-compressed findings ledger (one per pass-1 reviewer). The summariser call (`XPOLL_SUMMARISER_MODEL`, default `openai/gpt-5.4-mini`; `XPOLL_SUMMARISER_REASONING`, default `xhigh`) is spawned as a background process the moment each pass-1 reviewer writes its `success` status, so summarisation overlaps with the remaining pass-1 fan-out and removes summarisation wall time from the critical path before pass-2 starts. Implementation: `scripts/summarize_pass1_output.py` is a self-contained Python helper (uses `urllib`, no non-stdlib deps beyond what the bootstrap already ships) that writes `summary_<safe_model>.txt` and a `.done` sentinel; `scripts/review_run_reviewers.sh::_maybe_spawn_pass1_summariser` launches one per successful pass-1 reviewer, `wait_for_pass1_summaries` blocks between pass-1 and pass-2 up to `XPOLL_SUMMARISER_WAIT_TIMEOUT_SECS` (default `300`), and `drain_summariser_failures_tg` emits a single aggregated Telegram `WARNING` alert summarising any fallback events at pass-1 end. On summariser HTTP error / timeout / empty content / wait-timeout, that reviewer's summary is synthesised as a `head -n XPOLL_SUMMARISER_FALLBACK_LINES` (default `200`) truncation labelled "TRUNCATED FALLBACK" — pass-2 is never blocked and never receives a missing summary. `build_cross_pollination_summary` prefers `summary_<safe_model>.txt` and falls back to head-truncation only when the summariser is disabled or a specific reviewer's summary is absent. The editor prompt (`scripts/review_apply_fixes.sh`) gains on-demand path references to both `summary_<safe_model>.txt` (preferred) and raw `pass1_<safe_model>.txt` under `${PREVIOUS_REVIEWS_DIR}` — they are NOT inlined into the editor bundle (pass-2 review_*.txt files already incorporate pass-1 context). See README "Review autofix reviewer pipeline" env var table for individual knobs (`XPOLL_SUMMARISER_LINES_PER_REVIEWER`, `XPOLL_SUMMARISER_CALL_TIMEOUT_SECS`, `XPOLL_SUMMARISER_MAX_INPUT_LINES`).
...
- Template-mode preflight render recovery (contract): when `HARNESS_MODE=template_generate` and preflight fails with lint-class failures (`PRE_FLIGHT_FAILURE_KIND=lint`), `scripts/validate_process.sh` MUST run one deterministic recovery pass (`run_template_validation_harness_renderer` then `run_preflight_checks`) before terminal `harness_error`. This path is renderer-driven only (no freehand harness generation), emits `PRE_FLIGHT_RECOVERY_STATUS` (`not_attempted|not_applicable|attempted|render_failed|recheck_failed|recovered`) for metadata/logging, and fails open to the existing preflight hard-fail path when recovery cannot succeed.

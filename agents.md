...
- Review issue ledger controls: `REVIEW_LEDGER_ENABLED` (default `1`) toggles lifecycle tracking, `REVIEW_LEDGER_PERSIST_LIMIT` (default `2`) sets the `accepted-residual` threshold (`>=` after increment), and `REVIEW_LEDGER_PATH` (default `.ai/review_issue_ledger.txt`) points to persisted per-PR ledger state. Malformed prior ledgers fail open with `ledger_reset=1` reset semantics.

...
- Validation harness template-mode toggle: `VALIDATION_USE_TEMPLATES` (default `true`) keeps `scripts/validate_process.sh` Phase 1 on deterministic renderer mode (`scripts/render_validation_templates.py`) when truthy (`1|true|yes|on`, case-insensitive). Template mode requires `.ai/validate.yml`, `scripts/render_validation_templates.py`, `scripts/templates/slot_manifest.schema.json`, and `workflow-templates/validation-harness/`; missing assets fail with `raw_status=harness_error` (no fallback path). Freehand Codex harness generation was removed, so explicitly setting `VALIDATION_USE_TEMPLATES=false` now also returns `raw_status=harness_error`. Workflow bootstrap in `.github/workflows/validate.yml` stages renderer/schema/template assets best-effort for consumer repos by default.

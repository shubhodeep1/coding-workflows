...
- `scripts/untrusted_process_sandbox.sh` is the mandatory boundary for planner,
  ...
  provider credentials, or host Codex auth caches. Namespace-control state,
  copied provider credentials, forwarded model outputs, and validator/guard
  writable trees live beneath a canonical non-`/tmp` runner root so
  `PrivateTmp=yes` cannot invalidate host bind paths; unsafe or unavailable
  roots fail closed, and forwarded artifacts are checked as regular files;
  writable trees reject all non-regular objects before crossing back to the host runtime.
...
- `review_autofix.yml` keeps its workflow definition below GitHub's 512,000-byte loading limit by running four extracted step bodies listed in `docs/INVENTORY.md`. Its staging step installs these files before runtime initialization: source-repo runs require the main support snapshot, and consumer runs use the pinned workflow support checkout. A missing script fails closed rather than executing a PR-head copy. Preflight requires all four scripts, and `tests/test_review_autofix_review_pipeline_contract.py` checks the wiring.

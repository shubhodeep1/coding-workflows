...
- Review autofix deterministic pre-review skip: `AUTOFIX_SKIP_DOC_ONLY` (default `true`), `AUTOFIX_SKIP_MAX_ADDITIONS` (default `10`), `AUTOFIX_SKIP_MAX_DELETIONS` (default `10`). The `gate` job in `.github/workflows/review_autofix.yml` runs a final deterministic-skip check after the existing `[skip ai]`/draft/claude-branch/self-triggered-autofix gates. A PR qualifies for skip when EITHER (a) every changed file matches the doc-only glob set — `*.md`, `*.txt`, `*.rst`, `LICENSE*`, `CHANGELOG*`, `docs/**` (case-insensitive suffix on `.md`/`.txt`/`.rst`; case-sensitive prefix on `LICENSE`/`CHANGELOG` to match GitHub's own convention; `docs/**` is depth-agnostic but rooted, so `src/docs/foo.py` does NOT match) — OR (b) total additions ≤ `AUTOFIX_SKIP_MAX_ADDITIONS` AND total deletions ≤ `AUTOFIX_SKIP_MAX_DELETIONS` (both bounds simultaneously, regardless of file types). Override per PR: title contains `[force-review]` OR PR carries the `force-review` label — skip is bypassed and full review runs. When the gate fires, the new sibling job `deterministic-skip-merge` (next sibling after `post-merge-validate-dispatch`) adds `ai:review-skipped` to the PR (color `c5def5`), sets `ai:ready-to-merge` on every linked issue resolved via GraphQL `closingIssuesReferences` with the same body/title regex fallback used by `post-merge-validate-dispatch`, and enables auto-merge (squash) on the PR — mirroring the tail of the codex-agent normal-review path so the orchestrator phase machine still advances. Reviewer panel, editor, summarizer, and merge-conflict resolver are all skipped on this path; if a doc-only/tiny PR happens to have a merge conflict, auto-merge blocks and the orchestrator stall cron (`internal-orchestrate-poll.yml` every ~30 min) re-dispatches via `_dispatch_review_for_conflicts` per existing stall-recovery contracts. The gate fails open on a `gh api .../files` lookup failure (full review runs). API-hygiene contract per §17: the existing PR-state fetch at the top of the gate evaluate step is extended to additionally return `(.labels // []) | map(.name) | join(",")`, so the `force-review` label check piggy-backs on a single call; the only added call on the skip path is the `repos/{r}/pulls/{n}/files` paginated fetch, which is unavoidable (the `/pulls/{n}` endpoint does not include per-file diff stats). Public log prefixes (renames are breaking per §6): `AUTOFIX_GATE_DET_SKIP_OVERRIDE` (force-review override fired), `AUTOFIX_GATE_DET_SKIP_EVAL pr=<n> files=<k> additions=<a> deletions=<d> max_add=<x> max_del=<y> doc_only=<bool> small_diff=<bool> skip=<bool> reason=<docs_only|small_diff|empty>`, `AUTOFIX_GATE_DET_SKIP_FILES_UNAVAILABLE` (fail-open path). New gate outputs (consumed only by the new sibling job): `deterministic_skip` (`true|false`), `det_skip_reason` (`docs_only|small_diff|""`). New `skip_reason` values reported by the gate (additive — existing values preserved per §6): `deterministic_skip_docs_only`, `deterministic_skip_small_diff`. New labels: `ai:review-skipped` (PR audit marker, color `c5def5`), `force-review` (per-PR override, no auto-color set — created on first use by GitHub's default).

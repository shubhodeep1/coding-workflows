...
- The poller's review-blocked judge is decision-only. Its trusted actuator normalizes a bounded `fix` decision to `merge_with_followup` when the source PR is already merged and to `close_and_reissue` after `MAX_REVIEW_BLOCKED_RETRIES`, preventing a noncompliant model response from restarting an exhausted review loop.
...
- `scripts/orchestrate_state_v2.py` signs and verifies canonical state through `sign` / `verify` using `GH_TOKEN` only from the environment. `scripts/verify_integration_fingerprints.py --export-resolver-safe-fingerprints` emits only bounded escaped-literal fingerprints with normalized non-`.git` repository paths whose positive issue IDs are present as merged wave entries; malformed entries are omitted before resolver scope expansion.

...
- Promote-cycle comments use the separate `comprehensive_cycle_marker.v1` domain in `scripts/orchestrate_state_v2.py`. The signed envelope binds source/role/dispatcher and smoke-run evidence plus promotion SHAs; readers require producer ID `41898282`, and the poller re-fetches the smoke run before promotion. `COMPREHENSIVE_CYCLE_MARKER_TRUSTED_ASSOCIATIONS` remains accepted for compatibility but grants no authority.

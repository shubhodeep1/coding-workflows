...
- Missing log-archive soft-fail normalization (`scripts/collect_workflow_logs.py`): log-archive fetches (`GET /repos/{repo}/actions/runs/{run_id}/logs`) treat HTTP `404`/`410` and empty archive payload as partial-data conditions, normalized to `partial_data:missing_log_archive repository=<repo> run_id=<id> detail=<...>` and reported under `errors.scope=logs` without failing the collector run. Retry behavior remains for transient transport/platform failures (`rate limit`, timeout, connection, `502/503/504`) with `LOG_ARCHIVE_FETCH_RETRIES=3` and linear `LOG_ARCHIVE_FETCH_BACKOFF_SECONDS=1.0`.
- Validation harness template-mode toggle (`supersedes`): `VALIDATION_USE_TEMPLATES` (default `true`) runs `scripts/validate_process.sh` Phase 1 in deterministic renderer mode (`scripts/render_validation_templates.py`) when truthy (`1|true|yes|on`, case-insensitive). Setting `VALIDATION_USE_TEMPLATES=false` now returns `raw_status=harness_error` because freehand Codex harness generation was removed. Template mode requires `.ai/validate.yml`, `scripts/render_validation_templates.py`, `scripts/templates/slot_manifest.schema.json`, and `workflow-templates/validation-harness/`; missing assets fail with `raw_status=harness_error`. Workflow bootstrap in `.github/workflows/validate.yml` stages renderer/schema/template assets best-effort for consumer repos.

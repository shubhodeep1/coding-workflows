...
- Validation harness template-mode toggle: `VALIDATION_USE_TEMPLATES` (default `true`) runs `scripts/validate_process.sh` Phase 1 in deterministic renderer mode (`scripts/render_validation_templates.py`) when truthy (`1|true|yes|on`, case-insensitive). Setting `VALIDATION_USE_TEMPLATES=false` now returns `raw_status=harness_error` because freehand Codex harness generation was removed. Template mode requires `.ai/validate.yml`, `scripts/render_validation_templates.py`, `scripts/templates/slot_manifest.schema.json`, and `workflow-templates/validation-harness/`; missing assets fail with `raw_status=harness_error`. When `.ai/validate.yml` is absent, Phase 0 discovery (or a sanity-checked hints cache) is materialized into `.ai/validate.yml` in the runner's working tree before Phase 1 renders, so the renderer never reports a missing manifest while a discovered or cached hints file is on disk; the materialized file is not pushed back to the consumer repo. Workflow bootstrap in `.github/workflows/validate.yml` stages renderer/schema/template assets best-effort for consumer repos.

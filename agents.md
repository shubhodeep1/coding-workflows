...
- Promote-cycle comments use the separate `comprehensive_cycle_marker.v1` domain in `scripts/orchestrate_state_v2.py`. The signed envelope binds source/role/dispatcher and smoke-run evidence plus promotion SHAs; before bot-authored posting, `orchestrate.yml` verifies it from immutable support and requires the signed smoke actor to match the authenticated dispatcher, preventing copied-envelope replay through `tracking_comment`. Readers require producer ID `41898282`, and the poller re-fetches the smoke run before promotion. The API-reported display title attests the coverage-affecting gate inputs (`gate_only`, `skip_e2e`, `dry_run`, `test_repo`, and `review_workflow_file`) instead of trusting the signed envelope's self-report alone. `COMPREHENSIVE_CYCLE_MARKER_TRUSTED_ASSOCIATIONS` remains accepted for compatibility but grants no authority.

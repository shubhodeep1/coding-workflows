...
- Renderer-supported families in `workflow-templates/validation-harness/` are `python-mongo-flask`, `node-hardhat-solidity`, and `python-mongo-repo-checks`; keep `.ai/validate.yml type` aligned with one of these when template mode is enabled. Hint-only types used in freehand examples (for example `http-server`) are not valid template-family IDs.

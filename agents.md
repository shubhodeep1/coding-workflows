...
- Review PR-state poll interval: `REVIEW_PR_STATE_POLL_INTERVAL_SECS` (default `10`) controls `sleep` cadence for reviewer watchdog PR-state polling in `scripts/review_run_reviewers.sh`; accepts integer `10..3600`, and invalid/out-of-range values emit `rate_limit_audit_fallback` warning then fail open to `10`.

...
- Orchestrator PR autofix flow (renames are breaking per §6): the per-PR autofix loop in `.github/workflows/review_autofix.yml` and the orchestrator-level judge cycle cap in `scripts/orchestrate_poll_process.sh` jointly enforce a per-PR-kind contract gated by `ORCH_PR_AUTOFIX_FLOW_ENABLED` (default `true`, repo var). Classification happens inside the existing `retrigger_guard` step (id `retrigger_guard`) by reading `headRefName` / `baseRefName` from `${PR_META_FILE}` and matching them against `ORCH_INTEGRATION_BRANCH_PATTERN` (default `^orchestrator/project-`, mirrors the conventional integration branch naming `orchestrator/project-<TRACKING_ISSUE_NUMBER>` set in `.github/workflows/orchestrate.yml`). Three modes: (1) `orch_intermediate` — head matches AND base matches: cap `MAX_AUTOFIX_ITERATIONS` at `1` (override of the legacy default), emit new step output `skip_judge=true` so the rb_judge step (line ~3077, condition extended with `&& steps.retrigger_guard.outputs.skip_judge != 'true'`), the two `Mark linked issues review-blocked` / `Post review-blocked comment on PR` autofix-exhaustion steps, and the `Telegram review-blocked judge decision` step are all skipped; the `Mark linked issues ready to merge` / `Enable auto-merge on PR` / `Telegram success` step gates were widened from `max_iterations_reached != 'true'` to `(max_iterations_reached != 'true' || skip_judge == 'true')` so a PR that hits the cap on its second run (the `[ai-autofix]`-triggered re-run) still ends with auto-merge enabled. (2) `orch_final` — head matches AND base does NOT match: keep `MAX_AUTOFIX_ITERATIONS=3` inner loop; bypass the `MAX_JUDGE_CYCLES` cap in `orchestrate_poll_process.sh` (~ line 9831 block) when the orchestrator state has `final_merge_pr` non-empty AND `final_merge_status="pending"` — the `[final-merge] judge cap bypassed (final-PR loop active: PR #<n>, status=pending); JUDGE_STALL_CYCLES=<m> > MAX_JUDGE=<k>, proceeding to judge invocation.` log line audits each cycle the bypass fires. (3) `other` — neither: legacy behavior preserved (`MAX_AUTOFIX_ITERATIONS=3`, `MAX_JUDGE_CYCLES=25`). New step output (consumed by the gating changes): `skip_judge` (`true|false`); new step output for observability: `orch_pr_mode` (`orch_intermediate|orch_final|other`). The existing `force_rb_judge` short-circuit (set by the stall poller via `review_rb_judge_dispatch.yml`) remains the stall-recovery escape hatch and is honoured even on intermediate PRs: when `FORCE_RB_JUDGE=true` the retrigger guard suppresses `skip_judge` (forces it to `false`), so the rb_judge step runs and decides merge / fix / close-and-reissue per the existing stall-recovery contract. This preserves the only path that can rescue a stuck `ai:review-blocked` intermediate PR — without this carve-out, an intermediate PR that stalls past `STALL_THRESHOLD_REVIEW_BLOCKED_MINUTES` would have no judge available to escape. API hygiene (§15): zero new GH API calls — the classifier reuses `${PR_META_FILE}` which is already populated by the existing PR-state fetch; the orchestrator-side bypass reuses `${STATE_FILE}` jq reads which are already cached per poll iteration. Rollback: set `ORCH_PR_AUTOFIX_FLOW_ENABLED=false` (repo var) — both files honour the kill switch on the next workflow run / poll tick and revert to the legacy uniform `3-autofix → judge → cap-25` flow for every PR. Failure modes: (a) intermediate PR with persistent CI failure — the 1 autofix round is the single chance to fix CI; if CI stays red, the merge gate (`_pr_checks_completed` at `~ line 6370`) blocks the merge and existing stall-recovery contracts (`STALL_THRESHOLD_DONE_MINUTES`, `STALL_THRESHOLD_REVIEW_BLOCKED_MINUTES`) recover the issue; (b) final PR with bad judge verdict loop — the bypass means no automatic ceiling, so set `ORCH_PR_AUTOFIX_FLOW_ENABLED=false` to restore the cap and intervene; (c) branch naming mismatch — classifier falls through to `other` and the legacy flow applies; override `ORCH_INTEGRATION_BRANCH_PATTERN` to match.

...
- Review floor-rules controls: `REVIEW_FLOOR_RULES_ENABLED` (default `1`) enables/disables floor-rule extraction; `REVIEW_FLOOR_KEYWORDS_FILE` (default empty string) points to an optional keyword catalog, with missing/unreadable files failing open to built-in keywords.
- Review consolidator controls: `REVIEW_CONSOLIDATOR_ENABLED` (default `1`) enables/disables consolidator execution; `REVIEW_CONSOLIDATOR_MODEL` (default `openai/gpt-5.4-mini`), `REVIEW_CONSOLIDATOR_REASONING` (default `medium`), `REVIEW_CONSOLIDATOR_TIMEOUT_SECS` (default `300`), and `REVIEW_CONSOLIDATOR_MAX_TOKENS_OUT` (default `16000`) configure the consolidator invocation/output cap.
- Review parser control: `REVIEW_PARSER_FAILOPEN` (default `1`) keeps parser failures non-fatal (`1`) or hard-fails parser-stage errors (`0`).
- Review ledger controls: `REVIEW_LEDGER_ENABLED` (default `1`) toggles ledger lifecycle tracking, `REVIEW_LEDGER_PERSIST_LIMIT` (default `2`) sets promotion threshold to `accepted-residual`, and `REVIEW_LEDGER_PATH` (default `.ai/review_issue_ledger.txt`) sets the persisted ledger file path.
- Review prompt-shaping controls: `REVIEW_REVIEWER_CHECKLIST_ENABLED` (default `1`) toggles checklist/header injection and `REVIEW_REVIEWER_ITERATION_SCOPING` (default `1`) toggles non-first-iteration scoped reviewer prompts.
...
## 21. Review Pipeline Invariants

- Stage order in `review_autofix.yml` is fixed: `review_floor_rules.sh` → `review_consolidate.sh` → `review_parse_consolidator.sh` → `review_issue_ledger.sh`.
- Consolidator is advisory and never gating: disable/missing-script/timeout/error paths continue fail-open with `consolidator_raw.txt` available (possibly empty).
- Floor-rule findings are non-skippable classifier outputs when present: editor action must either fix or explicitly reject with rationale.
- Authority contract in `scripts/review_apply_fixes.sh`: `reviewer_bundle.txt` is authoritative; `review_issues.txt`, `ledger_status.txt`, and `floor_tags.txt` are advisory context only.
- Parser contract in `scripts/review_parse_consolidator.sh`: default fail-open (`REVIEW_PARSER_FAILOPEN=1`) emits empty `review_issues.txt` on parser failure; strict mode (`0`) hard-fails parser-stage errors.
- Ledger persistence contract in `scripts/review_issue_ledger.sh`: lifecycle statuses are `NEW`, `PERSISTING`, `FIXED`, `RESURGENT`, `accepted-residual`; still-present non-residual issues reaching `REVIEW_LEDGER_PERSIST_LIMIT` transition to `accepted-residual`, and their blocks are removed from `review_issues.txt` while state remains in the persisted ledger.
- Consolidator-override line convention is literal and required when used: `CONSOLIDATOR_OVERRIDDEN: <reason>`.
- Editor machine signal contract is literal: `Change status:` (`edited` | `not-edited`) is authoritative for downstream no-op/retrigger decisions.
